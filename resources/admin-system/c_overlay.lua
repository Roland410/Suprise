...
function getAdminTitle(thePlayer)
	...
	local text = ({ "RP Segéd", "Kezdõ Admin", "Admin", "Haladó Admin", "Fõ Admin", "Tulajdonos", nil, nil, nil, "Scripter" })[adminLevel] or "Játékos"
...
local function updateGUI()
	if show then
		...
		local onduty = getElementData( localPlayer, "adminlevel" ) <= 6 and "Nincs szolgálatban" .. " :: " or ""
		if getElementData( localPlayer, "adminduty" ) == 1 then
			onduty = "Szolgálatban" .. " :: "
		end
		textString = getAdminTitle( localPlayer ) .. " :: " .. onduty .. getElementData( localPlayer, "gameaccountusername" ) .. " :: " .. duty .. "/" .. online .. " Adminok :: " .. leadduty .. "/" .. lead .. " Fõ Adminok" .. ( getElementData( localPlayer, "adminlevel" ) <= 6 and ( " :: " .. ( openReports - handledReports ) .. " megválaszolatlan jelentések" .. reporttext .. " :: " .. handledReports .. " megválaszolt jelentések" .. ownreporttext ) or "" )
	end
end
