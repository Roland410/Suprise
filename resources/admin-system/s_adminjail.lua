	...
	if (exports.global:isPlayerAdmin(thePlayer)) then
		...
		if not (who) or not (minutes) or not (...) or (minutes<1) then
			...
			if (targetPlayer) then
				...
				outputChatBox("Büntetést Adtál " .. targetPlayerName .. " -nak,-nek" ..   minutes .. "Percet.", thePlayer, 255, 0, 0)
				...
				if commandName ~= "sjail" then
					if (hiddenAdmin==0) then
						...
						outputChatBox("AdmJail: " .. adminTitle .. " " .. playerName .. " Bezárta " .. targetPlayerName .. "  " .. minutes .. " Percre.", getRootElement(), 255, 0, 0)
						outputChatBox("AdmJail: Oka: " .. reason, getRootElement(), 255, 0, 0)
					else
						outputChatBox("AdmJail: Hidden Admin jailed " .. targetPlayerName .. "" .. minutes .. "Percre.", getRootElement(), 255, 0, 0)
						outputChatBox("AdmJail: Oka: " .. reason, getRootElement(), 255, 0, 0)
					end
				end
	...
	if(isElement(jailedPlayer)) then
		...
		if (timeServed) then
			...
			if (timeLeft<=0) then
				...
				outputChatBox("Az idő letelt, Viselkedj legközelebb!", jailedPlayer, 0, 255, 0)
				...
				exports.global:sendMessageToAdmins("AdmJail: " .. getPlayerName(jailedPlayer) .. " Büntetést " .. genderm .. " Börtön idő")
			else
...
function unjailPlayer(thePlayer, commandName, who)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		if not (who) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Játékosnév/ID]", thePlayer, 255, 194, 14)
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, who)
			...
			if (targetPlayer) then
				...
				if not (jailed) then
					outputChatBox(targetPlayerName .. " Nincs adminjailban", thePlayer, 255, 0, 0)
				else
					local query = mysql:query_free("UPDATE accounts SET adminjail_time='0', adminjail='0' WHERE id='" .. mysql:escape_string(accountID) .. "'")
					...
					setElementPosition(targetPlayer, 2340.5097,2456.9287,14.9687)
					...
					outputChatBox("" .. username .. " kivett a börtönbõl, Legközelebb viselkedj!", targetPlayer, 0, 255, 0)
					exports.global:sendMessageToAdmins("Figyelem: " .. targetPlayerName .. "-t kivette a börtönbõl " .. username .. "")
				end
			end
		end
	end
...
function jailedPlayers(thePlayer, commandName)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		outputChatBox("~~~~~~~~~ Adminbörtön ~~~~~~~~~", thePlayer, 255, 194, 15)
		
		local players = exports.pool:getPoolElementsByType("player")
		local count = 0
		for key, value in ipairs(players) do
			if getElementData(value, "adminjailed") then
				outputChatBox("[JAIL] " .. getPlayerName(value) .. " Börtönben " .. tostring(getElementData(value, "jailadmin")) .. " által, ült " .. tostring(getElementData(value, "jailserved")) .. " percet és," .. tostring(getElementData(value,"jailtime")) .. "  perc van hátra", thePlayer, 255, 194, 15)
				outputChatBox("[JAIL] Oka: " .. tostring(getElementData(value, "jailreason")), thePlayer, 255, 194, 15)
				count = count + 1
			elseif getElementData(value, "pd.jailtimer") then
				outputChatBox("[Börtönben] " .. getPlayerName(value) .. " ült " .. tostring(getElementData(value, "pd.jailserved")) .. " percet és " .. tostring(getElementData(value, "pd.jailtime")) .. " perc van még hátra", thePlayer, 0, 102, 255)
				count = count + 1
			end
		end
