	...
	function( id )
		if exports.global:isPlayerAdmin( client ) or  exports.global:isPlayerGameMaster( client ) then
			if id and tonumber( id ) then
				...
				if result then
					if tonumber( result.appstate ) == 1 then -- application is open
						...
						outputChatBox( result.username .. " jelentkezése elbírálás alatt van.", client, 255, 0, 0 )
					end
				end
			end
			...
			if result then
				...
				if #table == 0 then
					outputChatBox( "Nincsenek új jelentkezések.", client, 0, 255, 0 )
				else
...
addEventHandler( "apps:update", getRootElement( ),
	function( account, state, reason )
		if (exports.global:isPlayerAdmin( client ) or exports.global:isPlayerGameMaster( client )) and ( state == 2 or state == 3 ) and tonumber( account.id ) and tonumber(account.appid )then
			...
			outputChatBox( ( state == 3 and "Elfogadrad " or "Elutasítottad " ) .. account.name .. " jelentkezését ("..tostring(account.appid)..").", client, 0, 255, 0 )
	...
	function( account )
		if (exports.global:isPlayerAdmin( client ) or exports.global:isPlayerGameMaster( client )) and tonumber(account.id) then
			...
			if result then
				...
				outputChatBox( "Hiba az előzmények betöltésében.", source, 255, 0, 0)
			end
		end
	end
)
		...
		if exports.global:isPlayerAdmin( client ) or  exports.global:isPlayerGameMaster( client ) then
			if id and tonumber( id ) then
				...
				if result then
					...
					if #table <= 1 then
						outputChatBox( "Ennek a játékosnak csak egy jelentkezése van.", client, 0, 255, 0 )
					else
