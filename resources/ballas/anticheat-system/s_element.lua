			...
			if (isProtected) then
				...
				if (sourceClient) then
					if (getElementType(sourceClient) == "player") then
						...
						local playername = getPlayerName(source) or "Valami"
						-- Get rid of the player
						local msg = "[AdmWarn] " .. getPlayerName(sourceClient) .. " manipulálni próbálta az adatfolyamot ezért ki lett bannolva."
						local msg2 = " (victim: "..playername.." index: "..index .." újérték:".. tostring(newData) .. " régiérték:".. tostring(oldValue)  ..")"
						--outputConsole(msg)
						--outputConsole(msg2)
