...
function withdrawMoneyPersonal(amount)
	...
	if money >= 0 then
		...
		outputChatBox("Te kivettél" .. exports.global:formatMoney(amount) .. "Ft-ot a Számládról", client, 255, 194, 14)
		exports.logs:dbLog(client, 25, client, "WITHDRAW " .. amount)
	else
		outputChatBox( "Nem.", client, 255, 0, 0 )
	end
end
...
function depositMoneyPersonal(amount)
	...
	if exports.global:takeMoney(client, amount) then
		...
		outputChatBox("Te befizettél" .. exports.global:formatMoney(amount) .. " Ft-ot a számládra", client, 255, 194, 14)
...
function depositMoneyBusiness(amount)
	...
	if exports.global:takeMoney(client, amount) then
		...
		if exports.global:giveMoney(theTeam, amount) then
			...
			outputChatBox("Te befizettél" .. exports.global:formatMoney(amount) .. "Ft-ot a Céges számládra.", client, 255, 194, 14)
...
function transferMoneyToPersonal(business, name, amount, reason)
	...
	if not dbid and not reciever then
		outputChatBox("Személy nem található,kérjük adja meg a teljes nevét", client, 255, 0, 0)
	else
		if business then
			local theTeam = getPlayerTeam(client)
			if -getElementData(theTeam, "id") == dbid then
				outputChatBox("Magadnak nem tudsz utalni", client, 255, 0, 0)
				return
			end
			if exports.global:takeMoney(theTeam, amount) then
		...
		outputChatBox("Te utaltál" .. exports.global:formatMoney(amount) .. "Ft-ot neki "..(business and "business" or "personal").."a számlájára "..name..(string.sub(name,-1) == "s" and "'" or "'s").." account.", client, 255, 194, 14)
