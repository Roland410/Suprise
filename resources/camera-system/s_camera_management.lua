...
function createTrafficCam(thePlayer, commandName, maxSpeed)
	if (exports.global:isPlayerLeadAdmin(thePlayer)) then
		if not (maxSpeed) then
			outputChatBox("Használat: /" .. commandName .. " [Sebességkorlátozás KM/h-ban]", thePlayer, 255, 194, 14)
		else
			if (tonumber(maxSpeed) > 35) then
				...
				outputChatBox("Rendben, eltárolva. Most állítsd be a hatáskörét: /trafihatosugar.", thePlayer, 255, 0, 0)
			else
				outputChatBox("35 KM/h-nál nagyobbnak kell lennie a sebességkorlátozásnak.", thePlayer, 255, 0, 0)
			end
		end
	end
end
addCommandHandler("addtrafi", createTrafficCam, false, false)
...
function setTrafficCamRadius(thePlayer, commandName)
	if (exports.global:isPlayerLeadAdmin(thePlayer)) then
		...
		if (checkPoint) then
			...
			if (radius > 99) then
				outputChatBox("Túl nagy a sugár.", thePlayer, 255, 0, 0)
			else
				...
				if (id) then
				    outputChatBox("A trafipax lerakása sikeres.", thePlayer, 0, 255, 0)
					loadOneTrafficCam(id)
				else
					outputChatBox("A trafipax lerakása sikertelen.", thePlayer, 255, 0, 0)
				end
			end
		end
	end
end
addCommandHandler("trafihatosugar", setTrafficCamRadius, false, false)
...
function delTrafficCam(thePlayer, commandName)
	if (exports.global:isPlayerLeadAdmin(thePlayer)) then
		...
		if (colShape) then
			...
			outputChatBox("Trafi #" .. id .. " törölve.", thePlayer)
			destroyElement(colShape)
		else
			outputChatBox("Nem vagy egy trafipax közelében sem.", thePlayer, 255, 0, 0)
		end
	end
end
addCommandHandler("deltrafi", delTrafficCam, false, false)

function getNearbyTrafficCams(thePlayer, commandName)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		local posX, posY, posZ = getElementPosition(thePlayer)
		outputChatBox("Közeli trafipaxok:", thePlayer, 255, 126, 0)
		local found = false
		...
		for k, theColshape in ipairs(exports.pool:getPoolElementsByType("colshape")) do
			...
			if (isSpeedcam) then
				...
				if (distance<=20) then
					...
					if (enabled) then
						outputChatBox("   ID " .. dbid .. " - Bekapcsolva.", thePlayer, 255, 126, 0)
					else
						outputChatBox("   ID " .. dbid .. " - Kikapcsolva.", thePlayer, 255, 126, 0)
					end
					found = true
				end
			end
		...
		if not (found) then
			outputChatBox("   Nincsen közeli trafipax.", thePlayer, 255, 126, 0)
		end
	end
end
addCommandHandler("kozelitrafik", getNearbyTrafficCams, false, false)
