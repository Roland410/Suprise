...
function createComputerGUI()
	...
	if not (wComputer) then
		wComputer = guiCreateWindow(X, Y, Width, Height, "Számítógép", false)
		...
		chatButton = guiCreateButton ( 0.03 , 0.96 , 0.14 , 0.03 , "Chat használata" , true , desktopImage )
	...
	if(key == "left") then
		if(source == chatButton) then
			if (guiGetInputEnabled( )) then
				...
				showCursor(false)
				outputChatBox("Chatbox Aktiválba, Most tudsz írni!", 0, 255, 0, true)
				outputChatBox("Nyomd meg az M-et hogy visszatérj a PDA-ba!")
			else 
				guiSetInputEnabled(true)
				showCursor(false)
				outputChatBox("PDA Aktiválva most tudsz böngészni!", 0, 255, 0, true)
			end
		end
	end
end
...
-------------------------------------------------------------------------------- Internet   --------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
local now = tostring("www.hankook.hu")
local previous = tostring("www.hankook.hu")
...
function openEmailWindow()
	...
	if not (wEmail) then
		...
		-- Username
		username_label = guiCreateLabel(13,54,63,16,"Felhasználó:",false,wEmail)
		username_input = guiCreateEdit(78,52,200,22,"JKiss Bela",false,wEmail)
		
		address_label = guiCreateLabel(13,88,63,16,"Szolgáltató:",false,wEmail)
		
		address_1 = guiCreateRadioButton(79,88,200,16,"@flymta.eu",false,wEmail)
		...
		elseif (teamName=="Los Santos International Flight School") then
			address_f = guiCreateRadioButton(79,121,200,16,"@lsiflight.sa",false,wEmail)
		elseif (teamName=="Zaffa Incorporated") then
			address_f = guiCreateRadioButton(79,121,200,16,"@zaffa.sa",false,wEmail)
		end
		...
		-- Login
		email_login_button = guiCreateButton(67,184,70,26,"Bejelentkezés",false,wEmail)
		addEventHandler("onClientGUIClick",email_login_button,function()
			...
			if (tostring(username) =="") then -- username field
				guiSetText(email_error_label,"Írd be a felhasználód!")
			...
			elseif (tostring(guiGetText(password_input))=="") then
				...
				elseif (guiRadioButtonGetSelected(address_f)==true) then
					...
					elseif (teamName=="Zaffa Incorporated") then
						full_username = tostring(username.."@zaffa.sa")
					end
				end
		...
		-- Register
		email_register_button = guiCreateButton(164,184,70,26,"Regisztráció",false,wEmail)
		addEventHandler("onClientGUIClick",email_register_button,function()
			...
			if (tostring(username) =="") then -- username field
				guiSetText(email_error_label,"Írd be a felhasználód!")
			elseif (guiRadioButtonGetSelected(address_1)==false) and (guiRadioButtonGetSelected(address_2)==false) and (guiRadioButtonGetSelected(address_f)==false) then-- provider
				guiSetText(email_error_label,"Ki kell választanod a szolgáltatód!")
			elseif (tostring(guiGetText(password_input))=="") then
				guiSetText(email_error_label,"Írd be a jelszavad!")
			else
				local password = tostring(guiGetText(password_input)) -- password
...
function name_error()
	guiSetText(email_error_label, "A felhasználónév már létezik!")
end
addEvent("name_in_use",true)
addEventHandler("name_in_use", getLocalPlayer(),name_error)

function login_error()
	guiSetText(email_error_label, "Érvénytelen jelszó vagy felhasználó!")
end
addEvent("loginError",true)
addEventHandler("loginError", getLocalPlayer(),login_error)

function not_leader()
	guiSetText(email_error_label, "Nemtudsz regisztrálni ezzel a szolgáltatóval!")
end
...
function show_inbox(inbox_table, accountName)
	if not (wEmail) then
		...
		inbox_tab = guiCreateTab("Bejövő",email_tab_panel)
		outbox_tab = guiCreateTab("Elküldött",email_tab_panel)
		send_tab = guiCreateTab("Levél Írása",email_tab_panel)
		...
		------------------
		-- To:
		new_message_to_label = guiCreateLabel(0.1,0.1,0.2,0.05,"Címzett:",true,send_tab)
		new_message_to_input = guiCreateEdit(0.2,0.1,0.3,0.05,"",true,send_tab)
		-- Subject:
		new_message_subject_label = guiCreateLabel(0.1,0.2,0.2,0.05,"Tárgy::",true,send_tab)
		new_message_subject_input = guiCreateEdit(0.2,0.2,0.5,0.05,"",true,send_tab)
		-- Message:
		new_message_content_label = guiCreateLabel(0.1,0.3,0.3,0.05,"Üzenet:",true,send_tab)
		new_message_content = guiCreateMemo(0.1,0.4,0.8,0.45,"",true,send_tab)
		-- Send Button
		send_button = guiCreateButton(0.4,0.9,0.2,0.08,"Küldés",true,send_tab)
	...
	inbox_grid_date = guiGridListAddColumn(inbox_grid,"Dátum",0.16)
	inbox_grid_sender = guiGridListAddColumn(inbox_grid,"Küldő",0.3)
	inbox_grid_subject = guiGridListAddColumn(inbox_grid,"Tárgy",0.5)
	...
	if not(inbox_message_datetitle_label) then
		...
		inbox_message_datetitle_label = guiCreateLabel(0.02,0.4,0.96,0.1,"Dátum:",true,inbox_tab)
		...
		inbox_message_fromtitle_label = guiCreateLabel(0.5,0.4,0.96,0.1,"Küldő:",true,inbox_tab)
		...
		inbox_message_subjecttitle_label = guiCreateLabel(0.02,0.45,0.96,0.1,"Tárgy:",true,inbox_tab)
	...
	if not (check_mail_button) then
		check_mail_button = guiCreateButton(0.2,0.9,0.2,0.08,"Levél adatok",true,inbox_tab)
		...
		inbox_delete_mail_button = guiCreateButton(0.4,0.9,0.2,0.08,"Törlés",true,inbox_tab)
		...
		--REPLY BUTTON
		inbox_reply_button = guiCreateButton(0.6,0.9,0.2,0.08,"Továbbitás",true,inbox_tab)
...
function show_outbox(outbox_table,accountName)
	...
	outbox_grid_date = guiGridListAddColumn(outbox_grid,"Dátum",0.16)
	outbox_grid_sender = guiGridListAddColumn(outbox_grid,"Küldő",0.3)
	outbox_grid_subject = guiGridListAddColumn(outbox_grid,"Tárgy",0.5)
	...
	if not(outbox_message_datetitle_label) then
		-- Static labels showing date and sender
		outbox_message_datetitle_label = guiCreateLabel(0.02,0.4,0.96,0.1,"Dátum:",true,outbox_tab)
		...
		outbox_message_totitle_label = guiCreateLabel(0.5,0.4,0.96,0.1,"Címzett:",true,outbox_tab)
		...
		outbox_message_subjecttitle_label = guiCreateLabel(0.02,0.45,0.96,0.1,"Tárgy:",true,outbox_tab)
	...
	if not (outbox_delete_mail_button) then
		...
		outbox_delete_mail_button = guiCreateButton(0.4,0.9,0.2,0.08,"Törlés",true,outbox_tab)
...
function invalid_address()
	guiSetText(new_message_to_input, "Érvénytelen Cím!")
end
