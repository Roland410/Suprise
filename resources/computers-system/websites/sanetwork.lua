...
function www_hankook_hu()
	
	-- Webpage Properties
	---------------------
	local page_length = 764
	guiSetText(internet_address_label, "Hankook autószervíz és használtautókereskedés honlapja TEL: 511")
	guiSetText(address_bar,"www.hankook.hu")
	...
	-- Nav Links
		...
		local jobs_link = guiCreateLabel(460,52,85,22,"Szolgáltatások",false,bg)
		...
		addEventHandler("onClientGUIClick",jobs_link,function()
			local url = tostring("www.hankook.hu/szolgaltatas") -- Put hyperlink url in quotation marks
			get_page(url)
		end,false)
	...
	local latest_news_title = guiCreateLabel(10,92,450,20,"Hírek",false,bg)
	guiSetFont(latest_news_title,"default-bold-small")
	
	--------------- headline ------------
	local top_story_headline =  guiCreateLabel(10,279,440,16,"HANKOOK AUTÓSZERVÍZNÉL HASZNÁLTAUTÓK KAPHATÓAK! TEL.:  vagy LS",false,bg)
	guiLabelSetColor(top_story_headline,0,0,0)
	addEventHandler("onClientGUIClick",top_story_headline,function()
			local url = tostring("www.hankook.hu/hirek")
			get_page(url)
		end,false)
	-- date
	local top_story_date =  guiCreateLabel(10,292,440,16,"Szombat, Július 6, 2013",false,bg)
	guiSetFont(top_story_date,"default-bold-small")
	guiLabelSetColor(top_story_date,132,5,16)
	-- Summary
	local top_story_summary =  guiCreateLabel(10,306,440,64,"Jöjjön és vásároljon olcsó és luxusautót egyaránt.",false,bg)
	...
	local other_news_title = guiCreateLabel(10,362,450,20,"Egyéb Hírek",false,bg)
	...
	local top_links_title = guiCreateLabel(480,92,180,20,"Egyéb Honlapok",false,bg)
	...
	local footer_text = guiCreateLabel(14,739,254,14,"FLYMTA      2013 Hankook Autószervíz. All Rights Reserved.",false,bg)
...
function www_hankook_hu_szolgaltatas()
	
	-- Webpage Properties
	---------------------
	local page_length = 764
	guiSetText(internet_address_label, "Hankook autószervíz és használtautókereskedés honlapja TEL: 511")
	guiSetText(address_bar,"www.hankook.hu/szolgaltatas")
	...
	-- Nav Links
		...
		-- Jobs
		--local jobs_link_bg = guiCreateStaticImage(443,49,62,22,"websites/colours/1.png",false,san_logo)
		local jobs_link = guiCreateLabel(460,52,85,22,"Kezdőlap",false,bg)

		guiSetFont(jobs_link,"default-bold-small")
		guiLabelSetHorizontalAlign(jobs_link,"center")
		addEventHandler("onClientGUIClick",jobs_link,function()
			local url = tostring("www.hankook.hu") -- Put hyperlink url in quotation marks
			get_page(url)
		end,false)
	...
	-- Left Column
	------------- Latest News -------------
	local top_story_image = guiCreateStaticImage(95,90,280,189,"websites/images/csavar.png",false,bg)
	local top_story_mask_t = guiCreateStaticImage(5,112,450,5,"websites/colours/1.png",false,bg)
	local top_story_mask_b = guiCreateStaticImage(5,290,450,26,"websites/colours/1.png",false,bg)
	
	local latest_news_header_bg = guiCreateStaticImage(0,90,455,22,"websites/colours/3.png",false,bg)
	local latest_news_title = guiCreateLabel(10,92,450,20,"Szolgálatások:",false,bg)
	guiSetFont(latest_news_title,"default-bold-small")
	
	--------------- headline ------------
	local top_story_headline =  guiCreateLabel(10,279,440,16," Teljeskörű autójavítás, Lámpacsere, Autófestés, Kerékcsere, Spoilerek és egyéb tuningok felrakása.",false,bg)
	guiLabelSetColor(top_story_headline,0,0,0)
	addEventHandler("onClientGUIClick",top_story_headline,function()
			local url = tostring("www.hankook.hu/hirek")
			get_page(url)
	...
	guiLabelSetColor(top_story_date,132,5,16)
	-- Summary
	local top_story_summary =  guiCreateLabel(10,306,440,64,"Tuning érdekli? Hívja az 511-et!",false,bg)
	guiLabelSetColor(top_story_summary,38,38,38)
	guiLabelSetHorizontalAlign(top_story_summary,"left",true)
	...
	guiSetFont(top_links_title,"default-bold-small")
		...
		-- Országos Mentőszolgálat
		local top_link_3_bp = guiCreateStaticImage(474,153,6,6,"websites/images/dots/red_dot.png",false,bg)
		local top_link_3 = guiCreateLabel(490,146,142,16,"Országos Mentőszolgálat Honlapja",false,bg)
		guiLabelSetColor(top_link_3,38,38,38)
		addEventHandler("onClientGUIClick",top_link_3,function()
				local url = tostring("www.mento.hu")
				get_page(url)
...
function www_sanetwork_sa_fortunate_escape_in_mysterious_plane_crash()
	...
	-- Author
	local author =  guiCreateLabel(10,304,440,16,"- William Hunt",false,bg)
	guiSetFont(author,"default-small")
	guiLabelSetColor(author,38,38,38)
	
	-- Article
	local article =  guiCreateLabel(10,318,440,410," Jelntkezzen szerelőnek e-mailben!",false,bg)
...
function www_sanetwork_sa_impound_lot_auction()
	...
	-- Article
	local article =  guiCreateLabel(10,318,440,410,"Today, an auction was held at Los Santos Towing and Recovery. They auctioned off automobiles that have been sitting in their lot and no one has retrieved, or could not afford to un-impound. With car styles ranging from sports cars, trucks and SUV's, to your average Greenwood, LST&R pulled in quite a crowd. People from all over Los Santos came to place their bids in hopes of winning their dream car. Around twenty cars up for sale were lined up against a wall.\
\
...
\
After accepting the $2 million deal, the cars were moved off site to one of Mr. Vanderburg's estates. People were infuriated as they began to cuss and swear at the LST&R employees, but the damage had been done and Mr. Vanderburg's deal accepted. The situation began to get very heated after the multi-million dollar purchase. LST&R was forced to call in S.W.A.T and the LSES for precautionary reasons. The auction was put to a close and citizens were told to disperse. Only a few stuck around to express their rights to the police. All is quiet in Los Santos now; lets hope Mr. Vanderburg enjoys his new cars.",false,bg)
	guiLabelSetColor(article,38,38,38)
	guiLabelSetHorizontalAlign(article,"left",true)
...
function www_sanetwork_sa_corruption_in_bt_and_r()
	...
	---------------------
	local page_length = 764
	guiSetText(internet_address_label, "SANetwork.sa - Corruption In Los Santos Towing & Recovery - Waterwolf")
	...
	-- Title
	local title_header_bg = guiCreateStaticImage(0,90,455,22,"websites/colours/3.png",false,bg)
	local title = guiCreateLabel(10,92,450,20,"Corruption In Los Santos Towing & Recovery",false,bg)
	guiSetFont(title,"default-bold-small")
	...
	-- Article
	local article =  guiCreateLabel(10,318,440,410,"Bests Towing and Recovery, or better known as LST&R, is the only towing company in the city of Los Santos. They keep the streets clear of improperly parked vehicles and assist the Emergency Services and Police Department to clear up accident scenes to reopen roads for the public. They are an asset to the city to keep the roadways running and safe.\
\
Recently, there has been a shocking and appalling discovery made about LST&R. In the late night on January Fourth the SAN received an anonymous call about an illegal activity being conducted by the towing company. We were shocked to find that just outside of the SAN building, next to Pier 69, there were LST&R trucks towing away the many vehicles legally parked on the road.\
\
The cars being towed were brightly colored sports cars which Im sure anyone who has traveled to pier 69 or Rodeo has seen. All of these cars were perfectly legally parked according to the laws of Los Santos. Despite being legally parked, the LST&R drivers were ordered to tow all of these cars by one of the company managers.\
\
...
function www_sanetwork_sa_corruption_in_bt_and_r2()
	...
	---------------------
	local page_length = 764
	guiSetText(internet_address_label, "Hankook autószervíz és használtautókereskedés honlapja")
	guiSetText(address_bar,"www.hankook.hu")
...
-- Nav Links
	...
	-- Article
	local article =  guiCreateLabel(10,290,440,410,"One of the LST&R drivers spotted Richard Banks as he photographed the trucks going into and out of the lot. The driver was hostile toward Richard to the point where the driver started to insult and threaten him. The Richard attempted to leave but was chased by the infuriated LST&R driver. After a lengthy chase, Richard was cornered by the driver. The two exhausted men argued before the driver demanded to go onto air. An uplink was made with the SAN studio to the drivers phone and an interview between the CEO of SAN, Josh Stafford and the driver was transmitted live.\
\
The interview between the two men showed what LST&R was really doing. The driver stated that there had been multiple crashes in the area due to those cars. The reason that the company gave for the cars being towed was that they were posing a safety threat to the ublic and had to have been taken care of. During the interview, Josh asked if the company possessed a warrant to tow the vehicles. There was a long pause and we eventually found that no warrant had been obtained for the seizure of vehicles.\
\
After a long debate, the interview ended and the driver returned to work. The SAN employee stayed at the LST&R building for some time after the fact and reported hearing messages from the LST&R radio saying keep towing dem cars! showing that there was no intention to stop, even when it was made abundantly clear that the actions being taken were illegal.\
\
With the reckless and uncaring drivers in this city, LST&R is a vital resource to the well being of the city. Cars would literally line the streets and block essential areas if it were not for the LST&R drivers. The comments given to me by several employees shows that they did not want any part in the towing. They were ordered and forced to by a corrupt manager. It is essential that the leadership and decisions of this company are fair and legal to keep this organization functional and the city streets clear.",false,bg)
	guiLabelSetColor(article,38,38,38)
	guiLabelSetHorizontalAlign(article,"left",true)
