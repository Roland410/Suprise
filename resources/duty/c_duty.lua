...
function openDutyWindow()
	...
	if #availablePackages > 0 then
		...
		outputChatBox("Itt nem tudod felvenni a felszerelésed!")
	end
end
...
function selectPackageGUI_open(availablePackages)
	...
	gui["_root"] = guiCreateWindow(left, top, windowWidth, windowHeight, "Felszerelés kiválasztása", false)
	...
	for pIndex, packageDetails in ipairs(availablePackages) do
		...
		gui[guiTabName.."-cancel"] = guiCreateButton(10, 400, 200, 35, "Bezár", false, gui[guiTabName])
		...
		gui[guiTabName.."-spawn"] = guiCreateButton(440, 400, 200, 35, "Felvesz", false, gui[guiTabName])
