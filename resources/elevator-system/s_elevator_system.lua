...
function createElevator(thePlayer, commandName, interior, dimension, ix, iy, iz, car)
	if (exports.global:isPlayerSuperAdmin(thePlayer)) then
		if not (interior) or not (dimension) or not (ix) or not (iy) or not (iz) or not (car) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Interior ID] [Dimension ID] [X] [Y] [Z] [KOCSIVAL]", thePlayer, 255, 194, 14)
			outputChatBox("SYNTAX: [Kocsi értékek: 0 - csak játékos, 1 - Járékos és jármű, 2 - Csak jármű, 3 - nem bejárat]", thePlayer, 255, 194, 14)
		else
			...
			if id then
				local query = mysql:query_free("INSERT INTO elevators SET id='" .. mysql:escape_string(id) .. "', x='" .. mysql:escape_string(x) .. "', y='" .. mysql:escape_string(y) .. "', z='" .. mysql:escape_string(z) .. "', tpx='" .. mysql:escape_string(ix) .. "', tpy='" .. mysql:escape_string(iy) .. "', tpz='" .. mysql:escape_string(iz) .. "', dimensionwithin='" .. mysql:escape_string(dimensionwithin) .. "', interiorwithin='" .. mysql:escape_string(interiorwithin) .. "', dimension='" .. mysql:escape_string(dimension) .. "', interior='" .. mysql:escape_string(interior) .. "', car='" .. mysql:escape_string(car) .. "'")
				if (query) then
					...
					outputChatBox("Lift létrehozva ID #" .. id .. "!", thePlayer, 0, 255, 0)
				end
			else
...
function enterElevator(player, pickup)
	...
	if cartp == 3 then
		outputChatBox("Ez nem bejárat.", player, 255, 0,0, true)
		return
	end
	...
	if isInPickup ( player, pickup ) and ( ( vehicle and cartp ~= 0 and getVehicleOccupant( vehicle ) == player ) or not vehicle ) then
		if not vehicle and cartp == 2 then
			outputChatBox( "Ez a bejárat csak járműveknek.", player, 255, 0, 0 )
			return
		end
		
		if getElementModel( pickup ) == 1314 then
			outputChatBox( "Ez az interior ki van kapcsolva.", player, 255, 0, 0 )
			return
		end
		...
		if locked then
			outputChatBox("Ajtók bezárva.", player, 255, 0,0, true)
			return
		end
			...
			if getElementData( player, "adminduty" ) ~= 1 and not exports.global:hasItem( player, 5, getElementData( thePickup, "dbid" ) ) then
				...
				if fee and fee > 0 then
					if not exports.global:takeMoney( player, fee ) then
						outputChatBox( "Nincs elég pénzed a belépéshez.", player, 255, 0, 0 )
						return
					else
		...
		if vehicle then
			...
			for i = 0, getVehicleMaxPassengers( vehicle ) do
				...
				if p then
					...
					exports['anticheat-system']:changeProtectedElementDataEx(p, "realinvehicle", 1)
		...
		if vehicle then
			setTimer(function()
				if isElement(vehicle) then
					...
					exports['anticheat-system']:changeProtectedElementDataEx(player, "realinvehicle", 1)
