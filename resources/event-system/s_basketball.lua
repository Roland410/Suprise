...
addCommandHandler("passzol",passBall)
...
addCommandHandler("dobás",shootBall)
...
function basketHelp(thePlayer, commandName)
	for _,boundryShape in pairs(boundries) do
        if isElementWithinColShape(thePlayer,boundryShape) then
			outputChatBox("/labda: Ezzel a parancsal tudod a földről felvenni a labdát.", thePlayer, 250, 0, 0)
			outputChatBox("/dobás: Ezzel a parancsal tudod eldobni a labdát", thePlayer, 250, 0, 0)
			outputChatBox("/passzol: Ezzel a parancsal tudod passzolni a társadnak a labdát.", thePlayer, 250, 0, 0)
			outputChatBox("Tipp: csak fuss a kosárba, miközben a labda a kezedbe van.", thePlayer, 250, 0, 0)
			outputChatBox("Mindig sportszerűen  játsz", thePlayer, 250, 0, 0)
		end
	end
end
addCommandHandler("kosárhelp", basketHelp)

function enterBoundries(thePlayer, matchingDimension)
	if getElementType ( thePlayer ) == "player" and matchingDimension then 
		outputChatBox("Üdvözöljük a kosárlabdapályán kezdéshez ird be /kosárhelp", thePlayer, 0, 250, 0)
	end
end
