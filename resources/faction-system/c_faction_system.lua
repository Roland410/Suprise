...
function showFactionMenu(motd, memberUsernames, memberRanks, memberPerks, memberLeaders, memberOnline, memberLastLogin, memberLocation, factionRanks, factionWages, factionTheTeam, note)
	if (gFactionWindow==nil) then
		...
		tabOverview = guiCreateTab("Áttekintés", tabs)
		
		-- Make members list
		gMemberGrid = guiCreateGridList(0.01, 0.015, 0.8, 0.905, true, tabOverview)
		
		colName = guiGridListAddColumn(gMemberGrid, "Név", 0.23)
		colRank = guiGridListAddColumn(gMemberGrid, "Rang", 0.20)
		colOnline = guiGridListAddColumn(gMemberGrid, "Állapot", 0.115)
		colLastLogin = guiGridListAddColumn(gMemberGrid, "Utolsó bejelentkezés", 0.17)
		colLeader = guiGridListAddColumn(gMemberGrid, "Szint", 0.1)
		...
		if (factionType==2) or (factionType==3) or (factionType==4) or (factionType==5) or (factionType==6) then
			...
			colWage = guiGridListAddColumn(gMemberGrid, "Fizetés (Ft)", 0.1)
		else
		...
		for k, v in ipairs(memberUsernames) do
			...
			if (not memberLastLogin[k]) then
				...
				elseif (memberLastLogin[k]==1) then
					login = tostring(memberLastLogin[k]) .. " napja"
				else
					login = tostring(memberLastLogin[k]) .. " napja"
				end
			end
			...
			if (memberLeaders[k]) then
				...
				guiGridListSetItemText(gMemberGrid, row, colLeader, "Tag", false, false)
			end
		...
		guiSetText(gFactionWindow, tostring(teamName) .. " (" .. counterOnline .. " / " .. (counterOnline+counterOffline) .. " tag van online)")
		
		-- Make the buttons
		if (localPlayerIsLeader) then
			gButtonKick = guiCreateButton(0.825, 0.076, 0.16, 0.06, "Kirúgás", true, tabOverview)
			--gButtonLeader = guiCreateButton(0.825, 0.1526, 0.16, 0.06, "Leader állapot változtatása", true, tabOverview)
			gButtonPromote = guiCreateButton(0.825, 0.2292, 0.16, 0.06, "Rang növelése", true, tabOverview)
			gButtonDemote = guiCreateButton(0.825, 0.3058, 0.16, 0.06, "Rang csökkentése", true, tabOverview)
			
			if (factionType==2) or (factionType==3) or (factionType==4) or (factionType==5) or (factionType==6) then
				gButtonEditRanks = guiCreateButton(0.825, 0.3824, 0.16, 0.06, "Rang és fizetés állítása", true, tabOverview)
			else
				gButtonEditRanks = guiCreateButton(0.825, 0.3824, 0.16, 0.06, "Rang állítása", true, tabOverview)
			end
			
			gButtonEditMOTD = guiCreateButton(0.825, 0.459, 0.16, 0.06, "Napi üzenet szerkesztése", true, tabOverview)
			gButtonInvite = guiCreateButton(0.825, 0.5356, 0.16, 0.06, "Tag felvétele", true, tabOverview)
			gButtonRespawn = guiCreateButton(0.825, 0.6122, 0.16, 0.06, "Járművek respawnolása", true, tabOverview)
			...
			if factionPackages and #factionPackages > 0 then
				gButtonPerk = guiCreateButton(0.825, 0.6878, 0.16, 0.06, "/duty kezelése", true, tabOverview)
			...
			tabNote = guiCreateTab("(Leader) Jegyzet", tabs)
			eNote = guiCreateMemo(0.01, 0.02, 0.98, 0.87, note or "", true, tabNote)
			gButtonSaveNote = guiCreateButton(0.79, 0.90, 0.2, 0.08, "Mentés", true, tabNote)
			addEventHandler("onClientGUIClick", gButtonSaveNote, btUpdateNote, false)
	end
			
			--gButtonQuit = guiCreateButton(0.825, 0.7834, 0.16, 0.06, "Frakció elhagyása", true, tabOverview)
			gButtonExit = guiCreateButton(0.825, 0.86, 0.16, 0.06, "Bezár", true, tabOverview)
...
function btEditRanks(button, state)
	if (source==gButtonEditRanks) and (button=="left") and (state=="up") then
		...
		wRanks = guiCreateWindow(x, y, width, height, "Rangok és Fizetések", false)
		
		local y = 0
		for i=1, 20 do
			y = ( y or 0 ) + 23
			lRanks[i] = guiCreateLabel(0.05 * width, y + 3, 0.4 * width, 20, "Rang " .. i .. ": Rang & Fizetés: ", false, wRanks)
		...
		bRanksSave = guiCreateButton(0.05, 0.900, 0.9, 0.045, "Mentés", true, wRanks)
		bRanksClose = guiCreateButton(0.05, 0.950, 0.9, 0.045, "Bezárás", true, wRanks)
...
function saveRanks(button, state)
	if (source==bRanksSave) and (button=="left") and (state=="up") then
		...
		if (found) then
			outputChatBox("A rangok nevei speciális karaktert tartalmaznak, például: '@.;", 255, 0, 0)
		elseif not (isNumber) then
			outputChatBox("A fizetéseknek számoknak kell lennie.", 255, 0, 0)
		else
			local sendRanks = { }
			local sendWages = { }
...
function btEditMOTD(button, state)
	if (source==gButtonEditMOTD) and (button=="left") and (state=="up") then
		if not (wMOTD) then
			...
			wMOTD = guiCreateWindow(x, y, width, height, "Napi üzenet", false)
			tMOTD = guiCreateEdit(0.1, 0.2, 0.85, 0.1, tostring(theMotd), true, wMOTD)
			
			guiSetInputEnabled(true)
			
			bUpdate = guiCreateButton(0.1, 0.6, 0.85, 0.15, "Frissítés", true, wMOTD)
			addEventHandler("onClientGUIClick", bUpdate, sendMOTD, false)
			
			bMOTDClose= guiCreateButton(0.1, 0.775, 0.85, 0.15, "Ablak bezárása", true, wMOTD)
...
function sendMOTD(button, state)
	if (source==bUpdate) and (button=="left") and (state=="up") then
		...
		if (found1) or (found2) then
			outputChatBox("Tiltott karaktert tartalmaz az üzeneted.", 255, 0, 0)
		else
...
function btInvitePlayer(button, state)
	if (source==gButtonInvite) and (button=="left") and (state=="up") then
		if not (wInvite) then
			...
			wInvite = guiCreateWindow(x, y, width, height, "Játékos felvétele", false)
			tInvite = guiCreateEdit(0.1, 0.2, 0.85, 0.1, "Név részélet", true, wInvite)
			addEventHandler("onClientGUIChanged", tInvite, checkNameExists)
					
			lNameCheck = guiCreateLabel(0.1, 0.325, 0.8, 0.3, "Nincs vagy több ilyen nevű játékos van.", true, wInvite)
			...
			bInvite = guiCreateButton(0.1, 0.6, 0.85, 0.15, "Felvesz", true, wInvite)
			guiSetEnabled(bInvite, false)
			addEventHandler("onClientGUIClick", bInvite, sendInvite, false)
			
			bCloseInvite = guiCreateButton(0.1, 0.775, 0.85, 0.15, "Ablak bezárása", true, wInvite)
...
function btQuitFaction(button, state)
	if (button=="left") and (state=="up") and (source==gButtonQuit) then
		...
		if (numLeaders==1) and (isLeader) then
			outputChatBox("Valakit ki kell nevezned leadernek mielőtt kilépsz. Te vagy az egyedüli leader.", 255, 0, 0)
		else
			local sx, sy = guiGetScreenSize() 
			wConfirmQuit = guiCreateWindow(sx/2 - 125,sy/2 - 50,250,100,"Kilépés megerősítése", false)
			local lQuestion = guiCreateLabel(0.05,0.25,0.9,0.3,"Valóban ki akarsz lépni a " .. getTeamName(theTeam) .. " nevű frakcióból?",true,wConfirmQuit)
			guiLabelSetHorizontalAlign (lQuestion,"center",true)
			local bYes = guiCreateButton(0.1,0.65,0.37,0.23,"Igen",true,wConfirmQuit)
			local bNo = guiCreateButton(0.53,0.65,0.37,0.23,"Nem",true,wConfirmQuit)
...
function btKickPlayer(button, state)
	if (button=="left") and (state=="up") and (source==gButtonKick) then
		...
		if (playerName==getPlayerName(getLocalPlayer())) then
			outputChatBox("Nem tudod magadat kirúgni, lépj ki.", thePlayer)
		elseif (playerName~="") then
			...
			outputChatBox("Kirúgtad őt: "..tostring(playerName)..".", 0, 255, 0)
			triggerServerEvent("cguiKickPlayer", getLocalPlayer(), playerName)
		else
			outputChatBox("Válassz ki először egy tagot.")
		end
	end
end
...
function btButtonPerk(button, state)
	if (button=="left") and (state=="up") and (source==gButtonPerk) then
		...
		if (#factionPackages == 0) then
			outputChatBox("Ez az opció nem elérhető a frakciódnak.")
			return
		end
		...
		wPerkWindow = guiCreateWindow(x, y, width, height, "Felszerelés jogosultságok: "..playerName, false)
		...
		if not factionPerks then
			outputChatBox("Nem lehet betölteni "..playerName.. " felszereléshez való jogosultságait")
			factionPerks = { }
		end
		...
		bPerkSave = guiCreateButton(0.05, 0.900, 0.9, 0.045, "Mentés", true, wPerkWindow)
		bPerkClose = guiCreateButton(0.05, 0.950, 0.9, 0.045, "Bezárás", true, wPerkWindow)
...
function btToggleLeader(button, state)
	if (button=="left") and (state=="up") and (source==gButtonLeader) then
		...
		if (playerName==getPlayerName(getLocalPlayer())) then
			outputChatBox("Nem tudod a leadert elvenni magadtól.", thePlayer)
		elseif (playerName~="") then
			local row = guiGridListGetSelectedItem(gMemberGrid)
			
			if (currentLevel=="Leader") then
				guiGridListSetItemText(gMemberGrid, row, tonumber(colLeader), "Tag", false, false)
	...
	if (button=="left") and (state=="up") and (source==gButtonPromote) then
		...
		if (playerName~="") then
			...
			if (currRankNumber==20) then
				outputChatBox(playerName .. " jelenleg a legmagasabb rangon van.", 255, 0, 0)
			else
				guiGridListSetSelectedItem(gMemberGrid, 0, 0)
	...
	if (button=="left") and (state=="up") and (source==gButtonDemote) then
		...
		if (playerName~="") then
			...
			if (currRankNumber==1) then
				outputChatBox(playerName .. " a legalacsonyabb rangon van.", 255, 0, 0)
			else
				guiGridListSetSelectedItem(gMemberGrid, 0, 0)
				guiGridListSetItemText(gMemberGrid, row, colRank, arrFactionRanks[currRankNumber-1], false, false)
				if colWage then
