...
function tv(player)
	...
	if hasTV or (getDim > 0) or isPlayerFreecamEnabled(player) then
		...
		elseif getCameraTarget(player) ~= player then
			outputChatBox("Jelenleg nem tudod bekapcsolni a TV módot.", player, 255, 0, 0)
		elseif isTVRunning() then
			setPlayerFreecamEnabled(player)
		else
			outputChatBox("Jelenleg nem fut TV műsor.", player, 255, 194, 14)
		end
	else
		outputChatBox("Nincs TV a közeledben.", player, 255, 194, 14)
	end
end
...
addCommandHandler("movetv",
	function(player)
		if getElementData(player, "faction") == 20 then
			if isTVRunning() then
				outputChatBox("Már fut az adás.", player, 255, 0, 0)
			else
				-- I like to ... move it!
				...
				if moveCamera(posX, posY, posZ + 1, posInt, posDim) then
					for k, v in ipairs( getElementsByType( "player" ) ) do
						if getElementData(v, "faction") == 20 then
							outputChatBox("[TV] ".. getPlayerName(player):gsub("_", " ") .. " megváltoztatta a kamera pozicióját.", v, 200, 100, 200)
						end
					end
				else
	...
	function(player)
		if getElementData(player, "faction") == 20 then
			if not isTVRunning() then
				outputChatBox("[TV] " .. getPlayerName(player):gsub("_", " ") .. " elindított egy Tv adást. (( /tv hogy nézd ))", getRootElement( ), 200, 100, 200)
				exports.logs:dbLog(player, 23, player, "TV START")
				watching = 0
				earnings = 0
				setElementDimension(marker, dim)
			else
				outputChatBox("Egy TV műsor már fut.", player, 255, 0, 0)
			end
		end
	end
)
...
addCommandHandler("endtv",
	function(player)
		if getElementData(player, "faction") == 20 then
			if isTVRunning() then
				setElementDimension(marker, 65535)
				outputChatBox("[TV] " .. getPlayerName(player):gsub("_", " ") .. " befejezte a TV műsort.", getRootElement( ), 200, 100, 200)
				...
				for k, v in ipairs( getElementsByType( "player" ) ) do
					...
					if getElementData(v, "faction") == 20 then
						outputChatBox("[TV] legtöbb néző: " .. watching .. ", Kereset: " .. exports.global:formatMoney(earnings).." Ft", v, 200, 100, 200)
					end
				end
				
				exports.logs:dbLog(player, 23, player, "TV STOP - VIEWERS: " .. watching .. " EARNINGS: " .. earnings)
			else
				outputChatBox("Nem fut TV műsor.", player, 255, 0, 0)
			end
		end
	end
)
	...
	function(player)
		if getElementData(player, "faction") == 20 then
			if isTVRunning() then
				...
				outputChatBox("[TV] Nézők: " .. count .. ", Legtöbb néző: " .. watching, player, 200, 100, 200)
			else
				outputChatBox("Nem fut TV műsor.", player, 255, 0, 0)
			end
		end
