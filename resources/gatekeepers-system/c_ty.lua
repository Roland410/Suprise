...
function createTyGUI ()
	...
	if not (wTy) then
		-- Create the window
		wTy = guiCreateWindow(X, Y, Width, Height, "Párbeszéd egy idegennel.", false )
		...
		optionOne = guiCreateButton( 0.05, 0.35, 0.9, 0.2, "Kopogás az ajtón.", true, wTy )
		addEventHandler( "onClientGUIClick", optionOne, tyRequest, false )

		optionTwo = guiCreateButton( 0.05, 0.55, 0.9, 0.2, "Elsétálok.", true, wTy )
...
function tyStatement2( rooksFriend, tysFriend )
	...
	guiSetText ( tyText, "Ty ordítja: Ki a faszom vagy te?" )
	
	if(tonumber(tysFriend) == 0) then
		-- Create the new options
		optionOne = guiCreateButton( 0.05, 0.35, 0.9, 0.2, "Hé tesó, nyisd ki ezt a szaros ajtót.", true, wTy )
		addEventHandler( "onClientGUIClick", optionOne, tyStatement4, false )
		
		if (tonumber(rooksFriend) == 1) then
			optionTwo = guiCreateButton( 0.05, 0.55, 0.9, 0.2, "A szarka küldött. Azt mondta segítség kell neked valamilyen üzletben.", true, wTy )
			addEventHandler( "onClientGUIClick", optionTwo, tyStatement5, false )
		end
		
	else
		-- Create the new options
		optionOne = guiCreateButton( 0.05, 0.35, 0.9, 0.2, "Én vagyok az tesó, nyisd már ki.", true, wTy )
