...
function findPlayerByPartialNick(thePlayer, partialNick)
	if not partialNick and not isElement(thePlayer) and type( thePlayer ) == "string" then
		outputDebugString( "Hibás adatok a findPlayerByPartialNick függvényhez" )
	...
	if not matchPlayer or not isElement(matchPlayer) then
		if isElement( thePlayer ) then
			if #candidates == 0 then
				outputChatBox("Nem található játékos.", thePlayer, 255, 0, 0)
			else
				outputChatBox( #candidates .. " talált játékosok:", thePlayer, 255, 194, 14)
