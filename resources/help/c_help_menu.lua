...
function createMenu( node )
	if node and type( node ) == "table" then
		if #node == 0 and not node.window then
			outputDebugString( "" .. node.name .. "táblának nincsenek adatai." )
		else
			...
			if type( parent ) == "table" and parent.multi then
				...
				if id > 0 then
					if id ~= 1 then
						local button = guiCreateButton( x, y, width, height, "Előző", false )
					...
					if id ~= #parent then
						local button = guiCreateButton( x, y, width, height, "Következő", false )
			...
			if parent == true then -- we're in the main menu
				local button = guiCreateButton( x, y, width, height, "Kilépés", false )
				...
				local button = guiCreateButton( x, y, width, height, "< Vissza\n" .. parent.name, false )
		...
		ouputDebugString( "Hibás Node struktúra" )
	end
end
