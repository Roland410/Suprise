...
function showIntName(name, ownerName, inttype, cost, fee)
	...
	elseif name then
		if (inttype==3) then -- Interior name and Owner for rented
			...
			gOwnerName = guiCreateLabel(0.0, 0.90, 1.0, 0.3, "Bérlő: " .. tostring(ownerName), true)
			...
			gOwnerName = guiCreateLabel(0.0, 0.90, 1.0, 0.3, "Tulajdonos: " .. tostring(ownerName), true)
			guiSetFont(gOwnerName, "default-bold-small")
			guiLabelSetHorizontalAlign(gOwnerName, "center", true)
			guiSetAlpha(gOwnerName, 0.0)
		end
		if (ownerName=="Nincs") and (inttype==3) then -- Unowned type 3 (rentable)
			gBuyMessage = guiCreateLabel(0.0, 0.915, 1.0, 0.3, "Nyomd meg az 'F'-et hogy " .. tostring(exports.global:formatMoney(cost)) .. " Forintért megvedd.", true)
			guiSetFont(gBuyMessage, "default-bold-small")
			guiLabelSetHorizontalAlign(gBuyMessage, "center", true)
			guiSetAlpha(gBuyMessage, 0.0)
		elseif (ownerName=="Nincs") and (inttype<2) then -- Unowned any other type
			gBuyMessage = guiCreateLabel(0.0, 0.915, 1.0, 0.3, "Nyomd meg az 'F'-et hogy " .. tostring(exports.global:formatMoney(cost)) .. " Forintért megvedd.", true)
			guiSetFont(gBuyMessage, "default-bold-small")
			guiLabelSetHorizontalAlign(gBuyMessage, "center", true)
			guiSetAlpha(gBuyMessage, 0.0)
		else
			local msg = "Nyomd meg az F-et a belépéshez."
			if fee and fee > 0 then
				msg = "Belépési díj: " .. exports.global:formatMoney(fee) .." Ft"
				
				if exports.global:hasMoney( getLocalPlayer(), fee ) then
					msg = msg .. "\nNyomd meg az F-et a belépéshez."
				end
			end
...
function showHouseMenu( )
	...
	wRightClick = guiCreateWindow(ax, ay, 150, 200, "Ház " .. tostring( houseID ), false)

	bLock = guiCreateButton(0.05, 0.13, 0.9, 0.1, "Bezár/kinyír", true, wRightClick)
	addEventHandler("onClientGUIClick", bLock, lockUnlockHouse, false)
	
	bKnock = guiCreateButton(0.05, 0.27, 0.9, 0.1, "Kopogás", true, wRightClick)
	addEventHandler("onClientGUIClick", bKnock, knockHouse, false)
		
	bCloseMenu = guiCreateButton(0.05, 0.41, 0.9, 0.1, "Bezárás", true, wRightClick)
...
function knockHouse()
	...
	if tick - lastKnocked > 10000 then
		...
		outputChatBox("Kérlek várj egy kicsit mielőtt újra kopogsz.", 255, 0, 0)
	end
end
	...
	if (button == "right") and (state=="down") and not e then
		...
		if element then
			if getDistanceBetweenPoints3D(x, y, z, px, py, pz) < 5 then
				...
				outputChatBox("Túl messze vagy ettől a háztól.", 255, 0, 0)
			end
		else
			hideHouseMenu()
		end
...
addEventHandler("playerKnock", root,
	function()
		outputChatBox(" * Kopogás hang jön az ajtótól. *      ((" .. getPlayerName(source):gsub("_"," ") .. "))", 255, 51, 102)
