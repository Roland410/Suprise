...
addCommandHandler("interiordiff", function()
	...
	outputChatBox("Összes: "..tostring(countItems))
	outputChatBox("Betöltve: "..tostring(done))
end)
...
function debugme()
	...
	outputChatBox("Kész. (".. tostring(debugmode)..")")
end
...
function showLagCatcher()
	...
	lagCatcherWindow = guiCreateWindow( x*.5-150, y*.5-65, 300, 120, "Figyelem!", false )
	guiWindowSetSizable( lagCatcherWindow, false )
	lagCatcherMessage = guiCreateLabel( 40, 30, 260, 80, "A szerver elküldi az összes interiort\na számítógépedre, kérlek ne kapcsold ki. \nA játék beakadhat pár másodpercre.\n", false, lagCatcherWindow )
...
function updateLagCatcher()
	if lagcatcherenabled then
		...
		local str = "Állapot: "..tostring(done).."/"..tostring(total)
		guiSetText(lagCatcherMessage, "A szerver elküldi az összes interiort\na számítógépedre, kérlek ne kapcsold ki. \nA játék beakadhat pár másodpercre.\n\n"..str)
	end
end
