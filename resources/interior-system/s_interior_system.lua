...
function sellProperty(thePlayer, commandName, bla)
	if bla then
		outputChatBox("HAsználd a /sell parancsot hogy eladd ezt a helyet más játékosnak.", thePlayer, 255, 0, 0)
		return
	end
	...
	if dbid > 0 then
		if interiorType == 2 then
			outputChatBox("Állami épületet nem tudsz eladni.", thePlayer, 255, 0, 0)
		elseif interiorType ~= 3 and commandName == "unrent" then
			outputChatBox("Nem bérled ezt a házat.", thePlayer, 255, 0, 0)
		else
			...
			if exports.global:isPlayerLeadAdmin(thePlayer) or exports.donators:hasPlayerPerk(thePlayer, 14) or interiorStatus[INTERIOR_OWNER] == getElementData(thePlayer, "dbid") then
				...
				outputChatBox("Nincs ez az ingatlan tulajdonodban.", thePlayer, 255, 0, 0)
			end
		end
	else 
		outputChatBox("Nem vagy egy ingatlanban sem.", thePlayer, 255, 0, 0)
	end
end
...
function publicSellProperty(thePlayer, dbid, showmessages, givemoney)
	...
	if query then
		...
		if interiorType == 0 or interiorType == 1 then
			if interiorStatus[INTERIOR_OWNER] == getElementData(thePlayer, "dbid") then
				...
				if givemoney then
					...
					exports.global:takeMoney(getTeamFromName("Onkormanyzat"), money, true)
				end
				
				if showmessages then
					outputChatBox("Eladtad az ingatlanodat " .. exports.global:formatMoney(money) .. " Forintért.", thePlayer, 0, 255, 0)
				end
				...
				if showmessages then
					outputChatBox("Átálítottad a házat tulajdonítatlanná.", thePlayer, 0, 255, 0)
				end
			end
		else
			if showmessages then
				outputChatBox("Többé nem bérled ezt a házat.", thePlayer, 0, 255, 0)
			end
			call( getResourceFromName( "item-system" ), "deleteAll", 4, dbid )
			call( getResourceFromName( "item-system" ), "deleteAll", 5, dbid )
			triggerClientEvent(thePlayer, "removeBlipAtXY", thePlayer, interiorType, entrance[INTERIOR_X], entrance[INTERIOR_Y], entrance[INTERIOR_Z])
		...
		outputChatBox("Error 504914 - Jelentsd fórumon.", thePlayer, 255, 0, 0)
	end
end
...
function sellTo(thePlayer, commandName, targetPlayerName)
	...
	if dbid > 0 and not isPedInVehicle( thePlayer ) then
		...
		if interiorStatus[INTERIOR_TYPE] == 2 then
			outputChatBox("Állami ingatlant nem tudsz eladni.", thePlayer, 255, 0, 0)
		elseif not targetPlayerName then
			outputChatBox("HAsználat: /" .. commandName .. " [név részlet / id]", thePlayer, 255, 194, 14)
			outputChatBox("Eladtad az ingatlant a játékosnak.", thePlayer, 255, 194, 14)
			outputChatBox("Mondd a vevőnek hogy a /pay parancsot használja a fizetésre.", thePlayer, 255, 194, 14)
		else
						...
						if getElementData(targetPlayer, "dbid") ~= interiorStatus[INTERIOR_OWNER] then
							if exports.global:hasSpaceForItem(targetPlayer, 4, dbid) then
								...
								if query then
									...
									if interiorType == 0 or interiorType == 1 then
										outputChatBox("Sikeresen eladtad az ingatlant neki:" .. targetPlayerName .. ".", thePlayer, 0, 255, 0)
										outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " eladta neked az ingatlanját.", targetPlayer, 0, 255, 0)
									else
										outputChatBox(targetPlayerName .. " elvette a bérleti szerződést.", thePlayer, 0, 255, 0)
										outputChatBox("Elvetted " .. getPlayerName(thePlayer):gsub("_", " ") .. " bérleti szerződését.",  targetPlayer, 0, 255, 0)
									end
									exports.logs:dbLog(thePlayer, 37, { targetPlayer, "in"..tostring(dbid) } , "SELLPROPERTY "..getPlayerName(thePlayer).." => "..targetPlayerName)
									realReloadInterior(dbid)
								else
									outputChatBox("Error 09002 - Jelentsd fórumon.", thePlayer, 255, 0, 0)
								end
							else
								outputChatBox("A" .. targetPlayerName .. " nevű játékosnak nincsen elég hely a zsebében.", thePlayer, 255, 0, 0)
							end
						else
							outputChatBox("Nem tudod magadnak eladni az ingatlant.", thePlayer, 255, 0, 0)
						end
					else
						outputChatBox("Ez az ingatlan nem a tied.", thePlayer, 255, 0, 0)
					end
				else
					outputChatBox("Túl messze vagy a " .. targetPlayerName .. " nevű játékostól.", thePlayer, 255, 0, 0)
				end
			end
		end
	end
...
function realReloadInterior(interiorID)
	...
	if dbid > 0 then
		...
		outputDebugString("ID-nélküli Interior újratöltési próbálkozás(S_int_sys.lua/272).")
	end
end
...
function reloadOneInterior(id, hasCoroutine)
	...
	if row then
		...
		if row then
			...
			outputDebugString( "Hibás interior" .. id )
			return false
		end
	end
end --]]
...
function buyInterior(player, pickup, cost, isHouse, isRentable)
	if isRentable then
		...
		if result then
			...
			if count ~= 0 then
				outputChatBox("Már bérelsz egy másik házat.", player, 255, 0, 0)
				return
			end
		end
	elseif not exports.global:hasSpaceForItem(player, 4, 1) then
		outputChatBox("Nencsen hely a kulcsoknak.", player, 255, 0, 0)
		return
	end
	
	if exports.global:takeMoney(player, cost) then
		if (isHouse) then
			outputChatBox("Gratulálunk! megvetted ezt a házat " .. exports.global:formatMoney(cost) .. " Forintért.", player, 255, 194, 14)
			exports.global:giveMoney( getTeamFromName("Onkormanyzat"), cost )
		elseif (isRentable) then
			outputChatBox("Gratulálunk! Mostantól bérled ezt a házat " .. exports.global:formatMoney(cost) .. " Forintért.", player, 255, 194, 14)
		else
			outputChatBox("Gratulálunk! Megvetted ezt az üzletet " .. exports.global:formatMoney(cost) .. " Forintért.", player, 255, 194, 14)
			exports.global:giveMoney( getTeamFromName("Onkormanyzat"), cost )
		end
		...
		outputChatBox("Sajnálom de nem tudtad megvenni ezt a házat.", player, 255, 194, 14)
	end
end
		...
		if getElementData( thePlayer, "adminduty" ) ~= 1 and not exports.global:hasItem( thePlayer, 5, getElementData( theInterior, "dbid" ) ) and not (getElementData(thePlayer,"ESbadge")) and not (getElementData(thePlayer,"PDbadge"))and not (getElementData(thePlayer,"SheriffBadge")) and not (getElementData(thePlayer,"GOVbadge")) and not (getElementData(thePlayer,"SANbadge")) then
			...
			if fee and fee > 0 then
				if not exports.global:takeMoney( thePlayer, fee ) then
					outputChatBox( "Nincs nálad elég pénz hogy bemenj ebbe az Interiorba.", thePlayer, 255, 0, 0 )
					return
				else
					...
					if query then
						...
						outputChatBox( "Error 100.6 - Jelentsd fórumon.", thePlayer, 255, 0, 0 )
					end
				end
			end
		end
...
function addSafeAtPosition( thePlayer, x, y, z, rotz )
	...
	elseif dbid >= 20000 then -- Vehicle Interiors
		...
		if exports['vehicle-system']:getSafe( vid ) then
			outputChatBox("Ebben a házban már van széf. /movesafe paranccsal tudod átrakni máshova.", thePlayer, 255, 0, 0)
			return 1
...
function moveSafe ( thePlayer, commandName )
	...
	if (dbid < 19000 and (exports.global:hasItem( thePlayer, 5, dbid ) or exports.global:hasItem( thePlayer, 4, dbid))) or (dbid >= 20000 and exports.global:hasItem(thePlayer, 3, dbid - 20000)) then
		...
		elseif dbid > 0 and safeTable[dbid] then
			...
			outputChatBox("Széf kell ahhoz hogy elmozdítsd!", thePlayer, 255, 0, 0)
		end
	else
		outputChatBox("Először is kulcs kell a házhoz ehhez a parancshoz.", thePlayer, 255, 0, 0)
	end
end
...
local function hasKey( source, key )
	return  exports.global:hasItem(source, 4, key) or (exports.global:isPlayerFoAdmin(source))
end
...
function lockUnlockHouseEvent(player, checkdistance)
	...
	if found and itemValue then
		...
		if locked == 0 then
			...
			exports.global:sendLocalMeAction(source, "kiveszi a jobb kezével a kulcsot a zsebéből, beledugja a zárba és elfordítja.")
...
function client_requestHUDinfo()
	...
	local interiorOwnerName = exports['cache']:getCharacterName(interiorStatus[INTERIOR_OWNER]) or "Nincs"
