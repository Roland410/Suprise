...
function showChemistrySet()
	if not (wItems) then
		if not (wChemistrySet) then
			...
			-- buttons
			bMixItems = guiCreateButton(0.05, 0.91, 0.7, 0.15, "Keverék kiválasztva", true, wChemistrySet)
...
local drugMessages = { "Bazdmeg, ez nagyon durva cucc!", "Óóó ember, én repülök!", "Yoo, akarok még egy kicsit!", "Hű ember, ez aztán az őrjöngés!", "Yooo ember, miért mentek kásává a lábaid?", "Hé köcsög, akarsz táncolni?", "Tesó, adj nekem is abból a cuccból. Mi az?", "Oóoó, hívom mindjárt a zsarukat!" }
	...
	for key, value in ipairs(players) do
		if (value~=localPlayer) then
			...
			if (getDistanceBetweenPoints3D(x, y, z, px, py, pz)<15) then
				...
				outputChatBox(charname .. " mondja: " .. drugMessages[rand2], 255, 255, 255)
				break
			end
		end
	end
