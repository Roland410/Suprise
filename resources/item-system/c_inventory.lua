--Fordítási megjegyzés 13.07.29 CMG02 : Porn Tape illetve Note itemeknek nincs meg a végleges neve Hank-nél van ezeket a neveket majd kicserélni itt
...
local actionIcons =
{
	[TAB_ITEMS] = { 48, tocolor( 255, 255, 127, 63 ), "Tárgyak" },
	[TAB_KEYS] = { 4, tocolor( 255, 255, 127, 63 ), "Kulcsok" },
	[TAB_WEAPONS] = { -28, tocolor( 255, 255, 127, 63 ), "Fegyverek" },
	
	[ACTION_DROP] = { -202, tocolor( 127, 255, 127, 63 ), "Tárgy eldobása", "Nyomd a CTRL billentyűt miközben egy tárgyra kattintasz." },
	[ACTION_SHOW] = { -201, tocolor( 127, 127, 255, 63 ), "Tárgy megnézése" },
	[ACTION_DESTROY] = { -200, tocolor( 255, 127, 127, 63 ), "Tárgy törlése", "Nyomd a DELETE billentyűt miközben egy tárgyra kattintasz." }
}
...
local function getOverlayText( itemID, itemValue, isGrouped )
	...
	elseif itemID == 116 then
		...
		text = text .. s[3]:gsub("Töltény ehhez ", "") .. " Töltény"
	end
...
local function getTooltipFor( itemID, itemValue, isGrouped )
	...
	if itemID == 80 and isGrouped then
		return isGrouped .. " Általános tárgyak", ""
	end
	
	if isGrouped then
		return isGrouped .. "x " .. name, "Kattints az összes tárgy megtekintéséhez."
	end
			...
			if clickDown and ( getTickCount( ) - clickDown >= 200 ) and ( ( clickItemSlot and not clickItemSlot.group ) or clickWorldItem ) then
				...
				if hoverElement then
					if color == background_movetoelement_color then
						...
						tooltip( boxx + sbox, boxy + ( box - 50 ) / 2, getItemName( item[1], item[2] ), "Mozgatás ide: " .. name .. "." )
			...
			elseif not tooltipYet and getPedOccupiedVehicle( localPlayer )then
				...
				if element then
					...
					if not any and getElementType(element) == "vehicle" and element ~= getPedOccupiedVehicle( localPlayer ) and exports['vehicle-system']:hasVehiclePlates( element ) and getDistanceBetweenPoints3D( px, py, pz, getElementPosition( element ) ) < 10 then
						tooltip( cursorX, cursorY, "Rendszám: " .. getVehiclePlateText( element ) )
					end
				end
			end
		end
					...
					if state == "up" and clickItemSlot then
						...
						elseif not clickItemSlot.group then
									...
									if hoverElement == nil then
										if itemID > 0 then
											if itemID == 48 and countItems( localPlayer, 48 ) == 1 and getCarriedWeight( localPlayer ) - getItemWeight( 48, 1 ) > 10 then
												outputChatBox( "Túl sok dolog van a zsebedben.", 255, 0, 0 )
											else
							...
							elseif hoverAction == ACTION_DROP then
								if isWatchingTV() then
									outputChatBox( "Nem vagy a TV-nél. Csak ülj le elé.", 255, 0, 0 )
									clickItemSlot = nil
									return
								end
								local item = inventory[ clickItemSlot.invslot ]
							...
							elseif hoverAction == ACTION_DESTROY then
								...
								if itemID == 48 and countItems( localPlayer, 48 ) == 1 then -- backpack
									if getCarriedWeight( localPlayer ) - getItemWeight( 48, 1 ) > 10 then
										outputChatBox("Túl sok cucc van a zsebedben.")
									else
...
bindKey( "i", "down",
	function( )
		...
		elseif not getElementData(localPlayer, "adminjailed") and not getElementData(localPlayer, "pd.jailstation") then
			...
			outputChatBox("Itt nem tudsz belenulni a zsebedbe.", 255, 0, 0)
		end
	end
)
