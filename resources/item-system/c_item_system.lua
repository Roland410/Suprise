...
wWait = guiCreateButton( ( sx - 200 ) / 2, ( sy - 60 ) / 2, 200, 60, "Kérlek várj egy kicsit..", false )
...
function showItemMenu()
	...
	if itemID == 81 or itemID == 103 then
		...
		bPickup = guiCreateButton(0.05, y, 0.9, 0.1, "Felvétel", true, wRightClick)
	...
	if itemID == 54 then
		-- Ghettoblaster
		if itemValue > 0 then
			bToggle = guiCreateButton(0.05, y, 0.9, 0.1, "Kikapcsolás", true, wRightClick)
			
			y = y + 0.14
			
			bPreviousTrack = guiCreateButton(0.05, y, 0.42, 0.1, "Vissza", true, wRightClick)
			...
			bNextTrack = guiCreateButton(0.53, y, 0.42, 0.1, "Következő", true, wRightClick)
			...
			bToggle = guiCreateButton(0.05, y, 0.9, 0.1, "Bekapcsolás", true, wRightClick)
		end
	...
	bCloseMenu = guiCreateButton(0.05, y, 0.9, 0.1, "Bezárás", true, wRightClick)
...
function pickupItem(button, state, item)
	if (button=="left") then
		...
		if (restrain) and (restrain==1) then
			outputChatBox("Meg vagy bilincselve.", 255, 0, 0)
		elseif getElementData(item, "itemID") > 0 and not hasSpaceForItem(getLocalPlayer(), getElementData(item, "itemID"), getElementData(item, "itemValue")) then
			outputChatBox("Az Inventoryd tele van.", 255, 0, 0)
...
function showInventory(player)
	if not (wChemistrySet) then
		...
		if player ~= getLocalPlayer() then
			title = getPlayerName(player).. " inventoryja"
		end
		...
		tabItems = guiCreateTab("Tárgyak", tabPanel)
		tabKeys = guiCreateTab("Kulcsok", tabPanel)
		tabWeapons = guiCreateTab("Fegyverek", tabPanel)
		...
		colSlot = guiGridListAddColumn(gItems, "Hely", 0.1)
		colName = guiGridListAddColumn(gItems, "Név", 0.225)
		colValue = guiGridListAddColumn(gItems, "Mennyiség", 0.625)
		...
		colSlot = guiGridListAddColumn(gKeys, "Hely", 0.1)
		colName = guiGridListAddColumn(gKeys, "Név", 0.625)
		colValue = guiGridListAddColumn(gKeys, "Mennyiség", 0.225)
		...
		colWSlot = guiGridListAddColumn(gWeapons, "Hely", 0.1)
		colWName = guiGridListAddColumn(gWeapons, "Név", 0.225)
		colWValue = guiGridListAddColumn(gWeapons, "Töltény", 0.625)
		
		-- GENERAL
		lDescription = guiCreateLabel(0.025, 0.87, 0.95, 0.1, "Kattints egy tárgyra, hogy lásd a leírást", true, wItems)
		guiLabelSetHorizontalAlign(lDescription, "center", true)
		guiSetFont(lDescription, "default-bold-small")
		
		bClose = guiCreateButton(0.375, 0.91, 0.2, 0.15, "Inventory bezárása", true, wItems)
	...
	if (button=="left") then
		if (guiGetSelectedTab(tabPanel)==tabItems or guiGetSelectedTab(tabPanel)==tabKeys) then -- ITEMS
			...
			if (row==-1) or (col==-1) then
				guiSetText(lDescription, "Kattints egy tárgyra, hogy megnézd a leírását.")
			else
				...
				if not items[slot] then
					guiSetText(lDescription, "Üres hely.")
				else
		...
		elseif (guiGetSelectedTab(tabPanel)==tabWeapons) then -- WEAPONS
			...
			if (row==-1) or (col==-1) then
				...
				local desc = "" .. name .. " " .. ammo .. " tölténnyel."
...
--function useItem(button)
	...
	if itemSlot > 0 then -- ITEMS
		...
		elseif (itemID==6) then -- radio
			outputChatBox("Nyomj Y gombot a tárgy használatához. A rádiót a '/tuneradio' paranccsal hangolhatod be.", 255, 194, 14)
			return
		elseif (itemID==7) then -- phonebook
			outputChatBox("A tárgy használatához írd be: '/phonebook'.", 255, 194, 14)
			return
		...
		elseif (itemID==19) then -- MP3 PLayer
			if isPedInVehicle(getLocalPlayer()) or getElementData(getLocalPlayer(), "fishing") or getElementData(getLocalPlayer(), "jammed") then
				outputChatBox("Használd a - és = gombokat a rádióállomás váltáshoz.", 255, 194, 14)
			else
		...
		elseif (itemID==30) or (itemID==31) or (itemID==32) or (itemID==33) then
			outputChatBox("A tárgy használatához vásárolj kémikus szettet.", 255, 0, 0)
			return
		...
		elseif (itemID==45) or (itemID==46) or (itemID==47) or (itemID==66) then
			outputChatBox("Jobb gombbal kattints a játékosra akin ezt a tárgyat használni szeretnéd.", source, 255, 0, 0)
			return
		elseif (itemID==48) then
			outputChatBox("Az inventoryd ki lett bővítve.", 0, 255, 0)
			return
		elseif (itemID==50) or (itemID==51) or (itemID==52) then
			hideInventory()
		elseif (itemID==53) then -- Breathalizer
			outputChatBox("A tárgy használatához írd be: '/breathtest'.", 255, 194, 15)
			return
		...
		elseif (itemID==61) then -- Emergency Light Becon
			outputChatBox("Ezt a tárgyat rakd járműbe és a 'P' gombbal használhatod.", 255, 194, 14)
			return
		...
		elseif (itemID==67) then -- GPS
			outputChatBox("Ezt a tárgyat a járműved inventoryjába kell raknod és 'F5' gombot nyomni.", 255, 194, 14)
			return
		elseif (itemID==70) then -- First Aid Kit
			outputChatBox("Kattints rá jobb klikkel a sérült játékosra a segítéshez.", 255, 194, 14)
			return
		elseif (itemID==71) then -- Notebook
			outputChatBox("Írd be '/writenote [szöveg]' a tárgy használatához. " .. itemValue .. " papír van hátra.", 255, 194, 14)
			return
		elseif (itemID==72) then -- Note
			outputChatBox("A jegyzeten olvasható: " .. itemValue, 255, 194, 14)
		...
		elseif (itemID==81) then
			outputChatBox("A hűtőt egy épületben kell leraknod.", 255, 194, 14)
			return
		...
		elseif (itemID==85) then -- Emergency Light Becon
			outputChatBox("Ezt a tárgyat a járműved inventoryjába kell raknod és 'N' gombot nyomni.", 255, 194, 14)
			return
		...
		elseif (itemID==111) then
			outputChatBox( "Személyi GPS, a legújabb térképekkel.", 255, 0, 0 )
			return
		...
		if itemSlot == -100 then
			outputChatBox("Golyóálló mellény van rajtad.", 0, 255, 0)
		else
...
function showItemList()
	...
	if not (wItemsList) then
		wItemsList = guiCreateWindow(0.15, 0.15, 0.7, 0.7, "Tárgylista", true)
