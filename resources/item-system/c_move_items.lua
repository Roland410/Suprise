	...
	if getElementType( element ) == "vehicle" then
			...
			for key, value in pairs( data[ element ] ) do
				for k, v in ipairs( mods ) do
					if key == v then
						...
						guiGridListSetItemText(gElementItems, row, VIColName, "Tuning " .. getItemDescription( 114, v ), false, false)
		...
		if exports.global:isPlayerSuperAdmin(localPlayer) then
			for key, value in ipairs(mods) do
				...
				guiGridListSetItemText(gElementItems, row, VIColName, "TUNING " .. getItemDescription( 114, value ), false, false)
		...
		elseif slot then
			...
			if item then
				...
				if itemID < 0 and itemID ~= -100 then -- weapon
					...
					if totalfree == 0 then
						outputChatBox( "Több fegyvert nem tudsz magaddal vinni.", 255, 0, 0 )
					elseif free == 0 and cap == 0 then
						local weaponName = "Egyéb fegyver"
						...
						outputChatBox( "Nem viselheted ezt a fegyvert először dobd el az  " .. weaponName .. "-at/et.", 255, 0, 0 )
					elseif free == 0 then
						outputChatBox( "Ebből a típusú fegyverből nem tudsz többet magadnál viselni.", 255, 0, 0 )
					else
						guiSetVisible( wWait, true )
						guiSetEnabled( wInventory, false )
...
local function openElementInventory( ax, ay )
	if not getElementData(localPlayer, "exclusiveGUI") then
		...
		lYou = guiCreateLabel(0.25, 0.1, 0.87, 0.05, "TE", true, wInventory)
		...
		UIColName = guiGridListAddColumn(gUserItems, "Név", 0.9)
		
		gElementItems = guiCreateGridList(0.5, 0.15, 0.45, 0.65, true, wInventory)
		VIColName = guiGridListAddColumn(gElementItems, "Név", 0.9)
		
		bCloseInventory = guiCreateButton(0.05, 0.9, 0.9, 0.075, "Bezárás", true, wInventory)
		addEventHandler("onClientGUIClick", bCloseInventory, hideMenu, false)
		
		bGiveItem = guiCreateButton(0.05, 0.81, 0.45, 0.075, "Mozgat ---->", true, wInventory)
		...
		bTakeItem = guiCreateButton(0.5, 0.81, 0.45, 0.075, "<---- Mozgat ", true, wInventory)
		...
		outputChatBox("Ezt a tároló lehetőséget jelen pillanatban nem tudod elérni.", 255, 0, 0)
	end
end
