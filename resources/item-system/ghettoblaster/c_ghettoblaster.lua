...
function Streameles(item)
	setElementStreamable (item, true )
end
addEvent("streamGhettoblaster", true)
addEventHandler("streamGhettoblaster", getRootElement(), Streameles)
function startGB()
	...
	if itemValue > 0 then
		...
		if (getDistanceBetweenPoints3D(x, y, z, px, py, pz)<300) then
			local sound = playSound3D("" .. tracks[itemValue].file, x, y, z, true)
...
function toggleSound(dataname)
	if getElementModel(source) == 2226 and dataname == "itemValue" then
