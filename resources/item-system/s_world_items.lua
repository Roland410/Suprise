...
function protectItem(faction)
	...
	if type(faction) == "number" and exports.global:isPlayerAdmin(client) then
		...
		if protected then
			...
			outputChatBox("Védelem levéve.", client, 0, 255, 0)
			out = 0
		else
			exports['anticheat-system']:changeProtectedElementDataEx(source, "protected", faction, false)
			outputChatBox("Beállítva a " .. faction .. "-as/es frakcióra - ha másik frakcióra szeretnéd, /itemprotect [frakció id vagy -100]", client, 255, 0, 0)
			out = faction
		end
	...
	if not ammo then
		...
		if ( itemID == 116)  then -- Weapons
			...
			if (tonumber(weaponDetails[2]) and tonumber(weaponDetails[2]) == 2)  then -- /duty
				outputChatBox("Nem tudod eldobni a felszereléshez tartozó fegyvered.", source, 255, 0, 0)
				weaponBlock = true
			end
		elseif ( itemID == 116) then -- Ammo
			...
			if (checkString == " (D)")  then -- /duty
				outputChatBox("Nem tudod eldobni a felszereléshez tartozó töltényed.", source, 255, 0, 0)
				weaponBlock = true
			end
		end
		
		if itemID == 60 then
			outputChatBox( "Ezt nem lehet eldobni.", source )
		...
		elseif ( itemID == 81 or itemID == 103 ) and dimension == 0 then
			outputChatBox( "Egy interiorban tudod csak eldobni.", source )
		elseif (weaponBlock) then
			-- Do nothing
		else
			if not (hasItem(source, itemID)) then
				local haxStr = getPlayerName(thePlayer) .. " ".. getPlayerIP(thePlayer) .." megpróbált duplázni egy tárgyat " ..tostring(itemID) .." C0x0000003[BETA]"
			...
			if insert then
				...
				outputChatBox("Eldobtál egy " .. getItemName( itemID, itemValue ) .. "-t.", source, 255, 194, 14)
				...
				exports.global:sendLocalMeAction(source, "eldobott egy " .. getItemName( itemID, itemValue ) .. "-t.")
			end
		end
	else
		if tonumber(getElementData(source, "duty")) > 0 and itemID == 116 then
			outputChatBox("Nem tudod eldobni a felszereléshez tartozó fegyvered.", source, 255, 0, 0)
		elseif tonumber(getElementData(source, "job")) == 4 and itemID == 41 then
			outputChatBox("NEm tudod eldobni ezt a sprét.", source, 255, 0, 0)
		else
			...
			if (totalAmmoPosessed < ammo) then
				local haxStr = getPlayerName(thePlayer) .. " ".. getPlayerIP(thePlayer) .." megpróbált fegyvert duplázni " .. tokenweapon .. " ennyi tölténnyel: " .. ammo .." C0x0000003[BETA]"
				outputDebugString(haxStr)
				exports.logs:logMessage(haxStr, 32)
				exports.global:sendMessageToAdmins("AdmWrn: " .. haxStr)		
			end
			
			outputChatBox("Eldobtál egy " .. ( getWeaponNameFromID( itemID ) or "kevlár" ) .. "t.", source, 255, 194, 14)
			...
			if query then
				...
				exports.global:sendLocalMeAction(source, "eldobott egy " .. getItemName( -itemID ) .. "-t.")
...
function doItemGiveawayChecks(source, itemID)
	...
	if badge and getElementData(source, badge[1]) and not hasItem(source, itemID, removeOOC(getElementData(source, badge[1]))) then
		exports.global:sendLocalMeAction(source, "levette a " .. badge[2] .. " jelvényét.")
...
local function moveItem(item, x, y, z)
	...
	if not canPickup(source, item) then
		outputChatBox("Nem tudod mozgatni ezt a tárgyat. Keress fel egy admint az F2.", source, 255, 0, 0)
		return
	end
...
function pickupItem(object, leftammo)
	...
	if (getDistanceBetweenPoints3D(x, y, z, ox, oy, oz)<10) then
		...
		if not canPickup(source, object) then
			outputChatBox("Nem tudod felvenni ezt a tárgyat. Keress fel egy admint az F2.", source, 255, 0, 0)
			return
		end
		...
		outputChatBox("Felvettél egy " .. getItemName( itemID, itemValue ) .. "-t.", source, 255, 194, 14)
		exports.global:sendLocalMeAction(source, "lenyúlt és felvett egy " .. getItemName( itemID, itemValue ) .. "-t.")
		triggerClientEvent(source, "item:updateclient", source)
	else
		outputDebugString("Túl messze van a játékos és a tárgy.")
	end
end
...
function getNearbyItems(thePlayer, commandName)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		...
		outputChatBox("Közeli tárgyak:", thePlayer, 255, 126, 0)
		local count = 0
		...
		for k, theObject in ipairs(getElementsByType("object", getResourceRootElement(getResourceFromName("item-world")))) do
			...
			if dbid then
				...
				if distance <= 10 and getElementDimension(theObject) == getElementDimension(thePlayer) and getElementInterior(theObject) == getElementInterior(thePlayer) then
					outputChatBox("   #" .. dbid .. (getElementData(theObject, "protected") and ("(" .. getElementData(theObject, "protected").. ")") or "") .. " levédve " .. ( exports['cache']:getCharacterName( getElementData(theObject, "creator"), true ) or "?" ) .. " által - " .. ( getItemName( getElementData(theObject, "itemID") ) or "?" ) .. "(" .. getElementData(theObject, "itemID") .. "): " .. tostring( getElementData(theObject, "itemValue") or 1 ), thePlayer, 255, 126, 0)
					count = count + 1
				end
			end
		end
	...
	if (exports.global:isPlayerAdmin(thePlayer)) then
		if not (targetID) then
			...
			if object then
				...
				outputChatBox("Tárgy #" .. id .. " törölve.", thePlayer)
				destroyElement(object)
			else
...
function getNearbyItems(thePlayer, commandName)
	if (exports.global:isPlayerAdmin(thePlayer)) then
		...
		outputChatBox( count .. " tárgy törölve.", thePlayer, 255, 126, 0)
	end
end
