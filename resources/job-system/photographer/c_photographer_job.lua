...
setElementData( victoria, "beszéd", 1, false )
setElementData( victoria, "név", "Victoria Greene", false )
--[[
			...
			if (factionType==6) then
				if (weapon == 43) then
					...
					if(pictureValue==0)then
						outputChatBox("Senki sem fog fizetni, hogy a kép ...", 255, 0, 0)
					else
						collectionValue = collectionValue + pictureValue
						outputChatBox("#FF9933Ez egy állattartó! Kép érték: Ft"..exports.global:formatMoney(pictureValue), 255, 104, 91, true)
						triggerServerEvent("updateCollectionValue", localPlayer, collectionValue)
					end
					outputChatBox("#FF9933Kollekció érték: Ft"..exports.global:formatMoney(collectionValue), 255, 104, 91, true)
				end
			end
		end
	end
...
function cPhotoOptions()
	...
	if not open then
		...
		sanOptionMenu = guiCreateWindow(x, y, width, height, "Hogyan tudunk segíteni?", false)

		bPhotos = guiCreateButton(0.05, 0.3, 0.87, 0.25, "Kép eladás", true, sanOptionMenu)
		addEventHandler("onClientGUIClick", bPhotos, startPhotos, false)

		if (getElementDimension(victoria) ~= 9902) then
			bAdvert = guiCreateButton(0.05, 0.6, 0.87, 0.25, "Terület mutatás", true, sanOptionMenu)
	...
	function()
		...
		if getElementData(theTeam, "type") == 6 then
			if collectionValue == 0 then
				outputChatBox("Ez a kép Már nem ér semmit ", 255, 0, 0, true)
			else
...
addEventHandler("updateCollectionValue", localPlayer,
	function(value)
		...
		if value > 0 then
			outputChatBox("Még mindig megér ennyit: Ft" .. exports.global:formatMoney(collectionValue) .. ".", 255, 194, 14)
		end
	end
)
