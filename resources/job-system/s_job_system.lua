...
function onEmploymentServer()
	exports.global:sendLocalText(source, "Jessie Smith: Üdvözlöm! Miben segíthetek?", nil, nil, nil, 10)
	exports.global:sendLocalText(source, " *Jessie Smith ad egy papírt az álláslehetőségekről " .. getPlayerName(source):gsub("_", " ") .. ".", 255, 51, 102)
end
...
function givePlayerJob(jobID)
	...
	if (jobID==4) then -- CITY MAINTENANCE
		...
		outputChatBox("Keresd a falfirkát majd takarisd le.", source, 255, 194, 14)
		...
		if job == 0 then
			...
			if result then
				...
				if contracttime > 0 then
					outputChatBox( "Még várnod kell " .. contracttime .. " fizetést a kilépéshez.", source, 255, 0, 0)
				else
		...
		if targetPlayerName then
			...
			if targetPlayer then
				if getElementData( targetPlayer, "loggedin" ) == 1 then
					...
					if result then
						outputChatBox( "Munkaidő resetelve " .. targetPlayerName, thePlayer, 0, 255, 0 )
					else
						outputChatBox( "Nem sikerült resetelni a kötelező munkaidőt.", thePlayer, 255, 0, 0 )
					end
				else
					outputChatBox( "Nincs bejelentkezve!", thePlayer, 255, 0, 0 )
				end
			end
		else
			outputChatBox( "SYNTAX: /" .. commandName .. " [játékos]", thePlayer, 255, 194, 14 )
		end
	end
end
