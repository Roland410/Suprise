...
function giveTruckingMoney(vehicle)
	outputChatBox("Kerestél " .. exports.global:formatMoney( truckwage[vehicle] or 0 ) .. " Ft-ot a szállitással.", source, 255, 194, 15)
...
function saveDeliveryProgress(vehicle, earned)
	...
	outputChatBox("Befejezted a szálítást  " .. truckruns[vehicle] .. ".  ezért kaptál  Ft" .. exports.global:formatMoney(earned) .. ".", client, 0, 255, 0)
	if (earned > 60) then
		triggerTruckCheatEvent(client, 1, earned)
	end
	
	if (truckruns[vehicle] == 25) then
		outputChatBox("#FF9933A raktér üres! Térj vissza a  #CC0000telephelyre #FF9933megkapod a fizetséged.", client, 0, 0, 0, true)
	else 
		outputChatBox("#FF9933Ha ugy döntöttél vége a szállíttásnak akkor a #CC0000telephelyen  #FF9933kifüzetjük a béredet", client, 0, 0, 0, true)
...
function triggerTruckCheatEvent(thePlayer, cheatType, value1)
	local cheatStr = ""
	if (cheatType == 1) then
		cheatStr = "Leadta a szállitmányt, (keresett:"..value1..", )"
	elseif (cheatType == 2) then
		cheatStr = "Rendszeresen hordja az árút. (Keresett:"..value1..", )"
	else
