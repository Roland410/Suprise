...
function displayBusJob()
	...
	outputChatBox("#FF9933Menj a #FF66CCPályaudvarra#FF9933, hogy elkezdd a munkát.", 255, 194, 15, true)
end
...
function startBusJob()
	...
	if (job == 3) then
		...
		if busMarker then
			outputChatBox("#FF9933Már elkezdted az útvonalat.", 255, 194, 14, true)
		else
			...
			if vehicle and getVehicleController(vehicle) == getLocalPlayer() and bus[getElementModel(vehicle)] then
				...
				outputChatBox("#FF9933Járd végig az  #FFCC00útvonalat#FF9933, megállva minden #A00101megálloknál #FF9933.", 255, 194, 14, true)
				
			else
				outputChatBox("#F#FF9933Buszban kell lenned.", 255, 194, 14, true)
			end
		end
	else
		outputChatBox("Nem vagy Busz vezető.", 255, 194, 14)
	end
end
	...
	if thePlayer == getLocalPlayer() then
		...
		if vehicle and bus[getElementModel(vehicle)] then
			if curCpType == 3 then
				...
				outputChatBox("#FF9933Várj a megálloban, amíg az utasok fel-le szállnak(cp el nem tünik).", 255, 0, 0, true )
			...
			outputChatBox("#FF9933Buszban kell lenned..", 255, 0, 0, true ) -- Wrong car type.
		end
	end
end
...
function checkWaitAtStop(thePlayer)
	if thePlayer == getLocalPlayer() then
		if busStopTimer then
			outputChatBox("Nem vártál a megállóban .", 255, 0, 0)
...
function endOfTheLine()
	if busNextBlip then
		...
		--setVehicleLocked(vehicle, false)
		outputChatBox("#FF9933Végigmentél az útvonalon. Az újrakezdéshez írd be: '/startbus' ..", 0, 255, 0, true )
	end
end
		...
		if(bus[vehID])then
			if(seat~=0)then
				...
				if driver then -- you can only pay the driver if the bus has a driver
					if not exports.global:hasMoney(getLocalPlayer(), 400)then
						triggerServerEvent("removePlayerFromBus", getLocalPlayer())
						outputChatBox("Nincs elég pénzed(400-Ft) a buszjegyre.", 255, 0, 0)
					else
						triggerServerEvent("payBusFare", getLocalPlayer(), driver)
						outputChatBox("Fizettél 400-ft a buszjegyre.", 0, 255, 0)
					end
				end
			elseif not busMarker and getElementData(getLocalPlayer(), "job") == 3 then
				outputChatBox("#FF9933A munka elkezdéséhez írd be: '/startbus'.", 255, 0, 0, true)
			end
		end
	end
end
...
function startEnterBus(thePlayer, seat)
	if seat == 0 and bus[getElementModel(source)] then
		if getVehicleController(source) then -- if someone try to jack the driver stop him
			cancelEvent()
			if thePlayer == getLocalPlayer() then
				outputChatBox("A sofőrajtó be van zárva.", 255, 0, 0)
			end
		else
