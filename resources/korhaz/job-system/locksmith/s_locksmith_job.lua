	...
	if (logged==1) then
		if(getElementData(thePlayer,"job")==6)then
			if not (keyType) or not (keyID) then
				outputChatBox("SYNTAX: /" .. commandName .. " [kulcs tipus: 1=Ház 2=Biznisz 3=Jármu] [kulcs id]", thePlayer, 255, 194, 14)
			else
				if not exports.global:hasMoney(thePlayer, 25) then
					outputChatBox("Nincs elég pénzed(25$), hogy kulcsot másolj.", thePlayer, 255, 0, 0)
				else
					...
					if(success)then -- does the player have the key?
						...
						outputChatBox("Lemásoltál egy ".. keyname .."ot(".. keyID ..") 2000Ft-ért.", thePlayer)
						exports.global:sendLocalMeAction(thePlayer,"lemásol egy ".. keyname .."ot.")
					else
						outputChatBox("Nálad nincs ilyen kulcs.", thePlayer, 255, 0, 0)
					end
				end
			end
		end
