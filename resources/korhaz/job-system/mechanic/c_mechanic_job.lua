...
function displayMechanicJob()
	outputChatBox("#FF9933Használd a #FF0000jobb kattintásos menüt#FF9933, hogy megnézd a szolgáltatásokat.", 255, 194, 15, true)
end
...
function mechanicWindow(vehicle)
	...
	if (job==5) or (faction==5) then
		if not vehicle then
			outputChatBox("Ki kell választanod egy jármüvet.", 255, 0, 0)
		else
			...
			if not (wMechanic) then
				-- Create the window
				wMechanic = guiCreateWindow(X, Y, Width, Height, "Szerelő szolgáltatások", false )
				...
				-- Service
				bMechanicTwo = guiCreateButton( 0.05, y, 0.9, 0.1, "Szervíz - ", true, wMechanic )
				addEventHandler( "onClientGUIClick", bMechanicTwo, serviceTrigger, false)
				y = y + 0.1
				
				-- Tyre Change
				if not noTyres[getVehicleType(vehicle)] then
					bMechanicThree = guiCreateButton( 0.05, y, 0.9, 0.1, "Kerékcsere - ", true, wMechanic )
				...
				if (playerDimension == 0) then
					...
					-- Recolour
					if faction == 5 or ( tonumber( getElementData(vehicle, "job") or 5 ) == 0 and ( getElementData(vehicle, "faction") == -1 or getElementData(vehicle, "faction") == faction ) ) then
						bMechanicFour = guiCreateButton( 0.05, y, 0.9, 0.1, "Újrafestés ", true, wMechanic )
					...
					if vehicleWithPaintjob[getElementModel(vehicle)] then
						bMechanicSix = guiCreateButton( 0.05, y, 0.9, 0.1, " Festés ", true, wMechanic )
					...
					-- Replace lights
					--if vehicleWithPaintjob[getElementModel(vehicle)] then
						bMechanicNine = guiCreateButton( 0.05, y, 0.9, 0.1, "Fényszoró Javitás ", true, wMechanic )
					...
					if getVehicleUpgradeOnSlot(vehicle, 8) ~= 0 and faction == 30 then
						bMechanicSeven = guiCreateButton( 0.05, y, 0.9, 0.1, "Nitró eltávolítása", true, wMechanic )
					...
					if faction == 30 then
						if not getElementData(vehicle, "tinted") then
							bMechanicEight = guiCreateButton( 0.05, y, 0.9, 0.1, "Színárnyalat felrakása Ft", true, wMechanic )
							addEventHandler( "onClientGUIClick", bMechanicEight, addTintWindow, false)
							y = y + 0.1
						else
							bMechanicEight = guiCreateButton( 0.05, y, 0.9, 0.1, "Színárnyalat törlése ", true, wMechanic )
				...
				-- Close
				bMechanicClose = guiCreateButton( 0.05, 0.85, 0.9, 0.1, "Bezárás", true, wMechanic )
...
function tyreWindow()
	...
	if not (wTyre) then
		-- Create the window
		wTyre = guiCreateWindow(X+100, Y, Width, Height, "Válaszd ki melyiket akarod lecserélni", false )
		
		if getVehicleType(currentVehicle) ~= "Bike" then
			-- Front left
			bTyreOne = guiCreateButton( 0.05, 0.1, 0.45, 0.35, "Bal első", true, wTyre )
			...
			-- Back left
			bTyreTwo = guiCreateButton( 0.05, 0.5, 0.45, 0.35, "Bal hátsó", true, wTyre )
			...
			-- front right
			bTyreThree = guiCreateButton( 0.5, 0.1, 0.45, 0.35, "Jobb első", true, wTyre )
			...
			-- back right
			bTyreFour = guiCreateButton( 0.5, 0.5, 0.45, 0.35, "Jobb hátsó", true, wTyre )
			...
			-- Front
			bTyreOne = guiCreateButton( 0.05, 0.1, 0.9, 0.35, "Első", true, wTyre )
			addEventHandler( "onClientGUIClick", bTyreOne, function(button, state)
				if(button == "left" and state == "up") then
			...
			-- back right
			bTyreThree = guiCreateButton( 0.05, 0.5, 0.9, 0.35, "Hátsó", true, wTyre )
			addEventHandler( "onClientGUIClick", bTyreThree, function(button, state)
				if(button == "left" and state == "up") then
		...
		-- Close
		bTyreClose = guiCreateButton( 0.05, 0.9, 0.9, 0.1, "Bezárás", true, wTyre )
...
function paintWindow()
	...
	if not (wPaint) then
		...
		bPaintColor1 = guiCreateButton( 0.05, 0.1, 0.45, 0.2, "szin 1", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintColor1,  function(button, state)
			if(button == "left" and state == "up") then
				paintColor( 1 )
			end
		end, false)	 
		bPaintColor2 = guiCreateButton( 0.55, 0.1, 0.45, 0.2, "szin 2", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintColor2,  function(button, state)
			if(button == "left" and state == "up") then
				paintColor( 2 )
			end
		end, false)	
		bPaintColor3 = guiCreateButton( 0.05, 0.4, 0.45, 0.2, "szin 3", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintColor3,  function(button, state)
			if(button == "left" and state == "up") then
				paintColor( 3 )
			end
		end, false)	
		bPaintColor4 = guiCreateButton( 0.55, 0.4, 0.45, 0.2, "szin 4", true, wPaint )
		...
		bPaintSubmit = guiCreateButton( 0.05, 0.75, 0.90, 0.1, "mentés", true, wPaint )
		...
		bPaintClose = guiCreateButton( 0.05, 0.85, 0.90, 0.1, "Bezárás", true, wPaint )
	...
	if not (wPaint) then
		...
		-- Create the window
		wPaint = guiCreateWindow(X, Y, Width, Height, "Válaszd ki a szineket.", false )
		...
		lcol1 = guiCreateLabel( 0.85, 0.1, 0.25, 0.075, "szin 1 ID", true, wPaint )
		iColour2 = guiCreateEdit( 0.85, 0.4, 0.25, 0.075, "", true, wPaint )
		lcol2 = guiCreateLabel( 0.85, 0.3, 0.25, 0.075, "szin 2 ID", true, wPaint )
		iColour3 = guiCreateEdit( 0.85, 0.6, 0.25, 0.075, "", true, wPaint )
		lcol3 = guiCreateLabel( 0.85, 0.5, 0.25, 0.075, "szin 3 ID", true, wPaint )
		iColour4 = guiCreateEdit( 0.85, 0.8, 0.25, 0.075, "", true, wPaint )
		lcol4 = guiCreateLabel( 0.85, 0.7, 0.25, 0.075, "szin 4 ID", true, wPaint )
		
		-- Repaint
		bPaintSubmit = guiCreateButton( 0.05, 0.8, 0.3, 0.2, "kocsi festés", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintSubmit,  function(button, state)
			if(button == "left" and state == "up") then
		...
		-- Close
		bPaintClose = guiCreateButton( 0.35, 0.8, 0.3, 0.2, "bezár", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintClose,  function(button, state)
			if(button == "left" and state == "up") then
