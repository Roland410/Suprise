...
function serviceVehicle(veh)
	if (veh) then
		...
		if not exports.global:takeMoney(source, mechcost) then
			outputChatBox("Nem rendelkezel elegendõ pénzel ezért a szervizelést nem tudjuk elvégezni", source, 255, 0, 0)
		else
			...
			exports.global:sendLocalMeAction(source, "A szervizelés sikeressen megtörtént ")
...
function changeTyre( veh, wheelNumber )
	if (veh) then
		...
		if not exports.global:takeMoney(source, mechcost) then
			...
			exports.global:sendLocalMeAction(source, "Kerékcsere sikeresen megtörtént")
		end
	end
end
