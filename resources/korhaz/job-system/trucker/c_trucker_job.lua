...
function displayTruckerJob(notext)
	if (jobstate==0) then
		...
		if not notext then
			outputChatBox("#FF9933Menj a #CCCCCCTelephelyre#FF9933 és kezd el a munkát.", 255, 194, 15, true)
		end
	end
end
...
function startTruckerJob(routeid)
	if (jobstate==1) then
		...
		if vehicle and getVehicleController(vehicle) == localPlayer and truck[getElementModel(vehicle)] then
			outputChatBox("#FF9933Vezesd a #FFFF00teherautót#FF9933 amig a teljes rakományt le nem szállítottad .", 255, 194, 15, true)
			outputChatBox("#FF9933Ne felejsd el  #FFFF00az utca szabályai rád is vonatkoznak!#FF9933.", 255, 194, 15, true)
			outputChatBox("#FF9933Ha a teherautó #FFFF00rakománya#FF9933, kevesebbet ér .", 255, 194, 15, true)
			outputChatBox("#FF9933Ügyelj hogy éppségben  #FFFF00leszálitsd a rakományt#FF9933!", 255, 194, 15, true)
			destroyElement(blip)
			...
			outputChatBox("A munka elkezdéséhez teherautóba  kell ülnöd.", 255, 0, 0)
		end
	end
end
...
function waitAtDelivery(thePlayer)
	...
	if thePlayer == localPlayer and vehicle and getVehicleController(vehicle) == localPlayer and truck[getElementModel(vehicle)] then
		if getElementHealth(vehicle) < 350 then
			outputChatBox("A teherauó javításra szorul.", 255, 0, 0)
		else
			deliveryStopTimer = setTimer(nextDeliveryCheckpoint, 5000, 1)
			outputChatBox("#Várjon  amíg a teherautórol lepakolják az árut.", 255, 0, 0, true )
...
function checkWaitAtDelivery(thePlayer)
	...
	if vehicle and thePlayer == localPlayer and getVehicleController(vehicle) == localPlayer and truck[getElementModel(vehicle)] then
		if getElementHealth(vehicle) >= 350 then
			outputChatBox("Nem vártál eleget az árut még nem pakolták le.", 255, 0, 0)
...
function nextDeliveryCheckpoint()
	...
	if jobstate == 2 or jobstate == 3 then
		...
		if vehicle and getVehicleController(vehicle) == localPlayer and truck[getElementModel(vehicle)] then
			...
			outputChatBox("#FF9933A teljes rakományt le kell pakolnod.", 255, 0, 0, true ) -- Wrong car type.
		end
	end
end
...
function endDelivery(thePlayer)
	if thePlayer == localPlayer then
		...
		if not vehicle or getVehicleController(vehicle) ~= localPlayer or not (truck[id]) then
			...
			local health = getElementHealth(vehicle)
			if health <= 700 then
				outputChatBox("Ez a kocsi sérült, javítsd meg először.", 255, 194, 15)
			else
