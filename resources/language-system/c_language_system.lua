...
function displayGUI(remotelanguages, rcurrslot)
	if not (wLanguages) then
		...
		wLanguages = guiCreateWindow(x, y, width, height, "Nyelvek: " .. string.gsub(getPlayerName(localPlayer), "_", " "), false)
		...
		if (tlanguages[1]~=nil) then
			...
			bUse1 = guiCreateButton(0.83, 0.08+offset, 0.2, 0.05, "Használ", true, wLanguages)
			bUnlearnLang1 = guiCreateButton(0.83, 0.14+offset, 0.2, 0.05, "Elfelejt", true, wLanguages)
			...
			if (currslot==1) then
				guiSetText(lLang1Name, languages[lang1] .. " (Használatban)")
		...
		if (tlanguages[2]~=nil) then
			...
			bUse2 = guiCreateButton(0.83, 0.08+offset, 0.2, 0.05, "Használ", true, wLanguages)
			bUnlearnLang2 = guiCreateButton(0.83, 0.14+offset, 0.2, 0.05, "Elfelejt", true, wLanguages)
			...
			if (currslot==2) then
				guiSetText(lLang2Name, languages[lang2] .. " (Használatban)")
		...
		if (tlanguages[3]~=nil) then
			...
			bUse3 = guiCreateButton(0.83, 0.08+offset, 0.2, 0.05, "Használ", true, wLanguages)
			bUnlearnLang3 = guiCreateButton(0.83, 0.14+offset, 0.2, 0.05, "Elfelejt", true, wLanguages)
			...
			if (currslot==3) then
				guiSetText(lLang3Name, languages[lang3] .. " (Használatban)")
				guiSetVisible(bUse3, false)
			end
		end
		showCursor(true)
		local bClose = guiCreateButton(0.05, 0.92, 0.9, 0.07, "Bezár", true, wLanguages)
...
function unlearnLanguage(button, state)
	if (button=="left") then
		...
		if (source==bUnlearnLang1 and bUnlearnLang2==nil and  bUnlearnLang3==nil) then
			outputChatBox("Legalább egy nyelvet beszélned kell.", 255, 0, 0)
			return
		...
		if lang > 0 then
			...
			wConfirmUnlearn = guiCreateWindow(sx/2 - 125,sy/2 - 50,250,100,"Megerősítés", false)
			local lQuestion = guiCreateLabel(0.05,0.25,0.9,0.3,"Biztos vagy benne hogy mindent szeretnél elfelejteni a(z) " .. getLanguageName( lang ) .. " nyelvről?",true,wConfirmUnlearn)
