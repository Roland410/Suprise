...
function learnLanguage(player, lang, showmessages, skill)
	...
	if (hasLanguage) then
		if showmessages then
			outputChatBox("Már ismered a(z) " .. languages[lang] .. " nyelvet.", player, 255, 0, 0)
		end
		...
		return false, "Már ismeri a(z) " .. languages[lang] .. " nyelvet"
	else
		local freeslot = getNextEmptyLanguageSlot(player)

		if (freeslot==0) then
			if showmessages then
				outputChatBox("Már elérted a maximum megtanulható nyelvek számát.", player, 255, 0, 0)
			end
...
function useLanguage(lang)
	...
	if (hasLanguage) then
		outputChatBox("Mostantól a(z) " .. languages[lang] .. " nyelvet használod.", source, 255, 194, 14)
...
function unlearnLanguage(lang)
	...
	if (hasLanguage) then
		...
		outputChatBox("Elfelejtetted a(z) " .. languages[lang] .. " nyelvet.", source, 255, 194, 14)
...
for key, value in pairs( languages ) do
	function speakLang( thePlayer, commandName, ... )
		if doesPlayerHaveLanguage( thePlayer, key ) then
			...
			outputChatBox( "Nem beszéled a(z) " .. value .. " nyelvet.", thePlayer, 255, 0, 0 )
		end
	end
