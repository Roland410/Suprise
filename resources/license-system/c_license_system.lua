...
local Johnson = createPed(211,1489.0390, 1305.6357, 1093.2963)
setPedRotation(Johnson, 271.17)
setElementDimension(Johnson, 524)
...
function doTimer(thePed)
	setElementPosition(thePed, 1489.0390, 1305.6357, 1093.2963)
	setPedRotation(thePed, 271.17)
end
...
function showLicenseWindow()
	...
	wLicense= guiCreateWindow(x, y, width, height, "Los Santos Autósiskola", false)
	
	licenseList = guiCreateGridList(0.05, 0.05, 0.9, 0.8, true, wLicense)
	local column = guiGridListAddColumn(licenseList, "Jogosítvány", 0.7)
	local column2 = guiGridListAddColumn(licenseList, "Ár", 0.2)
	
	if (vehiclelicense~=1) then
		local row = guiGridListAddRow(licenseList)
		guiGridListSetItemText(licenseList, row, column, "Személyi gépjármű", false, false)
		guiGridListSetItemText(licenseList, row, column2, "10000", true, false)
	end
				
	bAcceptLicense = guiCreateButton(0.05, 0.85, 0.45, 0.1, "Megvesz", true, wLicense)
	bCancel = guiCreateButton(0.5, 0.85, 0.45, 0.1, "Mégse", true, wLicense)
...
function acceptLicense(button, state)
	if (source==bAcceptLicense) and (button=="left") then
		...
		if (row==-1) or (col==-1) then
			outputChatBox("Válaszd ki milyen jogosítványt szeretnél!", 255, 0, 0)
		else
			local license = 0
			...
			if (licensetext=="Személyi gépjármű") then
				license = 1
			end
				...
				if not exports.global:hasMoney( getLocalPlayer(), licensecost ) then
					...
					if (license == 1) then
						...
						elseif (getElementData(getLocalPlayer(),"license.car")==0) then
							triggerServerEvent("payFee", getLocalPlayer(), 10000)
...
questions = { 
	...
	{"Szabad-e megfordulni kijelölt gyalogos átkelőhelyen?", "Igen", "Nem", "Bármikor szabad", 1},
	{"Autópályán Ön a belső sávban közlekedik. Hogyan kell a megkülönböztető jelzéseit használó gépjármű részére szabad utat adnia, ha a sávokban összefüggő járműoszlopok alakultak ki?", " A forgalmi sáv jobb szélére húzódással.", " A forgalmi sáv belső szélére húzódással.", "Feltartom", 1}, 
	{"Mekkora a személygépkocsik megengedett legnagyobb sebessége?", "Lakott területen 50 km/h, autópályán 130 km/h, autóúton 110 km/h, egyéb úton 90 km/h.", " Lakott területen 50 km/h, autópályán és autóúton 100 km/h, egyéb úton 80 km/h.", "Lakott területen 50 km/h, autópályán 120 km/h, lakott területen kívül és egyéb úton 90 km/h.", 2},
	{"Szállíthat-e a vezető személygépkocsijában egyidejűleg két tíz éven aluli gyermeket és három felnőtt utast, ha a járműben szállítható személyek száma öt?", " Igen,", "Gyerekülésben igen", "Nem", 2},
	{"Az út melyik oldalán kell közlekedni autóval?", "Bal", "Jobb", "Járdán",2},
	{"Mi a teendő,ha a rendőr szirénázva jön mögöttem?", "Félreállok", "Gázra lépek", "Kinevetem" , 1},
	{"Tankolásnál le kell e állitani a motort?", "Igen,", "Felesleges", "Sose ", 1},
	{"Sötétedéskor kell e az autón lámpát kapcsolni?", "Nem,mert lemerül az akksi", "Majd ha a rendőr szól", "Mindig bekell", 3},
	{"Mit csinálsz ha a lámpa pirosra vált?", "Megvárom mig zöldre vált.", "Észre se veszem", "Továbbhajtok", 1},
	{"Szabad e kocsival embert elütni?", "Persze szabad", "Sose", "Csak ha okom van rá", 2}
}
...
function createlicenseTestIntroWindow()
	...
	outputChatBox("Ön fizetett 10.000 Ft-ot a Vezetési Vizsgáért", source, 255, 194, 14)
	...
	guiIntroWindow = guiCreateWindow ( X , Y , Width , Height , "Vezetési Teszt" , false )
	
	guiCreateStaticImage (0.35, 0.1, 0.3, 0.2, "banner.png", true, guiIntroWindow)
	
	guiIntroLabel1 = guiCreateLabel(0, 0.3,1, 0.5, [[Teszt 7 kérdésből fog állni. Tesztkérdéseknek
legalább 80%-nak helyesnek kell lenni,hogy megkapja a jogosítványt.

Sok Szerencsét.]], true, guiIntroWindow)
	
	guiLabelSetHorizontalAlign ( guiIntroLabel1, "center", true )
	guiSetFont ( guiIntroLabel1,"default-bold-small")
	
	guiIntroProceedButton = guiCreateButton ( 0.4 , 0.75 , 0.2, 0.1 , "Teszt indítása" , true ,guiIntroWindow)
...
function createLicenseQuestionWindow(number)
	...
	-- create the window
	guiQuestionWindow = guiCreateWindow ( X , Y , Width , Height , "Kérdés "..number.." ból "..NoQuestionToAnswer , false )
	...
	if(number < NoQuestionToAnswer) then
		guiQuestionNextButton = guiCreateButton ( 0.4 , 0.75 , 0.2, 0.1 , "Következő kérdés" , true ,guiQuestionWindow)
...
function createTestFinishWindow()
	...
	-- create the window
	guiFinishWindow = guiCreateWindow ( X , Y , Width , Height , "Teszt vége.", false )
	
	if(score >= passPercent) then
	
		guiCreateStaticImage (0.35, 0.1, 0.3, 0.2, "pass.png", true, guiFinishWindow)
	
		guiFinalPassLabel = guiCreateLabel(0, 0.3, 1, 0.1, "Gratulálunk! Sikeresen lerakta a vizsgát", true, guiFinishWindow)
		...
		guiFinalPassTextLabel = guiCreateLabel(0, 0.4, 1, 0.4, "Te pontszámod "..score.."%, Ebböl"..passPercent.."%. " ,true, guiFinishWindow)
		guiLabelSetHorizontalAlign ( guiFinalPassTextLabel, "center", true)
		
		guiFinalRegisterButton = guiCreateButton ( 0.35 , 0.8 , 0.3, 0.1 , "Folytatás" , true ,guiFinishWindow)
		...
		guiFinalFailLabel = guiCreateLabel(0, 0.3, 1, 0.1, "Sajnálom, Ön megbukott a vizsgán", true, guiFinishWindow)
		...
		guiFinalFailTextLabel = guiCreateLabel(0, 0.4, 1, 0.4, "Te pontszámod "..math.ceil(score).."%, Ebböl "..passPercent.."%." ,true, guiFinishWindow)
		guiLabelSetHorizontalAlign ( guiFinalFailTextLabel, "center", true)
		
		guiFinalCloseButton = guiCreateButton ( 0.2 , 0.8 , 0.25, 0.1 , "Bezár" , true ,guiFinishWindow)
...
testRoute = {
	{ 1828.6523, -1866.8212, 13.3966 },	-- Start, DMV Parking 
	...
	{ 1828.6523, -1866.8212, 13.3966 },	-- DMV End road [[PARK IT BACKWARDS]]
	{ 1835.1132, -1869.2519, 13.3828 },	-- DMV End road
}
...
function initiateDrivingTest()
	...
	outputChatBox("#FF9933Most menjél ki az oktató kocsiba,és állj be a zöld cp-be hogy elkez a városi vizsgát", 255, 194, 14, true)
...
function startDrivingTest(element)
	if element == getLocalPlayer() then
		...
		if not (testVehicle[id]) then
			outputChatBox("#FF9933Minden cp-n át kell haladni.", 255, 0, 0, true ) -- Wrong car type.
		elseif not exports.global:hasMoney( getLocalPlayer(), 5000) then
			outputChatBox("You can't pay the processing fee.", 255, 0, 0 )
		else
			destroyElement(blip)
			destroyElement(marker)
			
			outputChatBox("Ön fizetett 20.000 Ft-ot a vizsgáért", source, 255, 194, 14)
			triggerServerEvent("payFee", getLocalPlayer(), 20000)
			...
			outputChatBox("#FF9933Vigyázzon,hogy össze ne törje  kocsit.Sok szerencsét", 255, 194, 14, true)
		end
	end
end
...
function UpdateCheckpoints(element)
	if (element == localPlayer) then
		...
		local id = getElementModel(vehicle)
		if not (testVehicle[id]) then
			outputChatBox("Minden cp-n át kell haladni.", 255, 0, 0) -- Wrong car type.
		else
			destroyElement(blip)
			destroyElement(marker)
			blip = nil
			...
			if (tonumber(max_number-1) == tonumber(m_number)) then -- if the next checkpoint is the final checkpoint.
				outputChatBox("#FF9933Kérem parkolja le a kocsit a helyére #FF66C #FF9933hogy befejezze a tesztet", 255, 194, 14, true)
...
function EndTest(element)
	if (element == localPlayer) then
		...
		local id = getElementModel(vehicle)
		if not (testVehicle[id]) then
			outputChatBox("Minden cp-n át kell haladni.", 255, 0, 0)
		else
			...
			if (vehicleHealth >= 800) then
				if not exports.global:hasMoney( getLocalPlayer(), 250 ) then
					...
					-- PASS --
					----------
					outputChatBox("Miután ellenőrizte a járművet látjuk nincs sérülése.", 255, 194, 14)
				...
				-- Fail --
				----------
				outputChatBox("Miután ellenőrizte a járművet azt látjuk, hogy össze van törve.", 255, 194, 14)
				outputChatBox("Ön nem felelt meg a gyakorlati vezetési tesztnek.", 255, 0, 0)
			end
	...
	function( )
		...
		if veh and getVehicleOccupant( veh ) == getLocalPlayer( ) then
			if isElementFrozen( veh ) and getVehicleEngineState( veh ) then
				outputChatBox( "(( A kéziféked be van húzva. Írd be '/kézifék' a kiengedéshez. ))", 255, 194, 14 )
			elseif not getVehicleEngineState( veh ) then
				outputChatBox( "(( A motor le van állítva. Nyomj 'J' betűt az indításhoz. ))", 255, 194, 14 )
			end
		end
	end
)