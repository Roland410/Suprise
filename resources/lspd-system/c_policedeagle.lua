...
function switchMode()
	if (getPedWeapon(localPlayer)==24) and (getPedTotalAmmo(localPlayer)>0) then -- has an un-empty deagle
		...
		if mode == 0 then -- tazer mode
			...
			outputChatBox( "Átváltottad a többfunkciós pisztolyodat Éles üzemmódba.", 0, 255, 0 )
		elseif mode == 1 and isLSPD() then -- lethal mode
			outputChatBox( "Átváltottad a többfunkciós pisztolyodat Vaktöltényes üzemmódba.", 0, 255, 0 )
			triggerServerEvent("deaglemode", localPlayer, 2)
		elseif mode == 2 or mode == 1 then -- radar gun mode
			outputChatBox( "Átváltottad a többfunkciós pisztolyodat Sokkoló üzemmódba.", 0, 255, 0 )
