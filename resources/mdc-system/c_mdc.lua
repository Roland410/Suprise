...
function createDeleteCrimeWindow()
	...
	guiDeleteCrimeWindow = guiCreateWindow ( X , Y , Width , Height , "Bűncselekmény törlése" , false )
	
	guiDeleteCrimeLable = guiCreateLabel(0.05, 0.2, 0.9, 0.3, "Bűncselekmény ID:", true, guiDeleteCrimeWindow)
	
	guiDeleteCrimeEditBox = guiCreateEdit(0.1, 0.4, 0.8, 0.3, "", true, guiDeleteCrimeWindow)
	guiEditSetMaxLength ( guiDeleteCrimeEditBox , 5)
	
	guiDeleteCrimeSubmit = guiCreateButton ( 0.15 , 0.75 , 0.3 , 0.3 , "Törlés" , true ,guiDeleteCrimeWindow)
	guiDeleteCrimeBack = guiCreateButton ( 0.6 , 0.75 , 0.3, 0.3 , "Vissza" , true , guiDeleteCrimeWindow )
		...
		if(button == "left") then
			...
			if(crimeID) then
				if(triggerServerEvent("onDeleteCrime", getLocalPlayer(), crimeID)) then
					...
					if(crimeID) then
						guiSetText(guiMdcMemo, "~~~ Bűntett törlése "..suspectDetails[1].." aktájából. ~~~\n\nEz 10 másodpercet is igénybe vehet - Kérlek várj.")
						setTimer(function ()
				...
				guiSetText(guiMdcMemo, "~~~ A bűncselekmény azonosítója ami meg lett adva nincs kapcsolatba a személlyel akit figyelsz ~~~\n\nCsak akkor törölhetsz bűntettet ha az adott személy aktáját figyeled.")
...
function createDeleteSuspectWindow()
	...
	guiDeleteSuspectWindow = guiCreateWindow ( X , Y , Width , Height , "Körözés törlése" , false )
	
	guiDeleteSuspectLable = guiCreateLabel(0.05, 0.2, 0.9, 0.3, "Körözött neve:", true, guiDeleteSuspectWindow)
	...
	guiDeleteSuspectSubmit = guiCreateButton ( 0.15 , 0.75 , 0.3 , 0.3 , "Törlés" , true ,guiDeleteSuspectWindow)
	guiDeleteSuspectBack = guiCreateButton ( 0.6 , 0.75 , 0.3, 0.3 , "Vissza" , true , guiDeleteSuspectWindow )
		...
		if(button == "left") then
			...
			if not (name == "" ) then
				if(triggerServerEvent("onDeleteSuspect", getLocalPlayer(), name)) then
					...
					guiSetText(guiMdcMemo, "~~~ Körözés törlése '"..name.."' néven az adatbázisból ha létezik. ~~~\n\nEz 10 másodpercet is igénybe vehet - Kérlek várj.")
					setTimer(function ()
...
triggerEvent("onClientPlayerWeaponCheck", localPlayer)
	...
	guiMdcWindow = guiCreateWindow ( X , Y , Width , Height , "Hordozható adatátviteli számítógép" , false )
	
	guiMdcLogo = guiCreateStaticImage ( 0.08 , 0.05 , 0.9 , 0.18, "mdc.png", true, guiMdcWindow) 
	
	guiSearchLabel =  guiCreateLabel ( 0.05 , 0.23 , 0.3 , 0.1 , "Keresés a bűnügyi adatbázisban:", true,  guiMdcWindow )
	...
	if(suspectName == nil) then
		guiSearchEditBox = guiCreateEdit ( 0.05 ,0.28 , 0.3 , 0.05 , "Veztéknév_Keresztnév" , true , guiMdcWindow)
	else
		guiSearchEditBox = guiCreateEdit ( 0.05 ,0.28 , 0.3 , 0.05 , suspectName , true , guiMdcWindow)
	end
	guiSearchButton = guiCreateButton ( 0.36 , 0.28 , 0.2 , 0.05 ,"Keresés" , true , guiMdcWindow )
	guiEditSetMaxLength ( guiSearchEditBox, 20 )
	
	guiSearchLabel =  guiCreateLabel ( 0.60 , 0.23 , 0.3 , 0.1 , "Keresés a jármű adatbázisban:", true,  guiMdcWindow )
	...
	guiSearchVehicleButton = guiCreateButton ( 0.86 , 0.28 , 0.2 , 0.05 ,"Keresés" , true, guiMdcWindow )
	...
	if(user[3] == "1") then
		...
		guiViewAllSuspectsButton = guiCreateButton ( 0.78 , 0.35 , 0.2 , 0.05 , "Összes körözés" , true , guiMdcWindow )
		guiViewSuspectRemoveButton = guiCreateButton ( 0.78 , 0.42 , 0.2 , 0.05 , "Körözés törlése" , true , guiMdcWindow )
		guiAddPersonnelButton = guiCreateButton ( 0.78 , 0.49, 0.2 , 0.05 , "Körözés kiadása" , true , guiMdcWindow )
		guiViewPersonnelButton = guiCreateButton ( 0.78 , 0.56 , 0.2 , 0.05 , "Körözés szerkesztése" , true , guiMdcWindow )
		guiAddWantedButton = guiCreateButton ( 0.78 , 0.63 , 0.2 , 0.05 , "Elfogatási parancs" , true , guiMdcWindow )
		guiAddCrimeButton = guiCreateButton ( 0.78 , 0.7 , 0.2 , 0.05 , "Bűntény hozzá adása" , true , guiMdcWindow )
		guiDeleteCrimeButton = guiCreateButton ( 0.78 , 0.77 , 0.2 , 0.05 , "Bűntény törlése" , true , guiMdcWindow )
		guiEditAccountsButton = guiCreateButton ( 0.78 , 0.84 , 0.2 , 0.05 , "Felhasználó Beállítások" , true , guiMdcWindow )
					...
					if not(guiGetVisible(guiAddCrimeWindow) or guiGetVisible(guiDetailsWindow) or guiGetVisible(guiDeleteCrimeWindow) or guiGetVisible(guiWarrantWindow) ) then
						if(suspectName) then
							if(suspectDetails[2]) then
								...
								guiSetText(guiMdcMemo, "~~~ Nem lehet bűntényt törölni "..suspectName.." aktájából ameddig nem létezik. ~~~\n\nKérlek keress egy másik körözött személyt, vagy adj hozzá az adatbázishoz mielőtt új bűntényt adnál hozzá.")
							end
						else
							guiSetText(guiMdcMemo, "~~~ Először ki kell választanod egy körözött személyt ~~~\n\nÍrd be a körözött nevét a fenti dobozba, majd kattints a keresésre, ha bűntényt akarsz hozzá adni.")
						end
					end
				end
			end
		...
		guiAddPersonnelButton = guiCreateButton ( 0.78 , 0.35 , 0.2 , 0.05 , "Körözés kiadása" , true , guiMdcWindow )
		guiViewPersonnelButton = guiCreateButton ( 0.78 , 0.42 , 0.2 , 0.05 , "Körözés szerkesztése" , true , guiMdcWindow )
		guiAddWantedButton = guiCreateButton ( 0.78 , 0.49 , 0.2 , 0.05 , "Elfogatási parancs" , true , guiMdcWindow )
		guiAddCrimeButton = guiCreateButton ( 0.78 , 0.56 , 0.2 , 0.05 , "Bűntény hozzá adása" , true , guiMdcWindow )
		guiEditAccountsButton = guiCreateButton ( 0.78 , 0.63 , 0.2 , 0.05 , "Felhasználó Beállítások" , true , guiMdcWindow )
	end
	...
	addEventHandler ( "onClientGUIClick", guiEditAccountsButton,  function(button, state) 
		if(clearShowWindow()) then
			if(button == "left") then -- set it visible
				createAccountWindow()
				guiSetVisible(guiAccountWindow, true)
			end
		end
	end, false)
	...
	guiBackButton = guiCreateButton ( 0.78 , 0.9 , 0.2 , 0.05 , "Kijelentkezés" , true , guiMdcWindow )
	...
	addEventHandler ( "onClientGUIClick", guiSearchButton,  function(button, state)
		...
		if(button == "left") then
			if(clearShowWindow()) then
				...
				guiSetInputEnabled(false)
				guiSetText(guiMdcMemo, "~~~ Keresés erre a névre: "..suspectName.." - Kérlek várj ~~~ ")
	...
	addEventHandler ( "onClientGUIClick", guiSearchVehicleButton,  function(button, state)
		...
		if(button == "left") then
			if(clearShowWindow()) then
				...
				guiSetInputEnabled(false)
				guiSetText(guiMdcMemo, "~~~ Keresés erre a járműre: "..suspectName.." - Kérlek várj ~~~ ")
				...
				if not(guiGetVisible(guiAddCrimeWindow) or guiGetVisible(guiDetailsWindow) or guiGetVisible(guiDeleteCrimeWindow) or guiGetVisible(guiWarrantWindow) ) then
					if(suspectName) then
						if (suspectDetails[2]) then
							...
							guiSetText(guiMdcMemo, "~~~ Nincsen akta "..suspectName.." néven így nem lehet személyes információkat lekérni. ~~~\n\nKeress egy másik körözöttet az adat lekérdezéshez.")
						end
					else
						guiSetText(guiMdcMemo, "~~~ Ki kell választanod egy körözöttet mielőtt információkat kérdeznél le ~~~\n\nÍrd be a körözött személy nevét a fenti mezőbe majd nyomj a keresés gombra a személy kiválasztásához.")
					end
				end
			end
		end
				...
				if not(guiGetVisible(guiAddCrimeWindow) or guiGetVisible(guiDetailsWindow) or guiGetVisible(guiDeleteCrimeWindow) or guiGetVisible(guiWarrantWindow) ) then
					if(suspectName) then
						if(suspectDetails[2]) then
							...
							guiSetText(guiMdcMemo, "~~~ Nem tudod megnézni  "..suspectName.." elfogatási parancsát ha nincsen körözési aktája. ~~~\n\nVálassz egy másik körözöttet vagy készíts aktát a személyről.")
						end
					else
						guiSetText(guiMdcMemo, "~~~ Ki kell választanod egy körözött személyt mielőtt megnéznéd az elfogatási parancsot ~~~\n\nÍrd be a körözött személy nevét a fenti mezőbe majd nyomj a keresés gombra a személy kiválasztásához.")
					end
				end
			end
		end
...
function getAllSuspectsText()
	...
	for i, j in pairs(allSuspects) do
		tempsuspect = "  "..j[1].."      - Utoljára szerkesztve "..j[2].." által\n"
	...
	addEventHandler ( "onClientGUIClick", guiAccountShowAllButton,  function(button, state)
		if(button == "left") then
			...
			if(triggerServerEvent("onGetAllAccounts", getLocalPlayer(), details ) ) then
				...
				if guiPhotoImage then
				    outputChatBox(tostring(guiPhotoImage), 255, 0 , 0, true)
					destroyElement(guiPhotoImage)
				end
...
function addSuspectShow(button, state)
	if(button == "left") then
			if(clearShowWindow()) then
				if not (guiDetailsWindow and guiGetVisible(guiDetailsWindow)) then
...
addEventHandler("onCorrectLogInDetails", getLocalPlayer() ,   correctLogIn)
 ...
 function clearShowWindow()
	 if(  ( guiDeleteCrimeWindow  and guiGetVisible(guiDeleteCrimeWindow)) or
	 ( guiAddCrimeWindow and guiGetVisible(guiAddCrimeWindow)) or
	 ( guiDetailsWindow and guiGetVisible(guiDetailsWindow)) or
	 ( guiWarrantWindow and guiGetVisible(guiWarrantWindow)) or
	 ( guiDeleteSuspectWindow and guiGetVisible(guiDeleteSuspectWindow)) or
	 ( guiAccountWindow and guiGetVisible(guiAccountWindow))) then
		return false
	else
		return true
	end
