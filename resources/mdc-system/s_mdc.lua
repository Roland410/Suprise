...
function deleteSuspect(name)
	...
	if (mysql:num_rows(result)>0) then
		...
		outputChatBox(""..name.." névre írt akta sikeresen törölve, az összes bűntettel együtt.", client, 0, 255, 0, true)
	else
		outputChatBox("Nem lehet törölni a "..name.." névre írt aktát ameddig az nincs az adatbázisban.", client, 255, 0, 0, true)
	end
	mysql:free_result(result)
end
