...
function reloadUserPerks( userID )
	if (userID) then
		...
		if (found) then
			...
			outputDebugString("->Hívás('reloadUserPerks', '".. tostring(userID) .."')=200 OK")
		else
			outputDebugString("->Hívás('reloadUserPerks', '".. tostring(userID) .."')=402 USER NOT ONLINE")
		end
	end
end
...
function statTransfer(userID, fromCharacterID, toCharacterID)
	if (userID) and (fromCharacterID) and (toCharacterID) then
		...
		exports.global:sendMessageToAdmins("Figyelem: Felhasználó "..tostring(userID).." utalt " .. fromCharacterName .. " neki ".. toCharacterName)
		...
		local owner = getElementData(value, "owner")
			...
			if (owner) then
				if (tonumber(owner)==tonumber(fromCharacterID)) or (tonumber(owner)==tonumber(toCharacterID)) then
					...
					outputDebugString("* Járművek újratöltve ".. tostring(id))
		...
		for key, value in ipairs( getElementsByType("interior") ) do
			...
			if (owner) then
				if (tonumber(owner)==tonumber(fromCharacterID)) or (tonumber(owner)==tonumber(toCharacterID)) then
					local id = getElementData(value, "dbid")
					outputDebugString("*  interior újratöltve ".. tostring(id))
					exports['interior-system']:realReloadInterior(id)
				end
			end	
		end
		outputDebugString("* Stat átutalás feldolgozása")
	end
end
