...
function payAllWages(timer)
	...
	govAmount = exports.global:getMoney(getTeamFromName("Nav"))
...
function resumeThreads()
	...
	if not inFor then	
		...
		exports.global:setMoney(getTeamFromName("Nav"), govAmount)
...
function doPayDayPlayer(value, hourly)
	...
	elseif (getPlayerIdleTime(value) > 600000) then
		exports.global:sendMessageToAdmins("[Fizetés] '"..getPlayerName(value):gsub("_", " ").."' nem kapott fizetést, mert több, mint 10 percig egy helyben állt.")
	elseif (logged==1) and (timeinserver) and (timeinserver<60) then
		outputChatBox("Nem játszottál eleget, hogy fizetést kapj. (You require another " .. 60-timeinserver .. " Minutes of play.)", value, 255, 0, 0)
	end
end
