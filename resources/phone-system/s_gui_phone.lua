...
function requestPhoneGUI(itemValue, newSource)
	...
	if callerphoneIsTurnedOn == 0 then
		outputChatBox("Telefonod kivan kapcsolva.", client, 255,0,0)
		outputChatBox("((/kapcsol "..ownCellnumber.."  hogy be tudd kapcsolni a telefont))", client, 255,0,0)
		return
	end
...
function addPhoneContact(name, number, phoneBookPhone)
	if (client) then
		...
		outputChatBox("Hiba:Próbáld meg késöbb.", client, 255,0,0)
	end
end
