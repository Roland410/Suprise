			...
			if isElementWithinColShape( thePlayer, v ) then
				for kx, vx in pairs( getElementsByType( "player" ) ) do
					if getElementData( vx, "call.col" ) == v then
						outputChatBox( "Már hívásban vagy elöször tedd le ezt a telefont.", thePlayer, 255, 0, 0 )
						return
					end
				end
				publicphone = v
		...
		if publicphone or exports.global:hasItem(thePlayer, 2) then
			...
			elseif withNumber and withNumber > 10 then
				if exports.global:hasItem(thePlayer, 2, tonumber(withNumber)) or (localPhoneNumber > 10 and tonumber(localPhoneNumber) ==  tonumber(withNumber))  then
					...
					outputDebugString(getPlayerName(thePlayer).." Telefon kivan Kapcsolva")
					return
				end
			else
				...
				if localPhoneNumber and localPhoneNumber > 1 then
					...
					outputDebugString(getPlayerName(thePlayer).." Nincs ilyen telefonszám")
					return
				end
			end
			
			if not outboundPhoneNumber or outboundPhoneNumber == -1 then
				outputChatBox("Hiba: Nyomd meg az F2.", thePlayer, 255, 0, 0)
				return
			end
			
			if not (phoneNumber) then
				outputChatBox("Nyomj 'i' és Katt a telefonra a használathoz.", thePlayer)
				return
			...
			if callerphoneIsTurnedOn == 0 then
				outputChatBox("Telefon kivan kapcsolva.", thePlayer, 255, 0, 0)
			else
				local calling = getElementData(thePlayer, "calling")
				
				if (calling) then -- Using phone already
					outputChatBox("Ön már használja a telefon", thePlayer, 255, 0, 0)
				elseif getElementData(thePlayer, "injuriedanimation") then
					outputChatBox("Elöször térj magadhoz aztán hasznéld.", thePlayer, 255, 0, 0)
				else
					-- /me it
					if publicphone then
						exports.global:sendLocalMeAction(thePlayer, "Nyilvános telefon elérve.")
					else
						exports.global:sendLocalMeAction(thePlayer, "elővesz egy mobiltelefont.")
					end
					...
					-- Otherwise find a fool to answer it
						...
						if not exports.donators:hasPlayerPerk(thePlayer, 6) and not exports.global:hasMoney(thePlayer, 5) then
							outputChatBox("Nincs elég egyenleged a híváshoz.", thePlayer, 255, 0, 0)
							return
						end
						
						-- Yes, Is the target phone online or found at all?
						if not found or foundElement == thePlayer then
							outputChatBox("A hívott szám foglalt...", thePlayer, 255, 194, 14)
							return
						end
						...
						if calledphoneIsTurnedOn == 0 then
							outputChatBox("A hívott szám jelenleg nem kapcsolható.", thePlayer, 255, 194, 14)
							return
						end
						
						-- No, Is the player already calling?
						if getElementData(foundElement, "calling") then
							outputChatBox("A hívott szám foglalt.", thePlayer)
							return
						end
						...
						if not reconning then 
							...
							exports.global:sendLocalMeAction(foundElement, "Telefonja csörög.")
						end
						...
						outputChatBox("Telefon: #"..tostring(phoneNumber).." csörög. A kijelzőn'".. display .."' (( /felvesz a használathoz ))", foundElement, 255, 194, 14)
			...
			outputChatBox("Talán ha lenne nálad telefon sikerülne is..", thePlayer, 255, 0, 0)
		end
	end
end
...
addCommandHandler("hívás", callSomeoneHax)
	...
	if (logged==1) then
		if (exports.global:hasItem(thePlayer, 2)) then
			...
			if getElementData(thePlayer, "called") then
				outputChatBox("Te vagy az egyetlen hívó valaki, okostojás.", thePlayer, 255, 0, 0)
			elseif (calling) then
				if (phoneState==0) then
					local found, foundElement = searchForPhone(calling)
					--local target = calling
					outputChatBox("Felvetted a telefont. (( /p a telefonba beszélés ))", thePlayer)
					if not found then
						outputChatBox("", thePlayer)
						executeCommandHandler( "hangup", thePlayer )
					else
						outputChatBox("Felvették a telefont.", foundElement)
						exports.global:sendLocalMeAction(thePlayer, "előveszi a telefont.")
						exports['anticheat-system']:changeProtectedElementDataEx(thePlayer, "phonestate", 1, false)
						exports['anticheat-system']:changeProtectedElementDataEx(foundElement, "phonestate", 1, false)
						exports['anticheat-system']:changeProtectedElementDataEx(foundElement, "called", nil, false)
						exports.global:sendLocalMeAction(thePlayer, "fogadja a hívását.")
			...
			elseif not (calling) then
				outputChatBox("Telefon nem cseng.", thePlayer, 255, 0, 0)
			elseif (phoneState==1) or (phoneState==2) then
				outputChatBox("A telefon használatban van.", thePlayer, 255, 0, 0)
			end
		else
			outputChatBox("Talán ha lenne nálad telefon sikerülne is..", thePlayer, 255, 0, 0)
		end
...
addCommandHandler("felvesz", answerPhone)
	...
	if (logged==1) then
		if (exports.global:hasItem(thePlayer, 2)) or getElementData(thePlayer, "call.col") then
			...
			if (calling) then
				if not (isNumberAHotline(calling)) then
					...
					if found then
						if (isElement(foundElement)) then
							outputChatBox("Letették", foundElement)
							local ownPhoneNo = getElementData(foundElement, "calling")
				...
				outputChatBox("A telefonodat nem használod.", thePlayer, 255, 0, 0)
			end
		else
			outputChatBox("Talán ha lenne nálad telefon sikerülne is..", thePlayer, 255, 0, 0)
		end
...
addCommandHandler("letesz", hangupPhone)
...
function loudSpeaker(thePlayer, commandName)
	...
	if (logged==1) then
		if (exports.global:hasItem(thePlayer, 2)) or getElementData(thePlayer, "call.col") then -- 2 = Cell phone item
			...
			if (phoneState==1) then
				exports.global:sendLocalMeAction(thePlayer, "bekapcsolja a kihangosítót a telefonján.")
				outputChatBox("Kihangosítottad a telefonod.", thePlayer)
				exports['anticheat-system']:changeProtectedElementDataEx(thePlayer, "phonestate", 2, false)
			elseif (phoneState==2) then
				exports.global:sendLocalMeAction(thePlayer, "kikapcsolja a kihangosítót a telefonján.")
				outputChatBox("Visszahalkítottad a telefonod.", thePlayer)
				exports['anticheat-system']:changeProtectedElementDataEx(thePlayer, "phonestate", 1, false)
			else
				outputChatBox("Nem vagy hívás közben.", thePlayer, 255, 0 ,0)
			end
		else
			outputChatBox("Talán ha lenne nálad telefon sikerülne is..", thePlayer, 255, 0, 0)
		end
...
addCommandHandler("hangositás", loudSpeaker)
...
function talkPhone(thePlayer, commandName, ...)
	...
	if (logged==1) then
		if (exports.global:hasItem(thePlayer, 2)) or getElementData(thePlayer, "call.col") then
			if not (...) then
				outputChatBox("Parancs: /p [Üzenet]", thePlayer, 255, 194, 14)
			elseif getElementData(thePlayer, "injuriedanimation") then
				outputChatBox("YSérülten nem használhatod", thePlayer, 255, 0, 0)
			else
				local phoneState = getElementData(thePlayer, "phonestate")
				...
				if (phoneState>=1) then 
					...
					if (callprogress) then
						outputChatBox("Te [Mobiltelefonba]: " ..message, thePlayer)
						-- Send it to nearby players of the speaker
						exports.global:sendLocalText(thePlayer, username .. " [mobiltelefonba]: " .. message, nil, nil, nil, 10, {[thePlayer] = true})
					...
					outputChatBox("[" .. languagename .. "] ((" .. username .. ")) [mobiltelefonba]: " .. message2, target)
					
					-- Send the message to the person on the other end of the line
					outputChatBox("[" .. languagename .. "] Te [mobiltelefonba]: " ..message, thePlayer)
					...
					if (phoneState==2) then -- Loudspeaker
						...
						for index, nearbyPlayer in ipairs(getElementsByType("player")) do
							if isElement(nearbyPlayer) and nearbyPlayer ~= target and getDistanceBetweenPoints3D(x, y, z, getElementPosition(nearbyPlayer)) < 40 and getElementDimension(nearbyPlayer) == getElementDimension(target) then
								...
								outputChatBox("[" .. languagename .. "] " .. username .. "'Mobil Hangszóró: " .. message2, nearbyPlayer)
					...
					outputChatBox("Nem vagy hívásban.", thePlayer, 255, 0, 0)
				end
			end
		else
			outputChatBox("Talán ha lenne nálad telefon sikerülne is..", thePlayer, 255, 0, 0)
	...
	if (logged==1) then
		if (exports.global:hasItem(thePlayer, 7)) then
			if not (partialNick) then
				outputChatBox("Parancs: /phonebook [Játékos id]", thePlayer, 255, 194, 14)
			else
				exports.global:sendLocalMeAction(thePlayer, "keresgél a telefonkönyvben.")
				...
				if (mysql:num_rows(result)>10) then
					outputChatBox("sok találat.", thePlayer, 255, 194, 14)
				elseif (mysql:num_rows(result)>0) then
					...
					outputChatBox("Nincs ilyen személy a telefonkönyvben..", thePlayer, 255, 194, 14)
				end
...
addCommandHandler("telefonkönyv", phoneBook)
	...
	if (logged==1) then
		if (exports.global:hasItem(thePlayer, 2)) then
			...
			if not tonumber(number) then
				outputChatBox("Hibás Telefonszám.", thePlayer)
				return
			end
			...
			if not number or not (...) then
				outputChatBox("Parancs: /" .. commandName .. " [szám] [üzenet]", thePlayer, 255, 194, 14)
			elseif callerphoneIsTurnedOn == 0 then
				outputChatBox("Telefon kivan kapcsolva", thePlayer, 255, 0, 0)
			elseif getElementData(thePlayer, "injuriedanimation") then
				outputChatBox("Sérülten nem küldesz semmit", thePlayer, 255, 0, 0)
			elseif exports.global:hasMoney(thePlayer, 1) or exports.donators:hasPlayerPerk(thePlayer, 6) then
				...
				if tonumber(number) == 921 then
					...
					local reconning = getElementData(thePlayer, "reconx")
					if not reconning then
						exports.global:sendLocalMeAction(thePlayer, "küld egy szöveges üzenetet.")
					end
					outputChatBox("["..languagename.."] SMS elküldve #921 [#"..tostring(phoneNumber).."]: "..message, thePlayer, 120, 255, 80)
					setTimer( 
						function( thePlayer )
							if isElement( thePlayer ) then
								...
								if id then
									...
									if impounded then
										...
										if amount > 0 then
											outputChatBox("[Angol] SMS neki #921 [#"..phoneNumber.."]: " .. amount .. " of your vehicles are impounded. Head over to the Impound to release them.", thePlayer, 120, 255, 80)
										else
											outputChatBox("[Angol] SMS neki #921 [#"..phoneNumber.."]: None of your vehicles are impounded.", thePlayer, 120, 255, 80)
										end
									end
								end
							end
				...
				elseif tonumber(number) == 7332 then
					...
					elseif getElementData(thePlayer, "alcohollevel") and getElementData(thePlayer, "alcohollevel") ~= 0 then
						...
						local reconning2 = getElementData(thePlayer, "reconx")
						if not reconning2 then
							exports.global:sendLocalMeAction(thePlayer, "Üzenet elküldve.")
						end
						local message = table.concat({...}, " ")
				...
				outputChatBox("Nem sikerült elküldeni az sms-t.", thePlayer, 255, 0, 0)
			end
		else
			outputChatBox("Talán ha lenne nálad telefon sikerülne is..", thePlayer, 255, 0, 0)
		end
	...
	if logged == 1 then
		...
		elseif tonumber(phoneNumber) < 10 then
			...
			if not (exports.global:hasItem(thePlayer, 2, tonumber(phoneNumber))) then
				outputChatBox("Használazban a telefon", thePlayer, 255, 0, 0)
				return
			end
		end
		...
		if getElementData( thePlayer, "calling" ) then
			outputChatBox("Már hivásban vagy!", thePlayer, 255, 0, 0)
		else
			if calledphoneIsTurnedOn == 0 then
				outputChatBox("Bekapcsoltad a telefonod '"..tostring(phoneNumber).."' on.", thePlayer, 0, 255, 0)
			else
				outputChatBox("Kikapcsoltad a telefonod '"..tostring(phoneNumber).."' off.", thePlayer, 255, 0, 0)
			end
...
addCommandHandler("kikapcsol", togglePhone)
