...
function showPokerHand_func ( card1, card2 )
	...
	if gWindow["pokerOwnCards"] then
		...
		gWindow["pokerOwnCards"] = guiCreateWindow(screenwidth-303,screenheight-20,303,265,"Kézben lévő lapok",false)
...
function refreshPokerVarbs_func ( ownBet, curpot, curbet, curchips, p1, p2, p3, p4, p5, p6, p1s, p2s, p3s, p4s, p5s, p6s )
	...
	guiSetText ( gLabel["pokerPotAmount"], curpot.." Ft" )
	pokerCurBets = curbet
...
function showPokerDisplay ()
	...
	if gWindow["pokerCommunity"] then
		...
		gWindow["pokerCommunity"] = guiCreateWindow(screenwidth/2-450/2,screenheight/2-256/2,450,259,"Feldobott lapok",false)
		...
		gButton["pokerCheck"] = guiCreateButton(0.2443+0.17*-1+0.0251*-1,0.695,0.17,0.1351,"Passz",true,gWindow["pokerCommunity"])
		guiSetAlpha(gButton["pokerCheck"],0)
			...
			function ()
				if guiGetAlpha ( source ) == 1 and source == gButton["pokerCheck"] and block then
					if pokerCurBets == 0 or pokerCurBets == curOwnBet then
						...
						outputChatBox ( "Nem passzolhat!", 125, 0, 0 )
					end
				end
			end
		)
		
		gButton["pokerCall"] = guiCreateButton(0.2443,0.695,0.17,0.1351,"Megadás",true,gWindow["pokerCommunity"])
		guiSetAlpha(gButton["pokerCall"],0)
		...
		gButton["pokerFold"] = guiCreateButton(0.2443+0.17+0.0251,0.695,0.17,0.1351,"Bedobás",true,gWindow["pokerCommunity"])
		guiSetAlpha(gButton["pokerFold"],0)
		...
		gButton["pokerRaise"] = guiCreateButton(0.2443+0.17*2+0.0251*2,0.695,0.1005,0.139,"Emelés",true,gWindow["pokerCommunity"])
		guiSetAlpha(gButton["pokerRaise"],0)
				...
				if guiGetAlpha ( source ) == 1 and source == gButton["pokerRaise"] and block then
					...
					if raiseAmount >= 10 then
						...
						if pokerCurOwnChips >= raiseAmount then
							...
							outputChatBox ( "Nincs Elég Zsetonja!", 125, 0, 0 )
						end
					else
						outputChatBox ( "Legalább 10!", 125, 0, 0 )
					end
				end
			end
		)
		
		gButton["pokerLeave"] = guiCreateButton(0.7717,0.8417,0.18491,0.1236,"Kilép",true,gWindow["pokerCommunity"])
		guiSetAlpha(gButton["pokerLeave"],1)
		...
		gLabel["pokerPotAmount"] = guiCreateLabel(0.0365,0.9189,0.1142,0.0811,"- Ft",true,gWindow["pokerCommunity"])
		...
		gLabel["pokerInfo2"] = guiCreateLabel(0.1712,0.8456,0.121,0.0656,"Játékos:",true,gWindow["pokerCommunity"])
