...
createBlip ( 2225.3964, 1836.2773, 10.8203, 25 )
...
pokerEntranceMarker = createMarker ( 1120.2929, 9.2148, 1001.0300, "cylinder", 1.5, 125, 0, 0, 125 )
setElementInterior ( pokerEntranceMarker, 12 )
setElementDimension ( pokerEntranceMarker, 717 )
-- The poker table
pokerTable = createObject ( 2189, 1119.17, 11.49, 1001.88 )
setElementInterior ( pokerTable, 12 )
setElementDimension ( pokerTable, 717 )
...
setElementDimension ( pokerChair1, 717 )
setElementInterior ( pokerChair2, 12 )
setElementDimension ( pokerChair2, 717 )
				...
				if pokerCurPlayers <= 5 then
					...
					for i = 1, 6 do
						if not _G["pokerPlayer"..i] then
							...
							sendMessageToAllPokerPlayers ( getPlayerName(hit).." Leült az asztalhoz!", 200, 200, 0 )
					...
					outputChatBox ( "Asztal megtelt kérem várjon!", hit, 125, 0, 0 )
				end
			end
		end
	end
...
function takePlayerChips ( player, amount )
	...
	if _G["pokerPlayer"..player.."Money"] < 0 then
		...
		sendMessageToAllPokerPlayers ( getPlayerName(_G["pokerPlayer"..player]).." All in!", 0, 0, 170 )
	end
end
...
function roundCheck()
	...
	if pokerCurPlayers > 1 then
		curPokerPot = 0
		sendMessageToAllPokerPlayers ( "Osztó: Új kör!", 200, 200, 0 )
		...
		sendMessageToAllPokerPlayers ( "Osztó: Nagy Vak "..getPlayerName(_G["pokerPlayer"..curPokerBlindPlayer])..", Kis Vak: "..getPlayerName(_G["pokerPlayer"..smallBlindPlayer]).."!", 200, 200, 0 )
		cardsInUse = nil
		cardsInUse = {}
		pokerFirstLoop = false
...
function playerTurnCheck_func ( player )
	...
	sendMessageToAllPokerPlayers ( getPlayerName(player).." Passz!", 200, 0, 0 )
...
function playerTurnCall_func ( player )
	...
	local id = getPlayerPokerID ( player )
	sendMessageToAllPokerPlayers ( getPlayerName(player).." Hívott  "..getHighestCall().." )!", 200, 0, 0 )
	...
	if amount >= _G["pokerPlayer"..id.."Money"] then
		...
		sendMessageToAllPokerPlayers ( getPlayerName(_G["pokerPlayer"..id]).."  all in!", 0, 0, 170 )
	else
...
function playerTurnRaise_func ( amount )
	...
	local player = source
	sendMessageToAllPokerPlayers ( getPlayerName(player).." Emel : "..amount.."!", 200, 0, 0 )
	local id = getPlayerPokerID ( player )
...
function playerTurnFold_func ()
	...
	local player = source
	local id = getPlayerPokerID ( player )
	sendMessageToAllPokerPlayers ( getPlayerName(player).." Bedob!", 200, 0, 0 )
...
function nextRoundPlayer ()
	...
	if counter == 0 then
		sendMessageToAllPokerPlayers ( "Játék vége!", 0, 125, 0 )
		resetPokerValues()
	elseif counter == 1 then
		sendMessageToAllPokerPlayers ( "Játék vége - Nyertes: "..getPlayerName(_G["pokerPlayer"..pokerCPlayerFolder]).."!", 0, 125, 0 )
...
function setPlayerLeavingPoker_func ( player, id )
	...
	if isElement ( player ) then
		...
		setElementPosition ( player, 1120.2929, 9.2148, 1002.0784 )
		setElementInterior ( player, 12 )
		setElementDimension ( player, 717 )
		setCameraTarget ( player, player )
		toggleAllControls ( player, true, true, true )
		setPedAnimation ( player )
		showCursor ( player, false )
		setElementData ( player, "ElementClicked", false )
		sendMessageToAllPokerPlayers ( getPlayerName(player).." Felált az asztaltól és távozik!", 200, 200, 0 )
...
function nextPlayer ()
	...
	sendMessageToAllPokerPlayers ( getPlayerName(_G["pokerPlayer"..curPokerPlayer]).."'Következik!", 0, 250, 0 )
	outputChatBox ( "aktuális hívás: "..getHighestCall()..", a hívást: ".._G["pokerPlayer"..curPokerPlayer.."Bet"], _G["pokerPlayer"..curPokerPlayer], 150, 150, 25 )
	if isTimer ( timeoutTimer ) then
		killTimer ( timeoutTimer )
	end
...
function finalCheck ()
	...
	if curWinners == 1 then
		...
		if _G["pokerPlayer"..curWinner.."Money"] <= 0 then
			...
			sendMessageToAllPokerPlayers ( winnername.." Győzött "..value..". ( Kéz: "..h1..", "..h2.." ), "..(_G["pokerPlayer"..curWinner.."TotalBet"] * 2).." chips", 0, 125, 0 )
			if curPokerPot > 0 then
				...
				sendMessageToAllPokerPlayers ( "fennmaradó "..curPokerPot.." chips kerül elosztásra!", 0, 125, 0 )
				for i = 1, 6 do
					if _G["pokerPlayer"..i.."In"] then
						if i == curWinner then
						else
			...
			sendMessageToAllPokerPlayers ( winnername.." Nyertes: "..value..". ( Kéz: "..h1..", "..h2.." ), "..curPokerPot.." Összeg", 0, 125, 0 )
		...
		sendMessageToAllPokerPlayers ( "Osztott pot között: "..winnernames..", "..splot.." Lapok ( minden )!", 0, 125, 0 )
		for i, key in ipairs ( curWinnersList ) do
