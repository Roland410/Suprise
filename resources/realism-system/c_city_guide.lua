...
function createCityGuide( key, keyState )
	...
	if not (wGuidebook) then
		...
		-- Create the window
		wGuidebook = guiCreateWindow(X, Y, Width, Height, "Város Ismertető", false)
		
		-- Create close, previous and Next Button
		buttonPrev = guiCreateButton( 0.85, 0.25, 0.14, 0.05, "Vissza", true, wGuidebook)
		addEventHandler( "onClientGUIClick", buttonPrev, prevButtonClick, false )
		guiSetVisible(buttonPrev, false)
		
		buttonClose = guiCreateButton( 0.85, 0.45, 0.14, 0.05, "Bezár", true, wGuidebook)
		addEventHandler( "onClientGUIClick", buttonClose, closeButtonClick, false )
		
		buttonNext = guiCreateButton( 0.85, 0.65, 0.14, 0.05, "Következő", true, wGuidebook)
