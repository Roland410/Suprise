...
table.insert(limitSpeed, 522, 320) -- NRG-500
			...
			if (ccEnabled) then
				...
				if targetSpeed > 4 then
					if (getVehicleType(theVehicle) == "Automobile" or getVehicleType(theVehicle) == "Bike" or getVehicleType(theVehicle) =="Boat" or getVehicleType(theVehicle) == "Train") then
						outputChatBox("Tempómat bekapcsolva", 0,255,0)
						outputChatBox("Nyomd - és + sebesség változtatáshoz", 0,255,0)
						activateCruiseControl()
					end
				else
					outputChatBox("Tempómat fixálva.", 0,255,0)
				end
			end
		end
	end
