...
function inApproachStation(stationID)
	outputChatBox("Most közeledik " .. stationNames[stationID] .. ".")
end
...
function inAtStation(stationID)
	if (stationID + 1 < #stationNames) then
		outputChatBox("Ez " .. stationNames[stationID] .. ". Ez a vonat a " .. stationNames[#stationNames] .. ". The next stop is " .. stationNames[stationID+1] .. ".")
	else
		outputChatBox("Ez " .. stationNames[stationID] .. ". Ahol ez a vonat megszűnik.")
	end
end
		...
		if ( dimension ) and ( getVehicleType(train) == "Train" )  then
			...
			if (getTrainSpeed(train) > 0) then
				...
				if (absspeed > 5.9) then
					outputChatBox("gyorshajtás alkalmazott vészfékezés.")
					speed = absspeed
					timer = setTimer(emergencyBrake, 100, 0, train)
				end
				
				if (signalsRight[id]["state"] == 0) then -- passed red signal
					outputChatBox("Átment a piros jelzésen,ezért vészféket alkalmazott.")
					speed = absspeed
					timer = setTimer(emergencyBrake, 100, 0, train)
				end
...
function acknowledgeAWS(train)
	stopSound(sound)
	killTimer(timer)
	outputChatBox("AWS reszetelve.", 0, 255, 0)
end

function activateAWS(train)
	outputChatBox("Nem reagál a AWS, vészfék alkalmazására.", 255, 0, 0)
...
function handleVelocity()
	...
	if (train) then
		...
		guiSetText(lThrottle, "gázkar: " .. tostring((100 - guiScrollBarGetScrollPosition(scrollThrottle))) .. "%")
		guiSetText(lBrake, "Fék: " .. tostring((100 - guiScrollBarGetScrollPosition(scrollBrake))) .. "%")
...
function enterTrain(train)
	if ( getVehicleType(train)=="Train") then
		...
		wTrain = guiCreateWindow(x, y, width, height, "Vonat", false)
		...
		lThrottle = guiCreateLabel(0.1, 0.6, 0.5, 0.1, "Gázkar: 0%", true, wTrain)
		lBrake = guiCreateLabel(0.6, 0.6, 0.5, 0.1, "Fék: 0%", true, wTrain)
