...
function roadblockCreateWorldObject(objectid, tempObjectPosX, tempObjectPosY, tempObjectPosZ, tempObjectPosRot)
	if(isRoadblockEditAllowed(client)) then
		local slot = 0
		local korrX = math.abs(20.0*math.cos((tempObjectPosRot+90)*math.pi/180))
		local korrY = math.abs(4.0*math.sin((tempObjectPosRot+90)*math.pi/180))
		for i = 1, maxroadblocks do
			if (roadblocksarr[i]==nil) then
				...
				--if (objectid == 2899) then -- közepes szöges
				--	local shape = createColCuboid ( tempObjectPosX, tempObjectPosY, tempObjectPosZ, 8.0, 2.0, 2.5)
				--	exports.pool:allocateElement(shape)
				--	exports['anticheat-system']:changeProtectedElementDataEx(shape, "type", "spikes")
				--	exports['anticheat-system']:changeProtectedElementDataEx(roadblocksarr[i], "spike:shape1", shape, false)
				--end
				
				if (objectid == 2892) then -- Nagy szöges
					outputChatBox(tempObjectPosRot, client, 0, 255, 0)
					outputChatBox(korrX, client, 0, 255, 0)
					outputChatBox(korrY, client, 0, 255, 0)
										
					local shape = createColCuboid ( tempObjectPosX-korrX/2, tempObjectPosY-korrY/2, tempObjectPosZ, korrX, korrY, 2.5)
					exports.pool:allocateElement(shape)
					exports['anticheat-system']:changeProtectedElementDataEx(shape, "type", "spikes")
					exports['anticheat-system']:changeProtectedElementDataEx(roadblocksarr[i], "spike:shape1", shape, false)
				end
		...
		if not (slot == 0) then
			outputChatBox("Útzár lerakva ID #" .. slot.. ".", client, 0, 255, 0)
			exports['logs']:dbLog(client, 28, client, "Lerakott útzár "..tostring(slot))
		else
			outputChatBox("Több útzárat már nem tehetsz le szedj fel párat", client, 0, 255, 0)
		end
	end
end
	...
	if(isRoadblockEditAllowed(thePlayer)) then
		...
		for i = 1, maxroadblocks do
			if not (roadblocksarr[i]==nil) then
				...
				if (distance<=10) then
					outputChatBox("   útzár id " .. i .. ".", thePlayer, 255, 126, 0)
					found = true
				end
			end
		end
		...
		if not (found) then
			outputChatBox("   Nem.", thePlayer, 255, 126, 0)
		end
	end
end
...
function removeRoadblock(thePlayer, commandName, id)
	if(isRoadblockEditAllowed(thePlayer)) then
		if not (id) then
			outputChatBox("Parancs: /" .. commandName .. " [útzár ID]", thePlayer, 255, 194, 15)
		else
			id = tonumber(id)
			if (roadblocksarr[id]==nil) then
				outputChatBox("nem utzár.", thePlayer, 255, 0, 0)
			else
				...
				exports['logs']:dbLog(thePlayer, 28, thePlayer, "Útzár felvéve "..tostring(id))
				outputChatBox("Útzár felvéve ID #" .. id .. ".", thePlayer, 0, 255, 0)
			end
		end
	end
end
...
function removeAllRoadblocks(thePlayer, commandName)
	if(exports.global:isPlayerAdmin(thePlayer)) then
		...
		roadblocksarr = { }
		exports['logs']:dbLog(thePlayer, 28, thePlayer, "Összes útzár felvéve")
		outputChatBox("Összes útzár felvéve", thePlayer, 0, 255, 0)
	end
end
