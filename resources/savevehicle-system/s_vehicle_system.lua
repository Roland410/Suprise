...
function createPermVehicle(thePlayer, commandName, ...)
	if (exports.global:isPlayerSuperAdmin(thePlayer)) then
		local args = {...}
		if (#args < 7) then
			outputChatBox("HASZNÁLD: /" .. commandName .. " [id/NÉV] [szin1] [szin2] [Tulaj] [Frakció jármû (1/0)] [Ára] [Ablak státusz 0/1] ", thePlayer, 255, 194, 14)
			outputChatBox("MEGJEGYZÉS: Ha frakció jármû, a tulaj frakciójához fog tartozni a jármû.", thePlayer, 255, 194, 14)
			outputChatBox("MEGJEGYZÉS: Ha frakció jármû, az ára le lesz vonva a frakció pénzébõl.", thePlayer, 255, 194, 14)
		else
			...
			if targetPlayer then
				...
				if (factionVehicle==1) then
					...
					if not exports.global:takeMoney(theTeam, cost) then
						outputChatBox("Ez a frakció nem engedheti meg magának ezt a jármûvet.", thePlayer, 255, 0, 0)
						return
					end
				else
					factionVehicle = -1
					if not exports.global:takeMoney(targetPlayer, cost) then
						outputChatBox("Ez a játékos nem engedheti meg magának ezt a jármûvet.", thePlayer, 255, 0, 0)
						return
					elseif not exports.global:canPlayerBuyVehicle(targetPlayer) then
						outputChatBox("Ennek a játékosnak túl sok jármûve van.", thePlayer, 255, 0, 0)
						return
					elseif ( getVehicleType(id) == "Helicopter" or getVehicleType(id) == "Plane" ) and not exports.global:hasItem(targetPlayer, 78) then
						outputChatBox("Ennek a játékosnak nincs pilóta engedélye.", thePlayer, 255, 0, 0)
						return
				...
				if not (veh) then
					...
					if (insertid) then
						...
						outputChatBox(getVehicleName(veh) .. " lespawnolva (ID #" .. insertid .. ").", thePlayer, 255, 194, 14)
						
						local owner = ""
						if factionVehicle == -1 then
							owner = "Tulaj: " .. getPlayerName( targetPlayer )
						else
							owner = "Frakció #" .. factionVehicle
						end
...
function loadOneVehicle(id, hasCoroutine)
	...
	if row then
		...
		if veh then
			...
			-- by silenthunter
			if row.model == 420 or row.model == 438 then
				setVehicleColor(veh, 6,6,6,6)--az összes taxi sárga lesz
			else
				if row.color1 == -1 or row.color2 == -1 then
					setVehicleColor(veh, math.random(0,126), math.random(0,126), math.random(0,126), math.random(0,126))
				else
			...
			setVehicleSirensOn(veh, row.sirens == 0)
...
function setEngineStatusOnEnter(thePlayer, seat)
	if getElementType ( thePlayer ) == "ped" then--by silenthunter
		return true
	end
	if seat == 0 then
				...
				if engine == 0 then
					...
					if (vehKey) or (owner < 0) and (faction == -1) or (playerFaction == faction) and (faction ~= -1) or ((getElementData(source, "adminduty") or 0) == 1) then
						...
						if broke == 1 then
							...
							outputChatBox("A jármû motorja sérült.", source)
						...
						elseif fuel >= 1 then
							...
							exports.global:sendLocalMeAction(source, "beindította a jármű motorját.")
						elseif fuel < 1 then
							exports.global:sendLocalMeAction(source, "megpróbálja beindítani a jármûvet, de nem sikerül.")
							outputChatBox("Kifogyott a benzin.", source)
						end
					else
						outputChatBox("Nincs hozzá kulcsod.", source, 255, 0, 0)
					end
				else
					...
					setVehicleEngineState(veh, false)
					exports['anticheat-system']:changeProtectedElementDataEx(veh, "engine", 0, false)
					exports.global:sendLocalMeAction(source, "leállította a jármű motorját.")
				...
				if (owner ~= -2) then
					if ( getElementData(vehicle, "Impounded") or 0 ) == 0 then
						...
						if (locked) then
							...
							exports.global:sendLocalMeAction(source, "kinyitja a jármûvet.")
						else
							setVehicleLocked(vehicle, true)
							exports.global:sendLocalMeAction(source, "lezárja a jármûvet.")
						end
					else
						outputChatBox("(( Lefoglalt jármûvet nem tudsz lezárni. ))", source, 255, 195, 14)
					end
				else
					outputChatBox("(( Civil jármûvet nem tudsz lezárni. ))", source, 255, 195, 14)
				end
				return
			end
		end
...
function checkLock(thePlayer, seat, jacked)
	...
	if (locked) and not (jacked) then
		cancelEvent()
		outputChatBox("Ajtók zárva.", thePlayer)
	end
end
...
function checkBikeLock(thePlayer)
	if (isVehicleLocked(source)) and (getVehicleType(source)=="Bike" or getVehicleType(source)=="Boat" or getVehicleType(source)=="BMX" or getVehicleType(source)=="Quad" or getElementModel(source)==568 or getElementModel(source)==571 or getElementModel(source)==572 or getElementModel(source)==424) then
		if not getElementData(thePlayer, "interiormarker") then
			outputChatBox("Jármû le van zárva.", thePlayer, 255, 194, 15)
		end
		cancelEvent()
	end
end
...
function setRealInVehicle(thePlayer)
	...
	if isVehicleLocked(source) then
		...
		if (owner < 0) and (faction == -1) then
			outputChatBox("(( Civil jármû: '" .. carName .. "' ))", thePlayer, 255, 195, 14)
		elseif (faction==-1) and (owner>0) then
			local ownerName = exports['cache']:getCharacterName(owner)
			
			if ownerName then
				outputChatBox("(( Jármû tulaja: " .. ownerName .. ". ))", thePlayer, 255, 195, 14)
				if (getElementData(source, "Impounded") > 0) then
					local output = getRealTime().yearday-getElementData(source, "Impounded")
					outputChatBox("(( Ez a " .. carName .. " le van foglalva: " .. output .. (output == 1 and " Napra." or " Napra.") .. " ))", thePlayer, 255, 195, 14)
				end
			end
		end
	end
	...
	if (vfaction~=-1) then
		...
		if (faction~=vfaction) and (seat==0) then
			if (CanTowDriverEnter) then
				--outputChatBox("(( Ez a " .. getVehicleName(source) .. " a '" .. factionName .. "'-hez tartozik. ))", thePlayer, 255, 194, 14)
				exports['anticheat-system']:changeProtectedElementDataEx(source, "enginebroke", 1, false)
				setVehicleDamageProof(source, true)
				setVehicleEngineState(source, false)
				return
			end
			outputChatBox("Nem vagy a tagja a '" .. factionName .. "'-nek/nak.", thePlayer, 255, 194, 14)
--[[			exports['anticheat-system']:changeProtectedElementDataEx(thePlayer, "realinvehicle", 0, false)
			removePedFromVehicle(thePlayer)
			local x, y, z = getElementPosition(thePlayer)
...
--[[			if not exports['global']:isPlayerAdmin(thePlayer) then
	...
	if (vjob > 0) and (seat==0) then
		if (job~=vjob) then
			if (vjob==1) then
				outputChatBox("Nem vagy Csomaszállító!", thePlayer, 255, 0, 0)
			elseif (vjob==2) then
				outputChatBox("Nem vagy Taxi sofõr!", thePlayer, 255, 0, 0)
			elseif (vjob==3) then
				outputChatBox("Nem vagy Busz sofõr!", thePlayer, 255, 0, 0)
			end
			exports['anticheat-system']:changeProtectedElementDataEx(thePlayer, "realinvehicle", 0, false)
			removePedFromVehicle(thePlayer)
			local x, y, z = getElementPosition(thePlayer)
...
function sellVehicle(thePlayer, commandName, targetPlayerName)
	...
	if isPedInVehicle(thePlayer) then
		if not targetPlayerName then
			outputChatBox("HASZNÁLD: /" .. commandName .. " [név / id]", thePlayer, 255, 194, 14)
			outputChatBox("Add el a járműved egy másik játékosnak.", thePlayer, 255, 194, 14)
			outputChatBox("Mond a vevőnek hogy a /pay parancsal fizesse ki a jármű árát.", thePlayer, 255, 194, 14)
		else
								...
								if exports.global:hasSpaceForItem(targetPlayer, 3) then
									if exports.global:canPlayerBuyVehicle(targetPlayer) then
										...
										if query then
											...
											outputChatBox("Sikeresen eladtad a " .. getVehicleName(theVehicle) .. "-t neki" .. targetPlayerName .. ".", thePlayer, 0, 255, 0)
											outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " eladtad a " .. getVehicleName(theVehicle) .. "-t.", targetPlayer, 0, 255, 0)
										else
											outputChatBox("Error 09001 - Jelentsd a fórumon.", thePlayer, 255, 0, 0)
										end
									else
										outputChatBox(targetPlayerName .. " -nek/nak túl sok járműve van", thePlayer, 255, 0, 0)
										outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " Megpróbálod eladni az autót, de már túl sok van neki. ", targetPlayer, 255, 0, 0)
									end
								else
									outputChatBox(targetPlayerName .. " nincs elég hely a kulcsoknak", thePlayer, 255, 0, 0)
									outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " megpróbálod eladni a járműved, de nincs helye a kulcsoknak.", targetPlayer, 255, 0, 0)
								end
							else
								outputChatBox("Hogy adnád már el magadnak?! -.-'.", thePlayer, 255, 0, 0)
							end
						else
							outputChatBox("Ez a jármű nem a tiéd.", thePlayer, 255, 0, 0)
						end
					else
						outputChatBox("A járműben kell lenned.", thePlayer, 255, 0, 0)
					end
				else
			...
			if not locklessVehicle[model] or exports.global:hasItem( source, 3, dbid ) then
				if (getElementData(source, "realinvehicle") == 1) then
					...
					if seat == 0 or exports.global:hasItem( source, 3, dbid ) then
						if (locked) then
							setVehicleLocked(vehicle, false)
							exports.global:sendLocalMeAction(source, "kinyitotta a jármű ajtaját.")
						else
							setVehicleLocked(vehicle, true)
							exports.global:sendLocalMeAction(source, "bezárta a a jármű ajtaját.")
						end
					end
				end
			end
		else
			outputChatBox("(( Nem zárhatsz le lefoglalt járműveket. ))", source, 255, 195, 14)
		end
	else
		outputChatBox("(( Civil jármûveket nem tudsz lezárni. ))", source, 255, 195, 14)
	end
...
function lockUnlockOutside(vehicle)
	if (not source or exports.global:isPlayerAdmin(source)) or ( getElementData(vehicle, "Impounded") or 0 ) == 0 then
		...
		if (isVehicleLocked(vehicle)) then
			setVehicleLocked(vehicle, false)
			exports.global:sendLocalMeAction(source, "kinyitotta a járművét. ((" .. getVehicleName(vehicle) .. "))")
			...
			setVehicleLocked(vehicle, true)
			exports.global:sendLocalMeAction(source, "lezárta a járművét. ((" .. getVehicleName(vehicle) .. "))")
...
function fillFuelTank(veh, fuel)
	...
	if (math.ceil(currFuel)==100) then
		outputChatBox("Ez a jármű már teli van.", source)
	elseif (fuel==0) then
		outputChatBox("A kannád üres.", source, 255, 0, 0)
	elseif (engine==1) then
		outputChatBox("Nem tudod megtankolni a járművet, járó motorral. Állítsd le a motort!", source, 255, 0, 0)
	else
		...
		outputChatBox("Megtankoltál " .. math.ceil(fuelAdded) .. " liter benzint a benzines kannádból.", source, 0, 255, 0 )
		
		exports.global:sendLocalMeAction(source, "beleönti a benzint a kannából a járműbe")
...
function removeNOS(theVehicle)
	...
	exports.global:sendLocalMeAction(source, "kivette a NITROT-t a " .. getVehicleName(theVehicle) .. ".")
end
...
function setVehiclePosition(thePlayer, commandName)
	...
	if not veh or getElementData(thePlayer, "realinvehicle") == 0 then
		outputChatBox("Nem vagy a járműben.", thePlayer, 255, 0, 0)
	else
		if call( getResourceFromName("tow-system"), "cannotVehpos", thePlayer ) then
			outputChatBox("A Los Santos Parkolás felügyelet tagjai parkolhatnak csak a lefoglalt kocsik telephelyén!.", thePlayer, 255, 0, 0)
		elseif isElementWithinColShape( thePlayer, HospitalCol ) and getElementData( thePlayer, "faction" ) ~= 2 and not exports.global:isPlayerAdmin(thePlayer) then
			outputChatBox("A Los Santos Mentőszolgálat tagjainak van joga a kórház elé parkolni.", thePlayer, 255, 0, 0)
		elseif isElementWithinColShape( thePlayer, PershingSquareCol ) and getElementData( thePlayer, "faction" ) ~= 1  and not exports.global:isPlayerAdmin(thePlayer) then
			outputChatBox("A Los Santos Police Department tagjainak van joga a Pershing Squaren parkolni.", thePlayer, 255, 0, 0)
		else
			...
			if (owner==playerid and TowingReturn == 0) or (exports.global:hasItem(thePlayer, 3, dbid)) or (TowingReturn == 2) or (exports.global:isPlayerAdmin(thePlayer) and exports.logs:logMessage("[AVEHPOS] " .. getPlayerName( thePlayer ) .. " parked car #" .. dbid .. " at " .. x .. ", " .. y .. ", " .. z, 9)) then
				if (dbid<0) then
					outputChatBox("Ez nem egy állandó kocsi.", thePlayer, 255, 0, 0)
				else
					...
					outputChatBox("Jármű spawn pozíciója megváltoztatva.", thePlayer)
					...
					if ( getElementData(veh, "Impounded") or 0 ) > 0 then
						...
						if isElement( owner ) and exports.global:hasItem( owner, 2 ) then
							outputChatBox("(( Los Santos Parkolási felügyelet )) #999 [SMS]: Az Ön " .. getVehicleName(veh) .. " jráműve le lett foglalva. Telephelyünkön átveheti!", owner, 120, 255, 80)
						end
					end
				end
			end
...
function setVehiclePosition2(thePlayer, commandName, vehicleID)
	if exports.global:isPlayerAdmin( thePlayer ) then
		...
		if not vehicleID or vehicleID < 0 then
			outputChatBox( "HASZNÁLATA: /" .. commandName .. " [vehicle id]", thePlayer, 255, 194, 14 )
		else
			...
			if veh then
				...
				exports['anticheat-system']:changeProtectedElementDataEx(veh, "interior", interior)
				exports['anticheat-system']:changeProtectedElementDataEx(veh, "dimension", dimension)
				outputChatBox("A jármű spawn pozíciója megváltotatva ennek a járműnek: #" .. vehicleID .. ".", thePlayer)
				...
				if ( getElementData(veh, "Impounded") or 0 ) > 0 then
					local owner = getPlayerFromName( exports['cache']:getCharacterName( getElementData( veh, "owner" ) ) )
					if isElement( owner ) and exports.global:hasItem( owner, 2 ) then
						outputChatBox("((Los Santos Parkolási felügyelet)) #999 [SMS]: Az Ön " .. getVehicleName(veh) .. " járműve le lett foglalva. Telephelyünkön átveheti!", owner, 120, 255, 80)
					end
				end
				exports.logs:logMessage("[AVEHPOS] " .. getPlayerName( thePlayer ) .. " parked car #" .. vehicleID .. " at " .. x .. ", " .. y .. ", " .. z, 9)
			else
				outputChatBox("Jármű nem található.", thePlayer, 255, 0, 0 )
			end
		end
	end
end
...
function setVehiclePosition3(veh)
	if call( getResourceFromName("tow-system"), "cannotVehpos", source ) then
		outputChatBox("A Los Santos Parkolás felügyelet tagjai parkolhatnak csak a lefoglalt kocsik telephelyén!", source, 255, 0, 0)
	elseif isElementWithinColShape( source, HospitalCol ) and getElementData( source, "faction" ) ~= 2 and not exports.global:isPlayerAdmin(source) then
		outputChatBox("A Los Santos Mentőszolgálat tagjainak van joga a kórház elé parkolni.", source, 255, 0, 0)
	elseif isElementWithinColShape( source, PershingSquareCol ) and getElementData( source, "faction" ) ~= 1  and not exports.global:isPlayerAdmin(source) then
		outputChatBox("A Los Santos Police Department tagjainak van joga a Pershing Squaren parkolni.", source, 255, 0, 0)
	else
		...
		if (owner==playerid and TowingReturn == 0) or (exports.global:hasItem(source, 3, dbid)) or (TowingReturn == 2) or (exports.global:isPlayerAdmin(source) and exports.logs:logMessage("[AVEHPOS] " .. getPlayerName( source ) .. " parked car #" .. dbid .. " at " .. x .. ", " .. y .. ", " .. z, 9)) then
			if (dbid<0) then
				outputChatBox("Ez nem állandó kocsi.", source, 255, 0, 0)
			else
				...
				exports['anticheat-system']:changeProtectedElementDataEx(veh, "interior", interior)
				exports['anticheat-system']:changeProtectedElementDataEx(veh, "dimension", dimension)
				outputChatBox("Jármű spawn pozíciója megváltoztatva.", source)
			...
			outputChatBox( "Nem tudod leparkolni ezt a járművet.", source, 255, 0, 0 )
		end
	end
end
...
function setVehiclePosition4(thePlayer, commandName)
	...
	if not veh or getElementData(thePlayer, "realinvehicle") == 0 then
		...
		if (playerfl == 1) and (playerfid==carfid) then
			...
			exports['anticheat-system']:changeProtectedElementDataEx(veh, "interior", interior)
			exports['anticheat-system']:changeProtectedElementDataEx(veh, "dimension", dimension)
			outputChatBox("A jármű spawn pozíciója megváltotatva ennek a járműnek: #" .. dbid .. ".", thePlayer)
			...
			if ( getElementData(veh, "Impounded") or 0 ) > 0 then
				local owner = getPlayerFromName( exports['cache']:getCharacterName( getElementData( veh, "owner" ) ) )
				if isElement( owner ) and exports.global:hasItem( owner, 2 ) then
					outputChatBox("((Los Santos Parkolási felügyelet)) #999 [SMS]: Az Ön " .. getVehicleName(veh) .. " járműve le lett foglalva. Telephelyünkön átvheti!", owner, 120, 255, 80)
				end
			end
		end
	end
...
function detachVehicle(thePlayer)
	if isPedInVehicle(thePlayer) and getPedOccupiedVehicleSeat(thePlayer) == 0 then
		...
		if getVehicleTowedByVehicle(veh) then
			...
			outputChatBox("A vontatód levált.", thePlayer, 0, 255, 0)
		else
			outputChatBox("Ez nem vontató...", thePlayer, 255, 0, 0)
		end
	end
end
