...
function showGeneralshopUI(shop_type, race, gender, discount)
	...
	if (wGeneralshop==nil) then
		...
		if not shop or #shop == 0 then
			outputChatBox("Ez nem egy bolt. Menj innen.")
			return
		end
		...
		local lInstruction = guiCreateLabel( 0, 20, 500, 15, "Kattints rá kétszer arra amit meg akarsz venni.Árak Forintban értendőek és az Áfát tartalmazzák", false, wGeneralshop )
		...
		for _, category in ipairs( shop ) do
			...
			local cName = guiGridListAddColumn( grid, "Név", 0.25 )
			local cPrice = guiGridListAddColumn( grid, "Ár", 0.1 )
			local cDescription = guiGridListAddColumn( grid, "Leírás", 0.62 )
		...
		bClose = guiCreateButton(200, 315, 100, 25, "Bezárás", false, wGeneralshop)
