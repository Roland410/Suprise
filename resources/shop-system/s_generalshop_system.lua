...
local skins = { { 211, 217 }, { 179 }, false, { 178 }, { 82 }, { 80, 81 }, { 28, 29 }, { 169 }, { 171, 172 }, { 142 }, { 171 }, { 171, 172 }, {71}, { 50 }, {71}, {71} }
	...
	if (exports.global:isPlayerAdmin(thePlayer)) then
		...
		if shoptype and g_shops[shoptype] then
			...
			if skin then
				...
				if not ped then
					outputChatBox("Érvénytelen Skin.", thePlayer, 255, 0, 0)
					return
				else
					destroyElement(ped)
				end
...
addEventHandler( "shop:buy", resourceRoot, function( index )
	...
	if item.itemID == 48 and exports.global:hasItem( client, 48 ) then
		outputChatBox( "Neked már van " .. item.name .. ".", client, 255, 0, 0 )
		return
	end
	...
	if not exports.global:hasMoney( client, price ) then
		outputChatBox( "Nincs elég pénzed, hogy megvedd " .. item.name .. ".", client, 255, 0, 0 )
		return
	end
	...
	if exports.global:giveItem( client, itemID, itemValue ) then
		...
		outputChatBox( "Vásároltál egy " .. item.name .. " " .. exports.global:formatMoney( price ) .. "Ft-ért.", client, 0, 255, 0 )
...
--end
		...
		outputChatBox( "Ezt a tárgyat (" .. item.name .. ") nem bírod már el. ((nincs elég hely az inventorydban))", client, 255, 0, 0 )
	end
end )
