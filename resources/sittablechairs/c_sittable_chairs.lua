...
function useChair(chair)
	...
	outputChatBox("Leültél egy székre. Nyomd meg az M majd magadra nyomj, hogy felállj.", 255, 195, 14)
end

function attemptToSitOnChair(chair)
	if ( canISitOnChair(chair) ) then
		if ( chairPerson[chair] ~= nil ) then
			outputChatBox("Ez a hely már foglalt!", 255, 0, 0)
		else
			useChair(chair)
		end
	else
		outputChatBox("Túl messz vagy a széktől menj közelebb!", 255, 0, 0)
	end
end
