...
function createLookWindow( name, description, attributes, editFunction )
	...
	local column = guiGridListAddColumn( grid, "Objectek a playernél", 0.95)
	...
	if editFunction then
		...
		local edit = guiCreateButton( x, 295, 130, 25, "Szerkeszt", false, advlook_window )
		addEventHandler( "onClientGUIClick", edit, editFunction, false )
		
		x = x + 140
	end
	local close = guiCreateButton( x, 295, 130, 25, "Bezár", false, advlook_window )
...
addEventHandler( "social:look", getRootElement( ),
	function( age, race, gender, weight, height, description )
		local editButton = createLookWindow( getPlayerName( source ):gsub( "_", " " ),
			getPlayerName( source ):sub( 1, getPlayerName( source ):find( "_" ) ) .. " is a " ..
			age .. " éves " .. ( race == 0 and "Fekete" or race == 1 and "Fehér" or "Ázsiai" ) .. " " .. ( gender == 1 and "Female" or "Male" ) ..
			" Súlya " .. weight .. "kg" ..
			" Magasság " .. height .. "cm.\n" ..
			"Hajszín: " .. description[1] .. "\n" ..
			"Hajstílus: " .. description[2] .. "\n" ..
			"arcvonásai: " .. description[3] .. "\n" ..
			"fizikai tulajdonságok: " .. description[4] .. "\n" ..
			"Leírás: " .. description[5] .. "\n" ..
			"Kiegészítők: " .. description[6],
			description[7] or {},
...
function createEditablesWindow( )
	...
	advlook_edit_window = guiCreateWindow( 804, 172, 181, 297, "Szerkesztés!", false )
	...
	local column = guiGridListAddColumn( grid, "Beállitás", 0.9 )
	...
	local close = guiCreateButton( 12, 266, 157, 22, "Bezár", false, advlook_edit_window )
...
function createEditValueWindow( key )
	...
	advlook_editvalue_window = guiCreateWindow( 349, 300, 285, 105, "Szerkesztés - " .. stuff.name, false )
	guiWindowSetSizable( advlook_editvalue_window, false )
	
	guiCreateLabel(11, 26, 258, 16, "Te most módositottad a " .. stuff.name .. ".", false, advlook_editvalue_window)
	
	local edit = guiCreateEdit(12, 46, 250, 22, tostring( stuff.value ),false,advlook_editvalue_window)
	
	
	local save = guiCreateButton(15,72,123,21,"Mentés",false,advlook_editvalue_window)
	...
	local cancel = guiCreateButton(142,72,123,21,"Mégsem",false,advlook_editvalue_window)
