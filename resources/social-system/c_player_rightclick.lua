...
function showPlayerMenu(targetPlayer, friend)
	...
	if not friend then
		bAddAsFriend = guiCreateButton(0.05, 0.13, 0.87, 0.1, "Felvétel barátnak", true, wRightClick)
		addEventHandler("onClientGUIClick", bAddAsFriend, caddFriend, false)
	else
		bAddAsFriend = guiCreateButton(0.05, 0.13, 0.87, 0.1, "Törlés mint barát", true, wRightClick)
		addEventHandler("onClientGUIClick", bAddAsFriend, cremoveFriend, false)
	end
	
	-- FRISK
	bFrisk = guiCreateButton(0.52, 0.25, 0.43, 0.1, "Motozás", true, wRightClick)
	...
	if cuffed == 0 then
		bRestrain = guiCreateButton(0.05, 0.25, 0.43, 0.1, "Fékentartás", true, wRightClick)
		addEventHandler("onClientGUIClick", bRestrain, crestrainPlayer, false)
	else
		bRestrain = guiCreateButton(0.05, 0.25, 0.43, 0.1, "Elengedés", true, wRightClick)
	...
	if (blindfold) and (blindfold == 1) then
		bBlindfold = guiCreateButton(0.05, 0.51, 0.87, 0.1, "Szembekötő levétele", true, wRightClick)
		addEventHandler("onClientGUIClick", bBlindfold, cremoveBlindfold, false)
	else
		bBlindfold = guiCreateButton(0.05, 0.51, 0.87, 0.1, "Szembekötő", true, wRightClick)
	...
	if exports.global:hasItem(getLocalPlayer(), 70) and getElementData(player, "injuriedanimation") then
		bStabilize = guiCreateButton(0.05, y, 0.87, 0.1, "Stabilizál", true, wRightClick)
	...
	if stretcherElement then
		...
		if stretcherPlayer and stretcherPlayer == player then
			bStabilize = guiCreateButton(0.05, y, 0.87, 0.1, "Feszitőkötél elvétele", true, wRightClick)
			addEventHandler("onClientGUIClick", bStabilize, fTakeFromStretcher, false)
			y = y + 0.13
		end
		
		if not stretcherPlayer then
			bStabilize = guiCreateButton(0.05, y, 0.87, 0.1, "Feszitőkötél", true, wRightClick)
	...
	bCloseMenu = guiCreateButton(0.05, y, 0.87, 0.1, "Menü bezár", true, wRightClick)
	addEventHandler("onClientGUIClick", bCloseMenu, hidePlayerMenu, false)
	sent = false
	
	bInformation = guiCreateButton(0.05, 0.38, 0.87, 0.1, "Információ", true, wRightClick)
	...
	if button == "left" and state == "up" then
		if (exports.global:hasItem(getLocalPlayer(), 70)) then -- Has First Aid Kit?
			...
			if not knockedout then
				outputChatBox("Nincs kiütve.", 255, 0, 0)
				hidePlayerMenu()
			else
			...
			outputChatBox("Nincs elsõsegély csomagod.", 255, 0, 0)
		end
	end
end
	...
	if (button=="left") then
		if (exports.global:hasItem(getLocalPlayer(), 66)) then -- Has blindfold?
			...
			if (blindfolded==1) then
				outputChatBox("Ennek a játékosnak a szeme már be van kötve.", 255, 0, 0)
				hidePlayerMenu()
			elseif (restrained==0) then
				outputChatBox("Elõbb féken kell tartanod.", 255, 0, 0)
				hidePlayerMenu()
			else
			...
			outputChatBox("Nincs mivel fékentartanod.", 255, 0, 0)
		end
	end
end
...
function cremoveBlindfold(button, state, x, y)
	if (button=="left") then
		...
		if (blindfolded==1) then
			...
			outputChatBox("A játékos szeme nincs bekötve.", 255, 0, 0)
			hidePlayerMenu()
		end
	end
end
	...
	if (button=="left") then
		if (exports.global:hasItem(getLocalPlayer(), 45) or exports.global:hasItem(getLocalPlayer(), 46)) then
			...
			if (restrained==1) then
				outputChatBox("Ez a játékos már nem tud mozogni.", 255, 0, 0)
				hidePlayerMenu()
			else
				local restrainedObj
			...
			outputChatBox("Nem tudod mivel meggátolni a mozgásban.", 255, 0, 0)
			hidePlayerMenu()
		end
	end
end
...
function cunrestrainPlayer(button, state, x, y)
	if (button=="left") then
		...
		if (restrained==0) then
			outputChatBox("Játékos nincs fékentartva.", 255, 0, 0)
			hidePlayerMenu()
		else
			...
			if (exports.global:hasItem(getLocalPlayer(), 47, dbid)) or (restrainedObj==46) then -- has the keys, or its a rope
				...
				outputChatBox("Nincs kulcsod ehhez a bilincshez.", 255, 0, 0)
			end
		end
	end
end
...
function cfriskPlayer(button, state, x, y)
	if (button=="left") then
		...
		if restrained ~= 1 and not injured then
			outputChatBox("Játékos nincs fékentartva vagy animban.", 255, 0, 0)
			hidePlayerMenu()
		elseif getElementHealth(getLocalPlayer()) < 50 then
			outputChatBox("Játékos nincs eléggé megsérülve ahhoz hogy bármit is tudj csinálni vele.", 255, 0, 0)
			hidePlayerMenu()
		else
			gx = x
			gy = y
...
function friskShowItems(items)
	...
	triggerServerEvent("sendLocalMeAction", getLocalPlayer(), getLocalPlayer(), " Megmotozza " .. playerName .. ".")
	local width, height = 300, 200
	
	wFriskItems = guiCreateWindow(gx, gy, width, height, "Motoz: " .. playerName, false)
	guiSetText(wFriskItems, "Motozás: " .. playerName)
	guiWindowSetSizable(wFriskItems, false)
	
	gFriskItems = guiCreateGridList(0.05, 0.1, 0.9, 0.7, true, wFriskItems)
	FriskColName = guiGridListAddColumn(gFriskItems, "Név", 0.9)
	...
	bFriskClose = guiCreateButton(0.05, 0.85, 0.9, 0.1, "Bezár", true, wFriskItems)
