...
addEventHandler( "social:remove", root,
	function( otherAccount )
		...
		if accountID and friends[ accountID ] and friends[ otherAccount ] then
			...
			if not found then
				outputChatBox( "Nincs barátod.", client, 255, 0, 0 )
			else
				...
				outputChatBox( "Törölted a  " .. friends[ otherAccount ].name .. " már nincs a listádon.", client, 0, 255, 0 )
...
function new_addFriend( from, to )
	...
	if not fromID or not toID then
		outputChatBox( "Nem tudsz bejelentkezni?", from, 255, 0, 0 )
		return
	end
	...
	for k, v in ipairs({fromID, toID}) do
		if not friends[ v ] then
			friends[ v ] = { }
			outputDebugString( "Rögzített barátok A listán" .. tostring( v ))
		end
		
		if not friends[ v ].name then
			loadAccountData( v )
			outputDebugString( "Rögzített barátok B listán" .. tostring( v ))
		end
		
		-- make sure a palyer's friends are loaded
		if not friends[ v ].loadedFriends then
			loadFriends( v )
			outputDebugString( "Rögzített barátok C listán" .. tostring( v ))
		end
	end
	...
	if fromID and friends[ fromID ] and toID and friends[ toID ] then
		...
		if onFromList then
			outputChatBox( getPlayerName( to ):gsub("_", " ") .. " Barátaid " .. friends[ toID ].name .. ".", from, 255, 194, 14 )
		else
			...
			if onToList then -- the OTHER player has him on his friends list. shouldn't happen, but oh well.
				...
				outputChatBox( getPlayerName( to ):gsub("_", " ") .. " Sikeresen hozzáadtad a barátaid közé " .. friends[ toID ].name .. ".", from, 0, 255, 0 )
		...
		outputChatBox( "Kritikus hiba.", from, 255, 0, 0 )
	end
end
...
addEventHandler( "social:acceptFriend", root,
	function( )
		...
		if not from or not to then
			...
			sendFriends( from, fromID )
			sendFriends( to, toID )
			outputChatBox( getPlayerName( to ):gsub("_", " ") .. " Sikeresen felvetted barátnak " .. friends[ toID ].name .. ".", from, 0, 255, 0 )
			outputChatBox( getPlayerName( from ):gsub("_", " ") .. " Sikeresen felvett barátnak " .. friends[ fromID ].name .. ".", to, 0, 255, 0 )
		end
	end
)
