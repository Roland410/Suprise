		...
		local rotz = getPedRotation(sourcePlayer)
			...
			for value=1,SpikeLimit,1 do
				if(Spike[value] == nil) then
					...
					outputChatBox("Szögesdrót lerakva ID:" .. value, sourcePlayer, 0, 194, 0)
					exports['roadblock-system']:dbLog(sourcePlayer, 28, sourcePlayer, "Spawned spike ID "..tostring(value))
					break
				end
			end
		else
			outputChatBox("Túl sok szögesdrótot tettél már le elöbb vedd fel amit eddig leraktál  ", sourcePlayer, 255, 194, 14)
		end
	end
end
addCommandHandler("szögesdrot", ThrowSpikes)
		...
		outputChatBox("SYNTAX: /removespikes [ID]", sourcePlayer, 255, 194, 14)
			...
			if(Spike[message] ~= nil) then
				...
				if (getDistanceBetweenPoints3D(x1, y1, z1, x2, y2, z2) <= 10) then
					outputChatBox("szögesdrót felvéve ID:" .. message, sourcePlayer, 0, 194, 0)
					...
					outputChatBox("Túl messze vagy a szögesdrót-tól!", sourcePlayer, 255, 194, 14)
				end
			else
				outputChatBox("Hibás Szögesdrót id!", sourcePlayer, 255, 194, 14)
			end
		end
	end
end
addCommandHandler("szögesdrotfel", RemovingSpikes)
...
function AdminRemovingSpikes(sourcePlayer, command)
	if	(exports.global:isPlayerAdmin(sourcePlayer)) then
		...
		outputChatBox("Összes szögesdrót felvetted.", sourcePlayer, 0, 194, 0)
		exports['roadblock-system']:dbLog(sourcePlayer, 28, sourcePlayer, "Removed all the spawned spikestrips")
		TotalSpikes = nil
	end
end
addCommandHandler("aszögesdrot", AdminRemovingSpikes)