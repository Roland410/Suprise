...
function startSystem()
	for key, group in ipairs(tollPorts) do
		...
		for dataKey, dataGroup in ipairs (group.data) do
			...
			exports['anticheat-system']:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "languages.lang1" , 16, false)
...
function startTalkToPed ()
	...
	if (isPedInVehicle(thePlayer)) then
		...
		if (exports['vehicle-system']:isVehicleWindowUp(theVehicle)) then
			outputChatBox("Először húzd le az ablakot, így nem fogják az ajtón kívül hallani.", thePlayer, 255,0,0)
			return
		end
	end
	...
	if (isGateBusy) then
		processMessage(thePed, "Hmmm..")
		return
	end

	processMessage(thePed, "Az útadó 1500 forint, szeretne átmenni?.")
	setConvoState(thePlayer, 1)
	local responseArray = { "Igen kérem.", "Nem köszönöm." }
...
function processOpenTolls(tollKey, thePed, thePlayer, payByBank)
	if not tollElementsLocked[tollKey] or exports.global:hasItem(thePlayer, 64) or exports.global:hasItem(thePlayer, 65) or exports.global:hasItem(thePlayer, 112) then
		if payByBank then
			...
			local money = getElementData(thePlayer, "bankmoney") - 1200
			if money >= 0 then
			
				exports['anticheat-system']:changeProtectedElementDataEx(thePlayer, "bankmoney", money, false)
				exports.mysql:query_free("UPDATE characters SET bankmoney=bankmoney-1200 WHERE id=" .. exports.mysql:escape_string(getElementData( thePlayer, "dbid" )))
				outputChatBox("[Útadó] A bankszámládrol le lett vonva 1200 forint.", thePlayer, 0, 255, 0)
			else
				return "Nincs elég pénzed a bankszámládon."
			end
		else
			if not exports.global:takeMoney(thePlayer, 1500) then
				return "Sajnálom, de nem engedhetem át ha nincsen elég pénze."
			end
		end

		triggerGate(thePed, false, thePlayer)
		exports.global:giveMoney( getTeamFromName("Állam"), 3 )
		
		exports.global:giveMoney( getTeamFromName("Rendőrség"), 3 )

		return "Köszönöm, menjen."
	else
		return "Sajnálom, de azt a parancsot kaptuk hogy senkit se engedjünk át."
	end
end
...
function talkToPed(answer, answerStr)
	...
	if (convState == 1) then --  "Hey, want to pass? That'll be six dollar please."
		...
		if (languageSkill < 60) or (currLang ~= 16) then
			processMessage(thePed, "Hé haver, nem értem ezt a szart amit itt beszélsz.")
...
function testToggle(thePlayer, commandName, gateID, silent)
	...
	if not gateID or not tonumber(gateID) then
		outputChatBox("Használat: /"..commandName.." [ID]", thePlayer)
		--return ]]
		for tollID, tollName in ipairs(tollElementsName) do
			...
			outputChatBox(" "..tostring(tollID).. " - "..tollName .." - Állapot: "..state, thePlayer)
		end
		return
	end
	...
	if not tollElementsName[gateID] then
		outputChatBox("Nem létezik.", thePlayer)
	end	
	
	tollElementsLocked[gateID] = not tollElementsLocked[gateID]
	if tollElementsLocked[gateID] then
		local first = true
		exports['chat-system']:departmentradio(thePlayer, "d", "Kérlek zárd le a "..tollElementsName[gateID].." határkaput.")
...
function processRadio(thePed, message, source)
	...
	exports['global']:sendLocalText(source, "[Magyar] " .. name .. "'s Rádió: "..message, 255, 255, 255)
end
...
function tollCommand(thePlayer)
	...
	if duty > 0 and getElementDimension( thePlayer ) == 0 then
		...
		for key, value in ipairs( tollElements ) do
			...
			if name then
				outputChatBox( name .. " kinyitása.", thePlayer, 0, 255, 0 )
				for k, v in ipairs(value) do
					if v.ped then
		...
		if not any then
			outputChatBox("Nem vagy semelyik határkapunál.", thePlayer, 255, 0, 0)
		end
	end
end
