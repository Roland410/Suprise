...
function showImpoundUI(vehElementsret)
	if not wImpound then
		...
		wImpound = guiCreateWindow(x, y, width, height, "Lefoglalás feloldása", false)
		guiWindowSetSizable(wImpound, false)
		
		bClose = guiCreateButton(0.6, 0.85, 0.2, 0.1, "Bezárás", true, wImpound)
		bRelease = guiCreateButton(0.825, 0.85, 0.2, 0.1, "Feloldás", true, wImpound)
		...
		local col = guiGridListAddColumn(gCars, "Lefoglalt járművek", 0.7)
		...
		lCost = guiCreateLabel(0.3, 0.85, 0.2, 0.1, "Ára: 250 Ft", true, wImpound)
		...
		outputChatBox("Üdvözöljük a jármű telepen.")
	end
end
