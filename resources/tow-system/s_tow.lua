			...
			if (owner > 0) then
				if (faction > 3 or faction < 0) then
					if (source == towSphere2) then
						...
						outputChatBox("((A járművet parkold le:  /park és húzd be a kéziféket: /kezifek.))", thePlayer, 255, 194, 14)
					else
						if (getElementData(temp, "faction") ~= 30) then
							if (impounded == 0) then
								...
								exports.global:giveItem(temp, 72, "Lefoglalási megjegyzés: Lefoglalva ".. factionRank .." '".. getPlayerName(thePlayer) .."' által "..datestr.."-kor")
								outputChatBox("((A járművet parkold le:  /park és húzd be a kéziféket: /kezifek.))", thePlayer, 255, 194, 14)
							end
						end
					end
				else
					outputChatBox("Ez a frakció jármű nem lefoglalható.", thePlayer, 255, 194, 14)
				end
			end
		end
	end
...
function payRelease(vehID)
	if exports.global:takeMoney(source, 225) then
		exports.global:giveMoney(getTeamFromName("Parkolási felügyelet"), 225)
		...
		outputChatBox("A járműved fel lett oldva. (( Kérlek használd a /park parancsot hogy ne spawnoljon vissza a jármű ))", source, 255, 194, 14)
	else
		outputChatBox("Nincs nálad elég pénz.", source, 255, 0, 0)
	end
end
...
function unimpoundVeh(thePlayer, commandName, vehid)
	if thePlayer then
		if not vehid then
			outputChatBox("Használat: /" .. commandName .. " [Vehicle ID]", thePlayer, 255, 194, 14)
		else
				...
				if getElementData(vehID, "Impounded") and getElementData(vehID, "Impounded") >= 1 then
					if commandName == "unimpound" then
						if (getElementData(thePlayer,"faction") == 30) then
							if(getElementData(thePlayer, "factionleader") == 1) then
								...
								outputChatBox("Feloldottál egy járművet: #" .. vehid .. ".", thePlayer, 0, 255, 0)
								...
								outputChatBox("Frakció leadernek kell lenned.", thePlayer, 255, 0, 0)
							end
						end
					elseif commandName == "aunimpound" then
						if exports.global:isPlayerSuperAdmin(thePlayer) then
							...
							outputChatBox("Feloldottál egy járművet:  #" .. vehid .. ".", thePlayer, 0, 255, 0)
							exports.global:sendMessageToAdmins("AdmCmd: " .. adminTitle .. " " .. playerName .. " feloldott egy járművet: #" .. vehid .. ".")
