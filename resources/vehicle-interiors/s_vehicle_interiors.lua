	...
	function( vehicle )
		if vehicles[ vehicle ] then
			if isVehicleLocked( vehicle ) then
				outputChatBox( "Megpróbáltad kinyitni az ajtót, de úgy tűnik be van zárva.", source, 255, 0, 0 )
			else
	...
	for value in pairs( vehicles ) do
		if getElementData( value, "dbid" ) == dim then
			if isVehicleLocked( value ) then
				outputChatBox( "Megpróbáltad kinyitni az ajtót, de úgy tűnik be van zárva.", player, 255, 0, 0 )
			else
		...
		if (tpd > 20000) then
			...
			for key, value in ipairs( getElementsByType( "vehicle" ) ) do
				if getElementData( value, "dbid" ) == vid then
					exports.global:sendLocalText(player, " *" .. getPlayerName(player):gsub("_"," ") .. " a jobb kezét felemelve elkezdett kopogni a jármű ablakán.", 255, 51, 102)
					exports.global:sendLocalText(value, " * A jármű belsejéből kopogás hallatszik. *      ((" .. getPlayerName(player):gsub("_"," ") .. "))", 255, 51, 102)
				end
			end
		else
			if vehicles[veh] then
				...
				if (exit) then
					exports.global:sendLocalText(player, " *" .. getPlayerName(player):gsub("_"," ") .. " a jobb kezét felemelve elkezdett kopogni a jármű ablakán.", 255, 51, 102)
					exports.global:sendLocalText(exit, " * A jármű belsejéből kopogás hallatszik. *      ((" .. getPlayerName(player):gsub("_"," ") .. "))", 255, 51, 102)
				end
			end
		end
	end
