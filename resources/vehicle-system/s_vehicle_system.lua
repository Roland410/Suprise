...
-- /makeveh2
function createPermVehicle2(thePlayer, commandName, ...)
	if (exports.global:isPlayerSuperAdmin(thePlayer)) or (exports.donators:hasPlayerPerk(thePlayer, 16) and exports.global:isPlayerFullAdmin(thePlayer)) then
		local args = {...}
		if (#args < 7) then
			outputChatBox("SYNTAX: /" .. commandName .. " [id/name] [color1] [color2] [Owner] [Faction Vehicle (1/0)] [Cost] [Tinted Windows] ", thePlayer, 255, 194, 14)
			...
			local targetPlayer, username = exports.global:findPlayerByPartialNick(thePlayer, userName)
				...
				if not (veh) then
					...
					local interior = getElementInterior(thePlayer)
					local var1, var2 = exports['vehicle-system']:getRandomVariant(id)
					local insertid = mysql:query_insert_free("INSERT INTO vehicles SET model='" .. mysql:escape_string(id) .. "', x='" .. mysql:escape_string(x) .. "', y='" .. mysql:escape_string(y) .. "', z='" .. mysql:escape_string(z) .. "', rotx='0', roty='0', rotz='" .. mysql:escape_string(r) .. "', color1='[ [ 0, 0, 0 ] ]', color2='[ [ 0, 0, 0 ] ]', color3='[ [ 0, 0, 0 ] ] ', color4='[ [ 0, 0, 0 ] ]', faction='-1', owner='GOV', plate='" .. mysql:escape_string(plate) .. "', currx='" .. mysql:escape_string(x) .. "', curry='" .. mysql:escape_string(y) .. "', currz='" .. mysql:escape_string(z) .. "', currrx='0', currry='0', currrz='" .. mysql:escape_string(r) .. "', locked=1, interior='" .. mysql:escape_string(interior) .. "', currinterior='" .. mysql:escape_string(interior) .. "', dimension='" .. mysql:escape_string(dimension) .. "', currdimension='" .. mysql:escape_string(dimension) .. "', tintedwindows='" .. mysql:escape_string(tint) .. "',variant1="..var1..",variant2="..var2)
...
addCommandHandler("makeveh2", createPermVehicle2, false, false)
...
function loadOneVehicle(id, hasCoroutine)
	...
	if row then
		...
		if veh then
			...
			setVehicleColor(veh, color1[1], color1[2], color1[3], color2[1], color2[2], color2[3])
			...
			if not (enginelessVehicle[model]) then
				...
				if engine == 0 then
					...
					if (vehKey) or (owner < 0) and (faction == -1) or (playerFaction == faction) and (faction ~= -1) or ((getElementData(source, "adminduty") or 0) == 1) then
						...
						if broke == 1 then
							exports.global:sendLocalMeAction(source, "megpróbálta elindítani a motort, de nem sikerült neki.")
							outputChatBox("A motor elromlott.", source)
						...
						elseif fuel < 1 then
							exports.global:sendLocalMeAction(source, "megpróbálta elindítani a motort, de nem sikerült neki.")
							outputChatBox("Nincs a járműben üzemanyag.", source)
						end
					else
						outputChatBox("Sajnálom nincs a járműhöz kulcsod,vagy nem a tied", source, 255, 0, 0)
					end
				else
...
function checkLock(thePlayer, seat, jacked)
	...
	if (locked) and not (jacked) then
		cancelEvent()
		outputChatBox("Az ajtó be van zárva.", thePlayer)
	end
end
...
function checkBikeLock(thePlayer)
	if (isVehicleLocked(source)) and (getVehicleType(source)=="Bike" or getVehicleType(source)=="Boat" or getVehicleType(source)=="BMX" or getVehicleType(source)=="Quad" or getElementModel(source)==568 or getElementModel(source)==571 or getElementModel(source)==572 or getElementModel(source)==424) then
		if not getElementData(thePlayer, "interiormarker") then
			outputChatBox("Ez a jármű sajnos zárva van!.", thePlayer, 255, 194, 15)
		end
		cancelEvent()
	end
end
...
function setRealInVehicle(thePlayer)
	if isVehicleLocked(source) then
		...
		if owner < 0 and faction == -1 then
			outputChatBox("(( Ez a jármű (" .. carName .. ") a város tulajdona. ))", thePlayer, 255, 195, 14)
		elseif (faction==-1) and (owner>0) then
			local ownerName = exports['cache']:getCharacterName(owner)
			
			if ownerName then
				outputChatBox("(( Ez a jármű (" .. carName .. ") " .. ownerName .. " tulajdona. ))", thePlayer, 255, 195, 14)
...
function removeFromFactionVehicle(thePlayer)
	...
	if (vfaction~=-1) then
		...
		if (faction~=vfaction) and (seat==0) then
			if (CanTowDriverEnter) then
				outputChatBox("(( Jármű " .. getVehicleName(source) .. " Tulajdonos '" .. factionName .. "'. ))", thePlayer, 255, 194, 14)
	...
	local vfac = tonumber(getElementData(source, "faction"))
	for key, value in ipairs(exports.pool:getPoolElementsByType("team")) do
			local id = tonumber(getElementData(value, "id"))
			if (id==vfac) then
				factionName = getTeamName(value)
	...
	local seat = getPedOccupiedVehicleSeat(thePlayer)
	if (vfac>0) and (seat==0) then
		if (fac~=vfac) then
			if (vfac==1) then
				outputChatBox("Nem vagy Rendőrség tagja", thePlayer, 255, 0, 0)
			elseif (vfac==2) then
				outputChatBox("Nem vagy Önkormányzat tagja", thePlayer, 255, 0, 0)
			elseif (vfac==3) then
				outputChatBox("Nem vagy Mentős ", thePlayer, 255, 0, 0)
			elseif (vfac==4) then
				outputChatBox("Nem vagy Nav-nak a tagja", thePlayer, 255, 0, 0)
			elseif (vfac==5) then
				outputChatBox("Nem vagy Szerelő", thePlayer, 255, 0, 0)
			elseif (vfac==6) then
				outputChatBox("Nem vagy Család(LCN) tag", thePlayer, 255, 0, 0)
			elseif (vfac==7) then
				outputChatBox("Nem vagy Család tag(Yakuza)", thePlayer, 255, 0, 0)
			elseif (vfac==8) then
				outputChatBox("Nem vagy Banda (Gsf) tag", thePlayer, 255, 0, 0)
			elseif (vfac==9) then
				outputChatBox("Nem vagy Banda (Piru Bloods) tag", thePlayer, 255, 0, 0)
			elseif (vfac==10) then
				outputChatBox("Nem vagy Banda (Surenos) tag", thePlayer, 255, 0, 0)
			elseif (vfac==11) then
				outputChatBox("Nem vagy Banda (Vagos) tag", thePlayer, 255, 0, 0)
			end
			exports['anticheat-system']:changeProtectedElementDataEx(thePlayer, "realinvehicle", 0, false)
			removePedFromVehicle(thePlayer)
			local x, y, z = getElementPosition(thePlayer)
	...
	if (vjob>0) and (seat==0) then
		if (job~=vjob) then
			if (vjob==1) then
				outputChatBox("Nem vagy Teherszállitó,menj a városházára és vedd fel a munkát", thePlayer, 255, 0, 0)
			elseif (vjob==2) then
				outputChatBox("Nem vagy Taxis,menj a városházára és vedd fel a munkát", thePlayer, 255, 0, 0)
			elseif (vjob==14) then
				outputChatBox("Nem vagy buszvezető,menj a városházára és vedd fel a munkát,majd a /tavolsagi parancsal tudsz dolgozni", thePlayer, 255, 0, 0)
			elseif (vjob==15) then
				outputChatBox("Nem vagy Farmer,menj a városházára és vedd fel a munkát, majd a /földmüvelés parancsal tudsz dolgozni", thePlayer, 255, 0, 0)
...
function sellVehicle(thePlayer, commandName, targetPlayerName)
	...
	if isPedInVehicle(thePlayer) then
		if not targetPlayerName then
			outputChatBox("SYNTAX: /" .. commandName .. " [játékos neve / id]", thePlayer, 255, 194, 14)
									...
									if exports.global:canPlayerBuyVehicle(targetPlayer) then
										if exports.global:isPlayerFullAdmin(thePlayer) or exports['carshop-system']:isForSale(theVehicle) then
											...
											if query then
												...
												outputChatBox("Használd a /park parancsot " .. getVehicleName(theVehicle) .. " leparkoláshoz.", targetPlayer, 255, 255, 0)
											...
											outputChatBox("Speciális autó szolj adminnak segitsen.", thePlayer, 255, 0, 0)
										end
									else
										outputChatBox(targetPlayerName .. " Kocsislotod tele van", thePlayer, 255, 0, 0)
										outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " Kocsiszoltod tele van.", targetPlayer, 255, 0, 0)
									end
								else
									outputChatBox(targetPlayerName .. " Nincs hely a kulcsoknak.", thePlayer, 255, 0, 0)
									outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " Nincs hely a kulcsoknak", targetPlayer, 255, 0, 0)
								end
							else
								outputChatBox("Túl messze van.", thePlayer, 255, 0, 0)
							end
						else
...
function lockUnlockInside(vehicle)
	...
	if (owner ~= -2) then
			...
			if not locklessVehicle[model] or exports.global:hasItem( source, 3, dbid ) then
				if (getElementData(source, "realinvehicle") == 1) then
					...
					if seat == 0 or exports.global:hasItem( source, 3, dbid ) then
						if (locked) then
							setVehicleLocked(vehicle, false)
							exports.global:sendLocalMeAction(source, "kinyitja belülről az ajtókat.")
							exports.logs:dbLog(source, 31, {  vehicle }, "UNLOCK FROM INSIDE")
						else
							setVehicleLocked(vehicle, true)
							exports.global:sendLocalMeAction(source, "bezárja belülről az ajtókat.")
		...
		outputChatBox("(( Civil járműveket nem zárhatsz be! ))", source, 255, 195, 14)
	end
...
function lockUnlockOutside(vehicle)
	if (not source or exports.global:isPlayerAdmin(source)) or ( getElementData(vehicle, "Impounded") or 0 ) == 0 then
		...
		if (isVehicleLocked(vehicle)) then
			...
			setVehicleLocked(vehicle, true)
			exports.global:sendLocalMeAction(source, "bezárta a járművét. ((" .. getVehicleName(vehicle) .. "))")
...
function fillFuelTank(veh, fuel)
	...
	if (math.ceil(currFuel)==100) then
		outputChatBox("A jármű már meg van tankolva.", source)
	elseif (fuel==0) then
		outputChatBox("Ez a benzinkanna üres.", source, 255, 0, 0)
	elseif (engine==1) then
		outputChatBox("Nem tankolhatsz járó motorral. Először állítsd le.", source, 255, 0, 0)
	else
...
function getYearDay(thePlayer)
	...
	outputChatBox("Ma az év " .. currYearday .. ". napja van.", thePlayer)
end
...
-- VEHPOS
local PershingSquareCol = createColRectangle( 1486, -1723, 130, 257 )
...
function setVehiclePosition(thePlayer, commandName)
	...
	if not veh or getElementData(thePlayer, "realinvehicle") == 0 then
		...
		elseif isElementWithinColShape( thePlayer, PershingSquareCol ) and getElementData( thePlayer, "faction" ) ~= 1  then
			outputChatBox("Ide nem parkolhatsz le.", thePlayer, 255, 0, 0)
		else
