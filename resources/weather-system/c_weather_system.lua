...
function createWeatherGUI(weather)
	...
	local X = 0.35
		...
		guiWeather = guiCreateWindow(X, Y, Width, Height, "Időjárás beállitás", true)

	if(weather == "felhős") then
		guiWeatherImage =   guiCreateStaticImage( 0.3 , 0.1, 0.4 , 0.4, "cloudy.png", true, guiWeather )
		guiWeatherNowLabel =  guiCreateLabel  ( 0.25 , 0.5, 0.5, 0.1, "Aktuális időjárás: felhős", true, guiWeather )
	elseif(weather == "világos") then
		guiWeatherImage =   guiCreateStaticImage( 0.3 , 0.1, 0.4 , 0.4, "clear.png", true, guiWeather )
		guiWeatherNowLabel =  guiCreateLabel  ( 0.25 , 0.5, 0.5, 0.1, "Aktuális időjárás: Tiszta", true, guiWeather )
	elseif(weather == "hot") then
		guiWeatherImage =   guiCreateStaticImage( 0.3 , 0.1, 0.4 , 0.4, "hot.png", true, guiWeather )
		guiWeatherNowLabel =  guiCreateLabel  ( 0.25 , 0.5, 0.5, 0.1, "Aktuális időjárás: Forróság", true, guiWeather )
	elseif(weather == "forró") then
		guiWeatherImage =   guiCreateStaticImage( 0.3 , 0.1, 0.4 , 0.4, "rain.png", true, guiWeather )
		guiWeatherNowLabel =  guiCreateLabel  ( 0.25 , 0.5, 0.5, 0.1, "Aktuális időjárás: Eső", true, guiWeather )
	elseif(weather == "köd") then
		guiWeatherImage =   guiCreateStaticImage( 0.3 , 0.1, 0.4 , 0.4, "foggy.png", true, guiWeather )
		guiWeatherNowLabel =  guiCreateLabel  ( 0.25 , 0.5, 0.5, 0.1, "Aktuális időjárás: Ködös", true, guiWeather )
	end

	guiLabelSetHorizontalAlign ( guiWeatherNowLabel , "center")
	
	guiWeatherLable =  guiCreateLabel  ( 0.05 , 0.6, 1, 0.1, "Nyomj a gombra amilyen időjárást szeretnél", true, guiWeather )
	
	HotButton = guiCreateButton( 0.05, 0.72, 0.15, 0.1, "forró", true, guiWeather )
	ClearButton = guiCreateButton( 0.23, 0.72, 0.15, 0.1, "Tiszta", true, guiWeather )
	CloudyButton = guiCreateButton( 0.41, 0.72, 0.15, 0.1, "felhős", true, guiWeather )
	FogButton = guiCreateButton( 0.59, 0.72, 0.15, 0.1, "köd", true, guiWeather )
	RainButton = guiCreateButton( 0.77, 0.72, 0.15, 0.1, "eső", true, guiWeather )
	
	ExitButton = guiCreateButton( 0.4, 0.85, 0.2, 0.1, "vissza", true, guiWeather )
...
function createWeatherForecaseGUI(weather, nextWeather, laterWeather)
	...
	guiWeatherLaterTab = guiCreateTab ( "Later", guiWeatherForcecastTab)
		...
		guiWeatherForecastLaterBack = guiCreateButton( 0.4, 0.85, 0.2, 0.1, "Vissza", true, guiWeatherLaterTab)
